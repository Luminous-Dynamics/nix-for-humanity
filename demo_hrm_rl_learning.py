...
def simulate_user_session(hrm_rl: HRMwithRL, num_queries: int = 100):
    ...
    query_templates = [
        ...
        (
            ...
            ["docker", "virtualbox", "cuda", "bluetooth"],
        ),
    ...
    for i in range(num_queries):
        ...
        rating = np.clip(
            base_rating + learning_bonus + strategy_bonus + noise - 0.5, -1, 1
        )
        ...
        feedback = {
            "rating": rating,
            "success": success,
            "time_taken": np.random.uniform(0.1, 2.0)
            if success
            else np.random.uniform(2.0, 5.0),
        ...
        if (i + 1) % 10 == 0:
            ...
            print(
                f"  Episode {i+1:3d}: Avg Reward={avg_recent_reward:+.3f}, "
...
def plot_learning_curves(results: dict, save_path: str = "hrm_rl_learning_curves.png"):
    ...
    ax1.plot(
        range(window, len(results["rewards"]) + 1),
        moving_avg,
        color="red",
        linewidth=2,
    ...
    ax2 = axes[0, 1]
    ax2.plot(
        episodes, np.array(results["success_rates"]) * 100, color="green", linewidth=2
    )
    ...
    ax3.plot(
        episodes,
        np.array(results["exploration_rates"]) * 100,
        color="orange",
        linewidth=2,
    ...
    ax3.plot(
        episodes,
        np.array(results["confidences"]) * 100,
        color="purple",
        linewidth=2,
...
def main():
    ...
    print(
        f"  Reward improvement: {initial_reward:+.3f} → {final_reward:+.3f} "
    ...
    print(
        f"  Success rate improvement: {initial_success:.1%} → {final_success:.1%} "
