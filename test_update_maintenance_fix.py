...
def compare_versions():
    ...
    versions = {
        "v0.2.0-beta": {"overall": 80, "dev": 0, "update": 50, "other": 100},
        "v0.2.1": {"overall": 85, "dev": 100, "update": 50, "other": 100},
        "v0.2.2": {"overall": 90, "dev": 100, "update": 95, "other": 100},
    }

    print("Category     | v0.2.0 | v0.2.1 | v0.2.2")
    print("-------------|--------|--------|--------")
    print(
        f"Dev Queries  |   {versions['v0.2.0-beta']['dev']:3}% |  {versions['v0.2.1']['dev']:3}% |  {versions['v0.2.2']['dev']:3}%"
    )
    print(
        f"Update       |   {versions['v0.2.0-beta']['update']:3}% |  {versions['v0.2.1']['update']:3}% |  {versions['v0.2.2']['update']:3}%"
    )
    print(
        f"Other        |   {versions['v0.2.0-beta']['other']:3}% |  {versions['v0.2.1']['other']:3}% |  {versions['v0.2.2']['other']:3}%"
    )
    print(
        f"**Overall**  | **{versions['v0.2.0-beta']['overall']:3}%** | **{versions['v0.2.1']['overall']:3}%** | **{versions['v0.2.2']['overall']:3}%**"
    )

    print("\nImprovements:")
    print(
        f"• v0.2.0 → v0.2.1: +{versions['v0.2.1']['overall'] - versions['v0.2.0-beta']['overall']}% (dev fix)"
    )
    print(
        f"• v0.2.1 → v0.2.2: +{versions['v0.2.2']['overall'] - versions['v0.2.1']['overall']}% (update fix)"
    )
    print(
        f"• Total gain: +{versions['v0.2.2']['overall'] - versions['v0.2.0-beta']['overall']}%"
    )
...
def generate_training_data():
    ...
    for query, command in examples[:10]:  # Show first 10
        ...
        training_data.append(
            {"query": query, "command": command, "category": "update_maintenance"}
        )
...
def main():
    ...
    print("• Solution: Pattern-based specialist with safety")
    print("• Integration: HRM v4 ready")
