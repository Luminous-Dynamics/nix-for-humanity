...
use super::hdc_ltc_unified::{HdcLtcUnifiedNeuron, UnifiedConfig, UnifiedActivation};
...
mod hebbian_tests {
    ...
    fn test_hebbian_association_strengthening() {
        ...
        assert!(weight_change > 0.01,
            "Hebbian learning should modify weights (change: {})", weight_change);
    }
    ...
    fn test_hebbian_multiple_patterns() {
        ...
        println!("Response similarities: AB={:.3}, BC={:.3}, AC={:.3}",
                 sim_ab, sim_bc, sim_ac);

        // Responses to different patterns should be reasonably distinct
        assert!(sim_ab < 0.95 && sim_bc < 0.95 && sim_ac < 0.95,
    ...
    fn test_learning_rate_sensitivity() {
        ...
        for &lr in &learning_rates {
            ...
            println!("LR={:.3}: weight_norm={:.3}, stable={}", lr, weight_norm, is_stable);
        }
...
mod contrastive_tests {
    ...
    fn test_contrastive_positive_attraction() {
        ...
        println!("Positive similarity: {:.4} -> {:.4}", sim_pos_before, sim_pos_after);
        println!("Negative similarity: {:.4} -> {:.4}", sim_neg_before, sim_neg_after);
...
mod pattern_completion_tests {
    ...
    fn test_pattern_completion_basic() {
        ...
        let partials: Vec<ContinuousHV> = patterns.iter()
...
mod weight_normalization_tests {
    ...
    fn test_weight_bounded_growth() {
        ...
        assert!(final_norm <= 2.5, "Weights should be bounded (norm={})", final_norm);
        ...
        if !post_init_norms.is_empty() {
            ...
            assert!(max_norm <= 2.5, "Weight norm should stay bounded after first update (max={})", max_norm);
        }
    }
...
mod stdp_tests {
    ...
    fn test_stdp_positive_timing() {
        ...
        assert!(weight_change > 0.01, "Positive timing should modify weights");
    }
    ...
    fn test_stdp_negative_timing() {
        ...
        assert!(weight_change > 0.001, "Negative timing should modify weights");
    }
...
mod advanced_learning_tests {
    ...
    fn test_regularized_hebbian() {
        ...
        println!("Final state activity: {:.4} (target: {:.4})", final_activity, target_activity);
    ...
    fn test_triplet_learning() {
        ...
        println!("Before - Pos: {:.4}, Neg: {:.4}", sim_to_pos_before, sim_to_neg_before);
        println!("After  - Pos: {:.4}, Neg: {:.4}", sim_to_pos_after, sim_to_neg_after);
    ...
    fn test_adaptive_update() {
        ...
        assert!(weight_change > 0.01, "Adaptive updates should modify weights");
    ...
    fn test_learning_method_comparison() {
        ...
        for _ in 0..30 {
            n1.reset();
            for _ in 0..10 { n1.evolve(0.01, &pattern); }
            n1.hebbian_update(&pattern, Some(0.02));
        }
        println!("Hebbian\t\t\t{:.4}\t\t{:.4}",
                 1.0 - w1_before.similarity(n1.weight_hv_ref()), n1.weight_hv_ref().norm());
        ...
        for _ in 0..30 {
            n2.reset();
            for _ in 0..10 { n2.evolve(0.01, &pattern); }
            n2.contrastive_update(&pattern, &negative, 0.02);
        }
        println!("Contrastive\t\t{:.4}\t\t{:.4}",
                 1.0 - w2_before.similarity(n2.weight_hv_ref()), n2.weight_hv_ref().norm());
        ...
        println!("STDP\t\t\t{:.4}\t\t{:.4}",
                 1.0 - w3_before.similarity(n3.weight_hv_ref()), n3.weight_hv_ref().norm());
        ...
        for _ in 0..30 {
            n4.reset();
            for _ in 0..10 { n4.evolve(0.01, &pattern); }
            n4.regularized_hebbian_update(&pattern, 0.02, 1.0);
        }
        println!("Regularized Hebbian\t{:.4}\t\t{:.4}",
                 1.0 - w4_before.similarity(n4.weight_hv_ref()), n4.weight_hv_ref().norm());
...
pub fn benchmark_learning_strategy(
    ...
    let variance: f32 = stability_window.iter()
        .map(|x| (x - mean).powi(2))
        .sum::<f32>() / stability_window.len() as f32;
...
mod benchmarks {
    ...
    fn run_learning_benchmarks() {
        ...
        let results = vec![
            ...
            benchmark_learning_strategy("Contrastive Only", test_config(), &patterns, 50, false, true),
