...
mod tests {
    ...
    fn test_hierarchical_hopfield() {
        ...
        // Should be more similar to cat than car (retrieved from same category)
        assert!(
            cat.similarity(&retrieved) > car.similarity(&retrieved),
            "Hierarchical retrieval respects categories: cat similarity {} should be > car similarity {}",
            cat.similarity(&retrieved),
            car.similarity(&retrieved)
        );
