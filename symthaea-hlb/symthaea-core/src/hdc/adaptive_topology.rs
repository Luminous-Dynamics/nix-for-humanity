...
pub enum CognitiveMode {
    ...
    /// "sleep" cycles for memory consolidation and novel pattern discovery.
    Dreaming,

    /// Meditative mode (35-40% bridges) - High internal coherence, low reactivity
    ///
    /// Introspective state with reduced external responsiveness.
    /// Optimized for self-reflection and internal coherence maintenance.
    Meditative,

    /// Flow state (45-50% bridges) - Optimal task engagement
    ///
    /// Merged action-awareness with high temporal continuity.
    /// Represents peak performance with effortless concentration.
    Flow,

    /// Social mode (55-65% bridges) - Theory of Mind emphasis
    ///
    /// Heightened interpersonal processing with emotional attunement.
    /// Prioritizes empathic resonance and social cognition.
    Social,

    /// Vigilant mode (25-30% bridges) - Threat detection priority
    ///
    /// Heightened attention with fast reaction pathways.
    /// Reduced integration for rapid threat assessment.
    Vigilant,

    /// Playful mode (60-70% bridges) - Novelty-seeking exploration
    ///
    /// Creative experimentation with low-stakes exploration.
    /// High integration for novel associations, reduced inhibition.
    Playful,
}
...
impl CognitiveMode {
    ...
    pub fn description(&self) -> &'static str {
        match self {
            CognitiveMode::Balanced => "General processing with balanced integration",
            CognitiveMode::Exploratory => "Creative and exploratory thinking",
            CognitiveMode::Focused => "Analytical and convergent processing",
            CognitiveMode::GlobalAwareness => "Maximum integration for holistic awareness",
            CognitiveMode::DeepSpecialization => "Minimum integration for deep focus",
            CognitiveMode::PhiGuided => "Adaptive mode guided by Φ gradient",
            CognitiveMode::Dreaming => "Memory consolidation and creative recombination",
            CognitiveMode::Meditative => "Introspective state with high internal coherence",
            CognitiveMode::Flow => "Optimal task engagement with merged action-awareness",
            CognitiveMode::Social => "Interpersonal processing with emotional attunement",
            CognitiveMode::Vigilant => "Heightened attention for threat detection",
            CognitiveMode::Playful => "Novelty-seeking with low-stakes exploration",
        }
    }

    /// Get all available cognitive modes
    pub fn all() -> &'static [CognitiveMode] {
        &[
            ...
            CognitiveMode::PhiGuided,
            CognitiveMode::Dreaming,
            CognitiveMode::Meditative,
            CognitiveMode::Flow,
            CognitiveMode::Social,
            CognitiveMode::Vigilant,
            CognitiveMode::Playful,
        ]
    }
    ...
    pub fn suggest_for_context(
        ...
        // High positive emotion + moderate complexity → Flow
        if emotional_valence > 0.5 && task_complexity > 0.3 && task_complexity < 0.7 {
            return CognitiveMode::Flow;
        }

        // Very positive emotion + low complexity → Playful
        if emotional_valence > 0.6 && task_complexity < 0.3 {
            return CognitiveMode::Playful;
        }

        // Neutral emotion + low task demand → Meditative
        if emotional_valence.abs() < 0.2 && task_complexity < 0.2 {
            return CognitiveMode::Meditative;
        }

        // High complexity → Focused or DeepSpecialization
        if task_complexity > 0.8 {
            return CognitiveMode::DeepSpecialization;
        }
