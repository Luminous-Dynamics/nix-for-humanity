...
impl CausalMind {
    ...
    // =========================================================================
    // GROUNDED UNDERSTANDING BRIDGE
    // =========================================================================

    /// Learn causal structure from text using grounded understanding
    ///
    /// This is the bridge between text-based causal extraction and HDC causal space.
    /// Uses the rich linguistic markers from grounded_understanding to extract
    ...
    /// ```
    pub fn learn_from_grounded_text(&mut self, text: &str) -> GroundedCausalLearning {
        let text_markers = TextCausalMarkers::new();
        let lexicon = LexicalGrounding::new();

        let mut result = GroundedCausalLearning {
            relations_found: Vec::new(),
        ...
        // Extract causal relations using grounded understanding
        let relations = text_markers.extract_causal_relations(text);

        for relation in relations {
            // Map text-based relation type to strength
            let strength = Self::relation_to_strength(&relation.relation_type, relation.confidence);

            // Clean up cause and effect phrases - extract key concepts
            let cause_concepts = Self::extract_key_concepts(&relation.cause, &lexicon);
            let effect_concepts = Self::extract_key_concepts(&relation.effect, &lexicon);

            // Add causal links for each cause-effect pair
            for cause in &cause_concepts {
                for effect in &effect_concepts {
                    ...
                    // Track new concepts
                    if !result.concepts_added.contains(cause) {
                        result.concepts_added.push(cause.clone());
                    }
                    if !result.concepts_added.contains(effect) {
                        result.concepts_added.push(effect.clone());
                    }
                }
            }

            result.relations_found.push(GroundedRelation {
        ...
        // Also extract causal role information
        let roles = text_markers.extract(text);
        for (phrase, _role) in roles {
            // Create concept for each role-bearing phrase
            let _ = self.get_or_create_concept(&phrase);
            if !result.concepts_added.contains(&phrase) {
                result.concepts_added.push(phrase.clone());
            }
        }

        result.phi_delta = self.phi - phi_before;
        result
    }

    /// Convert text-based causal relation type to strength value
    fn relation_to_strength(relation_type: &TextCausalRelation, base_confidence: f32) -> f64 {
        let type_weight = match relation_type {
            TextCausalRelation::Causes => 0.9,      // Strong causal
            TextCausalRelation::Enables => 0.6,     // Enabling condition
            TextCausalRelation::Prevents => -0.7,   // Negative causation
            TextCausalRelation::Temporal => 0.3,    // Weak (temporal only)
            TextCausalRelation::Conditional => 0.5, // Medium (conditional)
        };
        (type_weight * base_confidence as f64).clamp(-1.0, 1.0)
    }

    /// Extract key concepts from a phrase using lexical grounding
    fn extract_key_concepts(phrase: &str, lexicon: &LexicalGrounding) -> Vec<String> {
        let words: Vec<&str> = phrase.split_whitespace().collect();

        // Filter to meaningful words (those in lexicon or nouns/verbs)
        ...
        for word in words {
            ...
            // Check if word is in lexicon (has semantic primes)
            if lexicon.decompose(&clean).is_some() {
                concepts.push(clean);
            } else if clean.len() >= 3 {
                // Include longer words as potential concepts
                concepts.push(clean);
            }
        }

        // If no concepts found, use the whole phrase
        if concepts.is_empty() && !phrase.trim().is_empty() {
            concepts.push(phrase.trim().to_lowercase());
        }

        concepts
    }

    /// Encode a text-extracted causal relation into the HDC space
    ///
    ...
    pub fn encode_causal_relation(&mut self, cause: &str, effect: &str, strength: f64) -> HV16 {
        ...
        // Encode: cause ⊗ CAUSES ⊗ effect ⊗ strength
        cause_hv
            .bind(&self.markers.causes)
            .bind(&effect_hv)
            .bind(strength_marker)
    }

    /// Query causal relations similar to a given text description
    pub fn query_similar_causation(&self, description: &str, top_k: usize) -> Vec<(String, f32)> {
        let text_markers = TextCausalMarkers::new();
        let relations = text_markers.extract_causal_relations(description);
        ...
        for (name, _concept) in &self.concepts {
            ...
            for relation in &relations {
                // Check similarity to both cause and effect
                let cause_words: Vec<&str> = relation.cause.split_whitespace().collect();
                let effect_words: Vec<&str> = relation.effect.split_whitespace().collect();

                for word in cause_words.iter().chain(effect_words.iter()) {
                    if name.contains(&word.to_lowercase()) {
            ...
            if max_sim > 0.0 {
                similarities.push((name.clone(), max_sim));
            }
        }
...
// =============================================================================
// GROUNDED CAUSAL LEARNING RESULTS
// =============================================================================

/// Results from learning causal structure from grounded text
#[derive(Debug, Clone)]
pub struct GroundedCausalLearning {
    /// Causal relations found in the text
    pub relations_found: Vec<GroundedRelation>,
    /// New concepts added to the causal space
    pub concepts_added: Vec<String>,
    /// Number of causal links added
    pub links_added: usize,
    /// Change in Φ (integrated information) from this learning
    pub phi_delta: f64,
}

/// A grounded causal relation extracted from text
#[derive(Debug, Clone)]
