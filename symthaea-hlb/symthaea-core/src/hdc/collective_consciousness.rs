...
impl CollectiveConsciousness {
    ...
    fn compute_topology_metrics(&self) -> TopologyMetric {
        ...
        // 3. Clustering coefficient (local clustering averaged)
        // For each node, measure how connected its neighbors are to each other
        let clustering = {
            let mut total_clustering = 0.0;
            let mut nodes_with_neighbors = 0;

            for node in &agent_ids {
                // Find neighbors of this node (both incoming and outgoing edges)
                let neighbors: Vec<&String> = agent_ids
                    .iter()
                    .filter(|&other| {
                        other != node
                            && (adjacency.contains_key(&(node.clone(), other.clone()))
                                || adjacency.contains_key(&(other.clone(), node.clone())))
                    })
                    .collect();
                ...
                for i in 0..neighbors.len() {
                    for j in (i + 1)..neighbors.len() {
                        let ni = neighbors[i];
                        let nj = neighbors[j];
                        if adjacency.contains_key(&(ni.clone(), nj.clone()))
                            || adjacency.contains_key(&(nj.clone(), ni.clone()))
                        {
                ...
                // Local clustering coefficient = actual / possible triangles
                let possible_edges = k * (k - 1) / 2;
                if possible_edges > 0 {
                    total_clustering += edges_between_neighbors as f64 / possible_edges as f64;
        ...
        // 4. Average path length (BFS-based)
        // For each pair of nodes, compute shortest path using BFS
        let avg_path_length = {
            ...
            for start in &agent_ids {
                ...
                while let Some(current) = queue.pop_front() {
                    ...
                    for neighbor in &agent_ids {
                        ...
                        // Check if edge exists (directed)
                        if adjacency.contains_key(&(current.clone(), neighbor.clone())) {
            ...
            if reachable_pairs > 0 {
                total_path_length / reachable_pairs as f64
            } else {
                f64::INFINITY // No paths exist
            }
        };
