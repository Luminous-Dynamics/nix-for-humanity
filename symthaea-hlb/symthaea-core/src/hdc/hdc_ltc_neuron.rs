...
pub struct HdcLtcConfig {
    ...
    /// Momentum for gradient updates (also used as beta1 for Adam)
    pub momentum: f32,

    /// L2 regularization strength
    pub weight_decay: f32,

    /// Adam beta2 for second moment estimate (default: 0.999)
    pub adam_beta2: f32,
...
pub struct HdcLtcNeuron {
    ...
    /// First moment estimate (m) for Adam optimizer / momentum for SGD
    weight_momentum: ContinuousHV,
    /// Second moment estimate (v) for Adam optimizer
    weight_velocity: ContinuousHV,
    /// Adam timestep counter for bias correction
    adam_timestep: u64,
    /// Input momentum for future advanced learning algorithms
    #[allow(dead_code)]
...
impl HdcLtcNeuron {
    ...
    pub fn new(config: HdcLtcConfig, seed: u64) -> Self {
        ...
        Self {
            ...
            weight_velocity: ContinuousHV::zero(dim),
            adam_timestep: 0,
    ...
    /// Adam optimizer update - Advanced learning algorithm
    ///
    /// Implements the Adam optimization algorithm for hypervector learning:
    /// - Uses adaptive learning rates based on first and second moment estimates
    /// - Includes bias correction for initial timesteps
    /// - Generally more stable than plain momentum SGD
    ///
    /// # Arguments
    /// * `input` - Input hypervector for computing correlation gradient
    /// * `learning_rate` - Optional learning rate override
    pub fn adam_update(&mut self, input: &ContinuousHV, learning_rate: Option<f32>) {
        let lr = learning_rate.unwrap_or(self.config.learning_rate);
        let beta1 = self.config.momentum; // Use momentum as beta1
        let beta2 = self.config.adam_beta2;
        let epsilon = self.config.adam_epsilon;

        // Increment timestep
        self.adam_timestep += 1;
        let t = self.adam_timestep as f32;

        // Compute gradient: correlation between input and state (Hebbian)
        let gradient = input.bind(&self.state);
        ...
        let m_hat = self.weight_momentum.scale(1.0 / bias_correction1);

        // Compute update: lr * m_hat / (sqrt(v_hat) + epsilon)
        // We need element-wise division with stabilization
        let v_hat_scaled = self.weight_velocity.scale(1.0 / bias_correction2);
    ...
    ///
    /// Uses running average of squared gradients for adaptive learning rates.
    /// Simpler than Adam but often effective.
    ///
    /// # Arguments
    /// * `input` - Input hypervector for computing correlation gradient
    /// * `learning_rate` - Optional learning rate override
    pub fn rmsprop_update(&mut self, input: &ContinuousHV, learning_rate: Option<f32>) {
        let lr = learning_rate.unwrap_or(self.config.learning_rate);
        let rho = self.config.adam_beta2; // Use beta2 as RMS decay rate
        let epsilon = self.config.adam_epsilon;
