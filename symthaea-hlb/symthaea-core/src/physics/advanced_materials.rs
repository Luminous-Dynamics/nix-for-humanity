...
impl AdvancedMaterials {
    ...
    fn init_max_phases(&mut self, _genesis: &GenesisSeed) {
        ...
        for (name, m, a, x, n, kink, delam, max_t, thermal) in max_data {
            ...
            let healing_score = (kink_factor * 0.4 + delam_factor * 0.4 + thermal_factor * 0.2) as f32;

            let vector = ContinuousHV::weighted_bundle(
                &[&self.max_phase, &self.kink_band, &self.delamination, &self.thermal_healing],
                &[1.0, kink_factor as f32, delam_factor as f32, thermal_factor as f32],
            );
    ...
    fn init_nano_laminates(&mut self, _genesis: &GenesisSeed) {
        ...
        for (name, a, b, spacing, interface, sink, dpa) in laminate_data {
            ...
            let healing_score = (sink_factor * 0.5 + dpa_factor * 0.3 + spacing_factor * 0.2) as f32;
    ...
    pub fn analyze_nano_laminate(
        ...
        let h_compatible = laminate.layer_a == 46 || laminate.layer_b == 46
            || laminate.layer_a == 22 || laminate.layer_b == 22
            || laminate.layer_a == 40 || laminate.layer_b == 40;
...
mod tests {
    ...
    fn test_max_phases_created() {
        ...
        assert!(materials.max_phases.len() >= 10, "Should have at least 10 MAX phases");
    }

    #[test]
    fn test_nano_laminates_created() {
        let (_, materials, _) = setup();
        assert!(materials.nano_laminates.len() >= 10, "Should have at least 10 nano-laminates");
    }
    ...
    fn test_ti3sic2_properties() {
        ...
        let ti3sic2 = materials.max_phases.iter()
        ...
        assert!(ti3sic2.healing_score > 0.5, "Ti3SiC2 should have good healing");
    }
    ...
    fn test_cu_nb_properties() {
        ...
        let cu_nb = materials.nano_laminates.iter()
            .find(|l| l.name == "Cu/Nb")
            .expect("Cu/Nb should exist");

        assert!(cu_nb.dpa_tolerance > 100.0, "Cu/Nb should have high DPA tolerance");
        assert!(cu_nb.healing_score > 0.7, "Cu/Nb should have excellent healing");
    }
    ...
    fn test_material_ranking() {
        ...
        assert!(rankings[0].lcf_score > 0.5, "Top material should have good score");
    }
    ...
    fn test_h_compatible_laminates() {
        ...
        let pd_nb = materials.nano_laminates.iter()
            .find(|l| l.name == "Pd/Nb")
