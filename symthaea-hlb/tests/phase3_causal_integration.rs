...
use symthaea::observability::{
    ...
    CausalTraceObserver, WithCausalMetadata,
...
fn test_scenario_2_parallel_processing() {
    ...
    assert!(effects_of_security.len() >= 2, "Security should cause at least 2 effects");
...
fn test_scenario_3_deep_causal_chain() {
    ...
    observer.record_workspace_ignition(WorkspaceIgnitionEvent {
        timestamp: Utc::now(),
        phi: 0.80,
        free_energy: 0.15,
        coalition_size: 5,
    ...
    observer.record_primitive_activation(PrimitiveActivationEvent {
        timestamp: Utc::now(),
    ...
    observer.record_response_generated(ResponseGeneratedEvent {
        timestamp: Utc::now(),
