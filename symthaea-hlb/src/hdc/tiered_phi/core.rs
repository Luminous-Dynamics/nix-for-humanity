...
impl TieredPhi {
    ...
    fn check_cache(&mut self, components: &[HV16]) -> Option<f64> {
        ...
        // Find matching entry and its index
        let mut found_idx = None;
        for (idx, &(cached_n, cached_hash, _)) in self.cache.iter().enumerate() {
        ...
        // If found, move to end (LRU touch) and return value
    ...
    fn update_cache(&mut self, components: &[HV16], phi: f64) {
        ...
        // LRU eviction: remove least recently used (front) if at capacity
        // Note: check_cache() moves accessed entries to the end, so front is LRU
    ...
    pub fn compute_attribution(&mut self, components: &[HV16]) -> PhiAttribution {
        ...
        importance_ranking.sort_by(|&a, &b| {
            component_scores[b].partial_cmp(&component_scores[a]).unwrap_or(std::cmp::Ordering::Equal)
        });
...
pub fn set_global_tier(tier: ApproximationTier) {
    *GLOBAL_PHI.lock() = TieredPhi::new(tier);
}
