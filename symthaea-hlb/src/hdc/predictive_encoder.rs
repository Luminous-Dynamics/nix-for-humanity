...
pub struct PredictiveHdcEncoder {
    ...
    /// Shape: [target_dim x source_dim] - sparse matrix for efficient projection
    /// This preserves HDC orthogonality unlike averaging/subsampling
...
impl PredictiveHdcEncoder {
    ...
    pub fn new(config: PredictiveEncoderConfig) -> Self {
        ...
        // Generate Random Projection matrix (Johnson-Lindenstrauss)
        // For dimension reduction from HDC_DIMENSION (16384) to ~128-256
        // Using sparse random projection with values {-1, 0, +1} * sqrt(3/target_dim)
        // Sparsity ~2/3 zeros makes this efficient while preserving distances
        let projection_matrix = Self::generate_sparse_projection_matrix(
            256,                     // Default target dimension (will be resized on use)
            config.dimension,        // Source dimension (16384)
    ...
    /// Generate a sparse random projection matrix (Johnson-Lindenstrauss lemma)
    ///
    /// Uses the Achlioptas (2003) scheme: values are {-1, 0, +1} with probabilities
    /// {1/6, 2/3, 1/6}, scaled by sqrt(3/target_dim) for unit variance.
    ///
    /// This preserves pairwise distances with high probability while being
    /// computationally efficient (~2/3 of multiplications are skipped).
    fn generate_sparse_projection_matrix(target_dim: usize, source_dim: usize, seed: u64) -> Vec<Vec<f32>> {
        use std::collections::hash_map::DefaultHasher;
        use std::hash::{Hash, Hasher};

        let scale = (3.0_f32 / target_dim as f32).sqrt();
        ...
        for i in 0..target_dim {
            ...
            for j in 0..source_dim {
                ...
                // Achlioptas distribution: P(-1)=1/6, P(0)=2/3, P(+1)=1/6
                let val = match h % 6 {
                    0 => -scale,      // 1/6 probability
                    5 => scale,       // 1/6 probability
                    _ => 0.0_f32,     // 4/6 = 2/3 probability
                };
                row.push(val);
            }
            matrix.push(row);
    ...
    /// Get compressed representation for LTC input using Random Projection
    ///
    /// Uses Johnson-Lindenstrauss random projection to reduce dimensionality
    /// while preserving the orthogonality structure that HDC relies on.
    /// This is critical: averaging/subsampling destroys HDC's semantic properties!
    ///
    /// The projection is: y = P * x where P is [output_dim x input_dim]
    /// Distance preservation: ||Px - Py|| ≈ ||x - y|| with high probability
    pub fn compress_for_ltc(&self, hdv: &RealHV, output_dim: usize) -> Vec<f32> {
        // If output_dim matches our precomputed matrix, use direct projection
        if output_dim <= self.projection_matrix.len() {
            // Use the first output_dim rows of the projection matrix
            let mut result = Vec::with_capacity(output_dim);

            for row in self.projection_matrix.iter().take(output_dim) {
                // Dot product: sum(row[j] * hdv[j]) for non-zero entries
                // Note: sparse matrix means ~2/3 of terms are 0, so we skip them
                let mut sum = 0.0_f32;
                for (j, &val) in row.iter().enumerate() {
                    if val != 0.0 && j < hdv.values.len() {
                        sum += val * hdv.values[j];
                    }
                }
                result.push(sum);
            }
            ...
            // Fallback: generate a new projection on the fly for larger dims
            // This is less efficient but handles edge cases
            let projection = Self::generate_sparse_projection_matrix(output_dim, hdv.values.len(), 42);
