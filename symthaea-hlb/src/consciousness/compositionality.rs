...
impl CompositionalityEngine {
    ...
    /// Compute harmonic contributions for a composition.
    ///
    /// Returns a map of harmonic name -> delta adjustment.
    /// The caller can apply these to whatever harmonic field representation they use.
    /// Harmonic names: "IntegralWisdom", "ResonantCoherence", "EvolutionaryProgression"
    pub fn harmonic_contributions(&self, composition_id: &str) -> HashMap<String, f64> {
        let mut contributions = HashMap::new();
        if let Some(composed) = self.composed_primitives.get(composition_id) {
            let phi = composed.metadata.expected_phi_contribution as f64;
            contributions.insert("IntegralWisdom".to_string(), phi * 0.4);
            contributions.insert("ResonantCoherence".to_string(), phi * 0.3);

            if matches!(composed.composition_type, CompositionType::FixedPoint { .. }) {
                *contributions.entry("EvolutionaryProgression".to_string()).or_insert(0.0) += 0.1;
            }

            if matches!(composed.composition_type, CompositionType::HigherOrder) {
                *contributions.entry("IntegralWisdom".to_string()).or_insert(0.0) += 0.1;
            }
        }
        contributions
    }
