...
struct IntegrationMetrics {
    ...
    spectral_ratio: f64,  // Alpha/Theta ratio: >1.0=Wake, <1.0=REM
}
...
/// Sleep Sentinel - Dual-channel LTC-based consciousness detector
struct SleepSentinel {
    ...
    // This is the "Third Dimension" that separates the REM paradox
...
impl SleepSentinel {
    fn new(sample_rate: f64) -> Self {
        Self {
            ...
            state_frontal: 0.0,
            state_occipital: 0.0,
            // Multi-Scale LTC for Wake/REM spectral discrimination
    ...
    fn process_sample(&mut self, frontal: f64, occipital: f64) {
        // Add to buffers
        ...
        // This extracts the "spectral texture" that separates Wake (Alpha) from REM (Theta)
        self.theta_alpha.process(frontal, dt);

        // Adapt tau based on signal dynamics (zero-crossings as frequency proxy)
        // Faster signals -> lower tau, slower signals -> higher tau
    }
    ...
    fn classify(&mut self) -> (ConsciousnessState, IntegrationMetrics) {
        if self.frontal_buffer.len() < 100 {
            return (ConsciousnessState::Awake, IntegrationMetrics {
                complexity: 0.0, synchrony: 0.0, dominant_freq: 0.0, phi_proxy: 0.0, spectral_ratio: 1.0,
            });
        }
        ...
        // R > 1.0: Fast activity dominates (Wake - Alpha/Beta)
        // R < 1.0: Slow activity dominates (REM - Theta)
        ...
        // Classification using IIT-inspired Φ = Integration × Differentiation
        // PLUS: Multi-Scale LTC Spectral Discriminator for Wake/REM
        //
        // INTEGRATION (Synchrony):
        //   N3: HIGH sync (0.45-0.48) - global delta synchronization
        //   Wake: LOW sync (0.19-0.20) - desynchronized activity
        //
        // DIFFERENTIATION (Permutation Entropy):
        //   Wake: HIGH PE (0.7-0.9) - chaotic, unpredictable patterns
        //   N3: LOW PE (0.3-0.5) - monotonic, predictable delta waves
        //   REM: MEDIUM-HIGH PE (0.6-0.8) - complex but structured
        //
        // SPECTRAL TEXTURE (Multi-Scale LTC Ratio):
        //   Wake: R > 1.0 - Fast activity (Alpha/Beta 8-25 Hz)
        //   REM:  R < 1.0 - Slow activity (Theta 4-8 Hz)
        //
        // The key insight: We now have THREE dimensions for classification!

        // EMPIRICAL THRESHOLDS (from SC4001 feature statistics):
        //   Synchrony:  N3=0.453, N2=0.392, N1=0.315, REM=0.272, Wake=0.200
        //   Complexity: N3=0.932, N2=0.970, N1=0.992, REM=0.993, Wake=0.984
        //
        // Key insight: N3 has LOWEST complexity (0.932) - monotonic delta waves
        //              Wake/REM have HIGHEST complexity (0.98-0.99) - chaotic
        //              BUT: Wake has R > 1.0 (Alpha), REM has R < 1.0 (Theta)

        let state = if metrics.synchrony > 0.40 && metrics.complexity < 0.96 {
            // HIGH sync (>0.40) + LOW complexity (<0.96) = Deep Sleep (N3)
            // N3: sync=0.453 > 0.40, complexity=0.932 < 0.96 ✓
            ConsciousnessState::DeepSleep
        } else if metrics.complexity > 0.975 && metrics.synchrony < 0.32 {
            // HIGH complexity + LOW sync = "High Entropy State" (Wake OR REM)
            //
            // EMPIRICAL FINDING from SC4001:
            //   Wake: sync=0.200 ± 0.123, R=3.88 ± 0.40
            //   REM:  sync=0.272 ± 0.110, R=4.28 ± 0.35
            //
            // Balanced approach: Use midpoint thresholds
            //   R threshold: 4.08 (midpoint between 3.88 and 4.28)
            //   Sync threshold: 0.236 (midpoint between 0.200 and 0.272)
            //
            // Classify as REM when BOTH conditions met:
            if spectral_ratio > 4.08 && metrics.synchrony > 0.236 {
                ConsciousnessState::REM
            } else {
                ConsciousnessState::Awake
            }
        } else {
            // Everything else = Light Sleep (N1/N2)
            // N2: sync=0.392, complexity=0.970
    ...
    fn compute_metrics(&self, frontal: &[f64], occipital: &[f64]) -> IntegrationMetrics {
        ...
        IntegrationMetrics {
            ...
            dominant_freq,
            phi_proxy,
            spectral_ratio: 1.0, // Placeholder, set by classify() from theta_alpha
        }
    }

    /// Reset the Multi-Scale LTC for a new epoch
...
fn main() {
    ...
    let occipital = psg.get_signal("Pz-Oz")
        .or_else(|| psg.get_signal("EOG"))  // Fallback to EOG
        .unwrap_or(frontal);  // Ultimate fallback: use frontal for both
    ...
    for epoch_idx in 0..max_epochs {
        ...
        if end > frontal.data.len() || end > occipital.data.len() {
            break;
        }
        ...
        sentinel.reset_spectral();  // Reset Multi-Scale LTC for fresh epoch analysis

        // Stream epoch through sentinel (decimated 5x for speed)
        for i in (start..end).step_by(5) {
            sentinel.process_sample(frontal.data[i], occipital.data[i]);
        }
        ...
        // Progress
        if epoch_idx % 50 == 0 {
            print!("\r   Epoch {}/{} | Acc: {:.1}% | Current: {} → {} (φ={:.2}, R={:.2})",
                epoch_idx + 1, max_epochs,
                100.0 * correct as f64 / total.max(1) as f64,
                actual.name(), predicted.name(), metrics.phi_proxy, metrics.spectral_ratio);
        }
    }
    ...
    println!("   Stage    │ Synchrony (μ±σ)  │ Complexity (μ±σ) │ Spectral R (μ±σ) │");
    println!("   ─────────┼──────────────────┼──────────────────┼──────────────────┤");
    ...
    for (i, name) in class_names.iter().enumerate() {
        ...
        println!("   {:8} │ {:.3} ± {:.3}       │ {:.3} ± {:.3}       │ {:.2} ± {:.2}        │",
            name, sync_mean, sync_std, comp_mean, comp_std, spectral_mean, spectral_std);
    }
    println!("   ─────────┴──────────────────┴──────────────────┴──────────────────┘\n");
    ...
    println!("      4. Fine-tune spectral ratio threshold for Wake/REM");
}
