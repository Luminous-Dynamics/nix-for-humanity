...
def search_packages(query: tuple, limit: int, persona: str | None):
    ...
    console.print(
        f"   [dim]or[/dim] [green]nix profile install nixpkgs#{results[0].name}[/green]"
    )
...
[cyan]Platforms:[/cyan] {', '.join(info.platforms)}"""
    ...
    panel = Panel(content, title=f"📦 {info.name}", border_style="blue", box=box.ROUNDED)
...
def find_by_command(command: str, persona: str | None):
    ...
    for i, match in enumerate(suggestions, 1):
        ...
        console.print(
            f"   Install: [green]nix profile install nixpkgs#{match.name}[/green]\n"
        )
