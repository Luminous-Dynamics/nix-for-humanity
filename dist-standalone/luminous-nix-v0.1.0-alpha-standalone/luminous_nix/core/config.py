...
class Config:
    ...
    def validate(self) -> List[str]:
        ...
        if self.verbose > 0 and self.quiet:
            issues.append("Warning: Both --verbose and --quiet specified. Using --quiet.")
            self.verbose = 0
            
        # Preview mode should be safe
        if not self.preview and not self.force:
            issues.append("Info: Running in apply mode. Changes will be made to the system.")
    ...
    def _create_parser() -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            '--force', '-f',
            action='store_true',
            help='Force operation (same as --yes)'
        )
        ...
        parser.add_argument(
            '--version',
            action='version',
            version='Luminous Nix v0.1.0-alpha'
        )
