...
class ProfileMigration:
    ...
    def migrate_profile(self, dry_run: bool = False) -> Tuple[bool, str]:
        ...
        if dry_run:
            ...
            message += f"1. Found {len(installed_packages)} packages in nix-env profile\n"
        ...
        self.migration_marker.write_text(f"Migrated on {subprocess.check_output(['date']).decode().strip()}\n")
        
        message = f"Successfully migrated profile!\n"
        message += f"- Backed up old profile\n"
        message += f"- Initialized new nix profile\n"
        message += f"- Ready to use modern nix commands\n"
