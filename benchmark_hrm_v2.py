...
from luminous_nix.ai.hrm_reasoner import (
    HRMNixOSReasoner,
    ReasoningTask,
)
from luminous_nix.ai.hrm_reasoner_v2 import HRMv2NixOSReasoner
from luminous_nix.ai.hrm_reasoner_v2 import ReasoningTask as TaskV2
...
def benchmark_hrm_v2(queries: list[str], warm_cache: bool = False) -> dict:
    ...
    for query in queries:
        task = TaskV2(
            task_type="config" if "config" in query else "dependency", description=query
        )
...
def main():
    ...
    print(
        f"  HRM v2 (batch):   {batch_results['avg_time_per_query_us']:.2f}μs per query"
    )
    ...
    print(
        f"  Total time for {batch_results['total_queries']} queries: {batch_results['total_time_us']:.0f}μs"
    )
    ...
    print(
        f"✨ Accuracy: {accuracy_results['v2_avg_accuracy']:.1f}% (up from {accuracy_results['v1_avg_accuracy']:.1f}%)"
    )
    print(f"✨ Throughput: {batch_results['queries_per_second']:.0f} queries/second")
