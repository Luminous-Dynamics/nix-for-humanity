...
def main():
    ...
    results = {"imports": [], "commands": [], "performance": []}
    ...
    avg_time = sum(t for _, _, t in results["commands"] if t > 0) / len(
        results["commands"]
    )
    ...
    print(
        f"  Modules working:  {imports_working}/{imports_total} ({imports_working*100//imports_total}%)"
    )
    print(
        f"  Commands working: {commands_working}/{commands_total} ({commands_working*100//commands_total}%)"
    )
    ...
    if imports_working >= 6 and commands_working >= 3 and avg_time < 2000:
        ...
        print("  Ready for: Alpha testing with known limitations")
        ...
        print("  Ready for: Internal development only")
