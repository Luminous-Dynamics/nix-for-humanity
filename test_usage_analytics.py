...
def test_basic_analytics():
    ...
    for event_type, query, cache_hit, source, selected in test_events:
        response_time = random.uniform(0.5, 50) if not cache_hit else random.uniform(0.01, 1)
    ...
    if analytics.current_session['total_queries'] > 0:
        cache_hit_rate = (
            ...
            analytics.current_session['total_queries'] * 100
        )
...
def test_smart_cache_recommendations():
    ...
    packages = ["vim", "neovim", "firefox", "chromium", "git", "python3", 
                "nodejs", "rust", "gcc", "vscode", "emacs", "tmux"]
    ...
    if recommendations['predictive_prefetch']:
        ...
        for pred in recommendations['predictive_prefetch'][:3]:
            print(f"      After '{pred['after']}' → prefetch '{pred['prefetch']}' "
...
def test_performance():
    ...
    # Performance targets
    print("\n📈 Performance Summary:")
    print(f"   Event tracking: {'✅' if events_per_second > 100 else '❌'} "
          f"({events_per_second:.0f}/s, target: >100/s)")
    print(f"   Insights: {'✅' if insights_time < 100 else '❌'} "
          f"({insights_time:.2f}ms, target: <100ms)")
    print(f"   Recommendations: {'✅' if rec_time < 100 else '❌'} "
...
def main():
    ...
    # Cleanup
    test_files = [
        "/tmp/test_analytics.db",
        "/tmp/test_analytics_perf.db"
    ]
