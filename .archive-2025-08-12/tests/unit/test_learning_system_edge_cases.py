...
from tests.test_utils.learning_mocks import Interaction, PreferenceManager
...
class TestPreferenceManagerEdgeCases(unittest.TestCase):
    ...
    def test_default_db_path_creation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("pathlib.Path.home") as mock_home:
                ...
                expected_path = (
                    Path(temp_dir) / ".config" / "nix-for-humanity" / "learning.db"
                )
    ...
    def test_interaction_validation(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT LENGTH(query) FROM interactions WHERE query = ?", (long_string,)
        )
    ...
    def test_sql_injection_comprehensive(self):
        ...
        for injection_attempt in sql_injection_attempts:
            ...
            for table in expected_tables:
                self.assertIn(
                    table, tables, f"Table {table} should not be dropped by injection"
                )
    ...
    def test_data_type_coercion(self):
        ...
        for test_name, success_value in test_cases:
            try:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT success FROM interactions WHERE query = ?",
                    (f"test_{test_name}",),
                )
                result = cursor.fetchone()
                conn.close()
    ...
    def test_single_record_edge_cases(self):
        ...
        # Single interaction
        interaction = Interaction(
            "single query", "single", "response", True, helpful=True
        )
        self.ls.record_interaction(interaction)
    ...
    def test_disk_space_simulation(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT LENGTH(query) FROM interactions WHERE intent = 'huge'"
            )
            result = cursor.fetchone()
            conn.close()
                ...
                for i in range(50):
                    if operation_id % 2 == 0:
                        ...
                        self.ls.learn_preference(
                            f"user-{operation_id}", "concurrent_pref", f"value-{i}"
                        )
