...
class TestPerformanceMonitor(unittest.TestCase):
    ...
    def test_metric_recording(self):
        ...
        self.monitor.record_metric(
            "test_metric", 42.0, "ms", {"context": "test"}, ["tag1"]
        )
    ...
    def test_operation_timing(self):
        ...
        test_metrics = [
            m for m in self.monitor.metrics if "test_operation_time_ms" in m.name
        ]
    ...
    def test_threshold_management(self):
        ...
        self.monitor.add_threshold(
            "test_metric", warning=50.0, critical=100.0, callback=callback
        )

        # Verify threshold was added
        self.assertEqual(len(self.monitor.thresholds), len(self.monitor.thresholds))
        threshold = next(
            t for t in self.monitor.thresholds if t.metric_name == "test_metric"
        )
    ...
    def test_trinity_metrics_recording(self):
        ...
        self.assertEqual(
            len(self.monitor.trinity_metrics["human_validation_time_ms"]), 1
        )
        self.assertEqual(
            len(self.monitor.trinity_metrics["claude_implementation_time_ms"]), 1
        )
        self.assertEqual(
            len(self.monitor.trinity_metrics["llm_consultation_time_ms"]), 1
        )
        ...
        # Check values
        self.assertEqual(
            self.monitor.trinity_metrics["human_validation_time_ms"][0], 5000.0
        )
        self.assertEqual(
            self.monitor.trinity_metrics["claude_implementation_time_ms"][0], 2000.0
        )
        self.assertEqual(
            self.monitor.trinity_metrics["llm_consultation_time_ms"][0], 1000.0
        )
    ...
    def test_optimization_recommendations(self):
        ...
        recommendations = self.monitor._get_optimization_recommendations(
            "response_time_ms"
        )
    ...
    def test_performance_decorator(self):
        ...
        sync_metrics = [
            m for m in self.monitor.metrics if "test_function_time_ms" in m.name
        ]
        async_metrics = [
            m for m in self.monitor.metrics if "test_async_function_time_ms" in m.name
        ]
