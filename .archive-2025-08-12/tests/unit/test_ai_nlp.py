...
class TestNLPPipeline(unittest.TestCase):
    ...
    def test_analyze_sentiment(self):
        ...
        self.pipeline.analyze_sentiment = Mock(
            return_value={"polarity": 0.8, "subjectivity": 0.2}
        )
    ...
    def test_pattern_matching(self):
        ...
        self.pipeline.match_pattern = Mock(
            side_effect=lambda text, pattern: any(
                word in text.lower() for word in pattern.split("|")
            )
        )

        # Test patterns
        self.assertTrue(
            self.pipeline.match_pattern("install firefox", patterns["install_package"])
        )
        self.assertTrue(
            self.pipeline.match_pattern("update system", patterns["update_system"])
        )
        self.assertTrue(
            self.pipeline.match_pattern(
                "search for editors", patterns["search_package"]
            )
        )
