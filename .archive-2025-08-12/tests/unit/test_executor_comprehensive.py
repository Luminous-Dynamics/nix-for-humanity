...
class TestSafeExecutor:
    ...
    def executor_with_api(self, test_backend):
        ...
        executor.nix_api = type(
            "obj",
            (object,),
            {
                "build": lambda *args: test_backend.execute(
                    "nix", ["build"] + list(args)
                ),
                "switch_to_configuration": lambda *args: test_backend.execute(
                    "nixos-rebuild", ["switch"]
                ),
                "rollback": lambda profile: test_backend.rollback(
                    42
                ),  # Deterministic generation
            },
        )
        executor.nix_models = type(
            "obj",
            (object,),
            {
                "Action": type(
                    "Action", (object,), {"SWITCH": "switch", "BOOT": "boot"}
                ),
    ...
    def test_execute_install_package(self, executor):
        ...
        with patch.object(
            executor, "_execute_install", new_callable=AsyncMock
        ) as mock_install:
    ...
    def test_execute_update_system(self, executor):
        ...
        with patch.object(
            executor, "_execute_update", new_callable=AsyncMock
        ) as mock_update:
    ...
    def test_execute_search_package(self, executor):
        ...
        with patch.object(
            executor, "_execute_search", new_callable=AsyncMock
        ) as mock_search:
    ...
    def test_execute_rollback(self, executor):
        ...
        intent = Intent(
            type=IntentType.ROLLBACK, entities={}, confidence=0.94, raw_input="rollback"
        )
        plan = ["Rollback system"]

        with patch.object(
            executor, "_execute_rollback", new_callable=AsyncMock
        ) as mock_rollback:
    ...
    def test_execute_install_with_python_api(self, executor_with_api):
        ...
        with patch.object(
            executor_with_api, "_run_command", new_callable=AsyncMock
        ) as mock_run:
    ...
    def test_execute_install_with_progress(self, executor_with_progress):
        ...
        with patch.object(
            executor_with_progress, "_run_command", new_callable=AsyncMock
        ) as mock_run:
            mock_run.return_value = {
                "returncode": 0,
                "stdout": "Installed",
            ...
            self.assertTrue(result.success)
            progress_messages = [
                call[0] for call in executor_with_progress.progress_calls
            ]
            self.assertIn(
                any(msg for msg in progress_messages if "Installing neovim" in msg),
                [True, False],
            )
    ...
    def test_execute_update_with_python_api(self, executor_with_api):
        ...
        with patch.object(
            ...
            with patch.object(
                executor_with_api, "_create_rebuild_script"
            ) as mock_script:
    ...
    def test_execute_update_with_progress(self, executor_with_progress):
        ...
        with patch.object(
            ...
            with patch.object(
                executor_with_progress, "_create_rebuild_script"
            ) as mock_script:
                mock_script.return_value = Path("/tmp/rebuild.sh")
                mock_run.side_effect = [
                ...
                self.assertIn(
                    any(msg for msg in progress_messages if "Updating channels" in msg),
                    [True, False],
                )
                self.assertIn(
                    any(msg for msg in progress_messages if "Rebuilding system" in msg),
                    [True, False],
                )
    ...
    def test_execute_search_with_progress(self, executor_with_progress):
        ...
        with patch.object(
            ...
            mock_run.return_value = {"returncode": 0, "stdout": "Results", "stderr": ""}
            ...
            self.assertIn(
                any(
                    msg for msg in progress_messages if "Searching for 'editor'" in msg
                ),
                [True, False],
            )
    ...
    def test_execute_rollback_with_progress(self, executor_with_progress):
        ...
        with patch.object(
            ...
            self.assertIn(
                any(msg for msg in progress_messages if "Rolling back" in msg),
                [True, False],
            )
