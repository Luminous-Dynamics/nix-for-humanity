...
# v2.0+ feature: from luminous_nix.ai.xai_engine import XAIEngine, CausalFactor, ExplanationLevel
# v2.0+ feature: from luminous_nix.ai.advanced_learning import AdvancedLearningSystem, PreferencePair, LearningMode
...
class MockNLPPipeline:
                ...
                if intent == "install":
                    factors.append(
                        CausalFactor(
                            ...
                            0.7,
                            "positive",
                            "Recognized installation intent",
                            1,
                        )
                    )
    ...
    def record_feedback(
        self, user_id, user_input, preferred_response, rejected_response, strength=0.8
    ):
...
class TestAINLPIntegration(unittest.TestCase):
    ...
    def test_user_adaptation_works(self):
        ...
        detailed_user_model = self.nlp_pipeline.advanced_learning.get_user_model(
            "detailed_user"
        )
        ...
        self.assertGreaterEqual(
            len(detailed_result["response"]), len(result["response"])
        )
    ...
    def test_multi_user_isolation(self):
        ...
        user1_model = self.nlp_pipeline.advanced_learning.get_user_model(
            "technical_user"
        )
    ...
    def test_explanation_consistency(self):
        ...
        result1 = self.nlp_pipeline.process(
            "install firefox", user_id="consistency_user"
        )
        result2 = self.nlp_pipeline.process(
            "install firefox", user_id="consistency_user"
        )
    ...
    def test_progressive_learning(self):
        ...
        initial_metrics = self.nlp_pipeline.advanced_learning.get_learning_metrics(
            user_id
        )
        ...
        final_metrics = self.nlp_pipeline.advanced_learning.get_learning_metrics(
            user_id
        )
    ...
    def test_context_awareness(self):
        ...
        result = self.nlp_pipeline.process(
            "install firefox", user_id=user_id, context=context
        )
...
class TestAdvancedIntegrationScenarios(unittest.TestCase):
    ...
    def test_explanation_depth_adaptation(self):
        ...
        # Beginner user
        beginner_result = self.nlp_pipeline.process(
            "install firefox", user_id="beginner"
        )
