...
class TestRealNixOSOperations:
    ...
    def test_search_package_real(self, backend):
        ...
        assert any(
            cmd for cmd in response.commands if "search" in cmd.get("command", "")
        )
    ...
    async def test_search_packages_native(self, native_ops):
        ...
        result = await native_ops.execute_native_operation(
            NativeOperationType.SEARCH_PACKAGES, packages=["python3"]
        )
    ...
    async def test_native_operations_speed(self, native_ops):
        ...
        for op_type, max_ms in operations_to_test:
            ...
            assert (
                duration < max_ms
            ), f"{op_type.value} took {duration}ms, expected < {max_ms}ms"
            assert result is not None
    ...
    async def test_full_conversation_flow(self, backend):
        ...
        for query in queries:
            request = Request(
                query=query, context=Context(personality="friendly", execute=False)
            )
    ...
    def test_disk_usage_check(self, backend):
        ...
        request = Request(
            query="disk space", context=Context(personality="minimal", execute=False)
        )
        ...
        assert any(
            ...
            for word in ["disk", "space", "storage", "usage"]
        )
    ...
    async def test_garbage_collection_dry_run(self, native_ops):
        ...
        result = await native_ops.execute_native_operation(
            NativeOperationType.GARBAGE_COLLECT, options={"dry_run": True}
        )
        ...
        if result.success:
            ...
            assert (
                "space" in result.message.lower() or "nothing" in result.message.lower()
            )
...
class TestErrorScenarios:
    ...
    def test_empty_input(self, backend):
        ...
        request = Request(query="", context=Context(personality="friendly"))
    ...
    def test_very_long_input(self, backend):
        ...
        request = Request(
            query=long_query, context=Context(personality="minimal", execute=False)
        )
    ...
    def test_special_characters(self, backend):
        ...
        for query in queries:
            request = Request(
                query=query, context=Context(personality="minimal", execute=False)
            )
