...
class ResourceLease:
    ...
    @property
    def time_held(self) -> timedelta | None:
...
class ResourceInfo:
    ...
    description: str
    exclusive: bool = True
    max_lease_duration: timedelta | None = None
...
class ResourceSteward:
    ...
    def __init__(self, state_path: Path | None = None):
        ...
        self.state_path = (
            state_path or Path.home() / ".luminous-nix" / "resource-state.json"
        )
    ...
    async def request_lease(
        ...
        logger.info(
            f"Added {plugin_id} to queue for {resource_id} (position: {lease.queue_position})"
        )
    ...
    async def _grant_to_next(self, resource_id: str):
        ...
        if resource_info and resource_info.max_lease_duration:
            next_lease.expires_at = (
                next_lease.granted_at + resource_info.max_lease_duration
            )
        ...
        logger.info(
            f"Granted lease for {resource_id} to {next_lease.plugin_id} (was waiting {next_lease.time_waiting})"
        )
    ...
    def _save_state(self):
        ...
        state = {
            ...
            "active_leases": {
                rid: lease.to_dict() for rid, lease in self.active_leases.items()
            },
            "waiting_queues": {
    ...
    def _load_state(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded state: {len(self.resources)} resources, {len(self.active_leases)} active leases"
            )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        lease1 = await steward.request_lease(
            "voice-commander", "microphone", "Voice command input"
        )
        print(f"\nvoice-commander lease: {lease1.status.value}")

        # Plugin 2 requests microphone (will queue)
        lease2 = await steward.request_lease(
            "meditation-timer", "microphone", "Play meditation bell"
        )
        print(
            f"meditation-timer lease: {lease2.status.value}, position: {lease2.queue_position}"
        )
