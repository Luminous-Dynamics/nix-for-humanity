...
Provides the central interface for configuration management in Luminous Nix.
"""
...
from ..config.profiles import ProfileManager, UserProfile
from ..config.schema import ConfigSchema


class ConfigManager:
    """Central configuration management for Luminous Nix"""
    ...
    def save(self, path: str | None = None) -> bool:
        ...
        with self._lock:
            ...
            if not save_path:
                ...
                save_path = os.path.expanduser("~/.config/luminous-nix/config.yaml")
    ...
    def export(self, format: str = "yaml") -> str | None:
        ...
        try:
            ...
            if format == "yaml":
                ...
                self.logger.warning(
                    "YAML export requested but PyYAML not installed, using JSON"
                )
            ...
            if format == "toml":
                ...
                self.logger.warning(
                    "TOML export requested but toml not installed, using JSON"
                )
                return json.dumps(data, indent=2)
        ...
        try:
            ...
            if format == "yaml":
                if HAS_YAML:
                    ...
                    self.logger.warning(
                        "YAML import requested but PyYAML not installed, trying JSON"
                    )
                ...
                if HAS_TOML:
                    ...
                    self.logger.warning(
                        "TOML import requested but toml not installed, trying JSON"
                    )
                    parsed = json.loads(data)
            else:
