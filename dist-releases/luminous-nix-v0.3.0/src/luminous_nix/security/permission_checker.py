...
Permission Checker for Luminous Nix
...
class PermissionChecker:
    ...
    PROTECTED_PATHS = {
        "/etc/nixos": {"write": "root", "reason": "System configuration directory"},
        "/nix/store": {"write": "root", "reason": "Nix store is immutable"},
        "/boot": {"write": "root", "reason": "Boot configuration requires root"},
    }

    @classmethod
    def check_operation_permission(
        cls, operation: str, context: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        if "file_path" in context:
            return cls._check_file_permission(
                context["file_path"], context.get("mode", "read"), current_user
            )
        ...
        if operation == "modify-configuration":
            return cls._check_configuration_permission(current_user)
        if operation == "manage-service":
    ...
    @classmethod
    def _check_privileged_command(
        cls, command: list[str], user_info: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        if requires_priv:
            if user_info["can_sudo"]:
                return {
                    ...
                    "reason": cmd_info.get(
                        "reason", "Operation requires elevated privileges"
                    ),
                    "suggestions": [
                        f"This will run with sudo: sudo {' '.join(command)}"
                    ],
                }
            return {
                "allowed": False,
    ...
    def _check_file_permission(
        cls, file_path: str, mode: str, user_info: dict[str, Any]
    ) -> dict[str, Any]:
        """
        Check file access permissions
        """
        ...
        for protected_path, rules in cls.PROTECTED_PATHS.items():
            if str(path).startswith(protected_path):
                if (
                    mode == "write"
                    and rules.get("write") == "root"
                    and not user_info["is_root"]
                ):
                    return {
            ...
            if path.exists():
                ...
                if file_gid in [grp.getgrnam(g).gr_gid for g in user_info["groups"]]:
                    if (
                        ...
                        return {
                            "allowed": True,
                            "requires_elevation": False,
                            "reason": "Group permission allows access",
                        }
                ...
                if mode == "read" and file_mode & stat.S_IROTH:
                    return {
                        "allowed": True,
                        "requires_elevation": False,
                        "reason": "File is readable by all",
                    }
                if mode == "write" and file_mode & stat.S_IWOTH:
                    return {
                        "allowed": True,
                        "requires_elevation": False,
                        "reason": "File is writable by all",
                    }
    ...
    @classmethod
    def _check_package_installation_permission(
    ...
    @classmethod
    def _check_configuration_permission(
        cls, user_info: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    @classmethod
    def explain_permission_requirement(
        cls, operation: str, permission_result: dict[str, Any]
    ) -> str:
        """
        ...
        """
        if permission_result["allowed"] and not permission_result.get(
            "requires_elevation"
        ):
        ...
        base_explanation = explanations.get(
            operation, f"Operation '{operation}' permission check"
        )
...
def demo():
    ...
    for test in test_operations:
        ...
        print(
            f"  Requires elevation: {'Yes' if result.get('requires_elevation') else 'No'}"
        )
