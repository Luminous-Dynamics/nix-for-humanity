...
{user_extra}
  ...
  services.fail2ban.enable = true;''',
    ...
    def parse_requirements(self, description: str) -> Dict[str, Any]:
        ...
        # Detect features
        if any(word in description_lower for word in ['develop', 'programming', 'coding']):
    ...
    def generate_config(self, 
        ...
        for feature in requirements['features']:
            if feature == 'development':
                ...
                if 'nginx' in requirements['services']:
                    ports.extend(['80', '443'])
                extra.append(self.templates['server'].replace('{ports}', ' '.join(ports)))
        ...
        for service_name in requirements['services']:
            if service_name in self.services_db:
                ...
                service_config = self.templates['service'].format(
                    ...
                    service_settings=service_info.get('settings', '    # Additional settings here')
                )
        ...
        if not users:
            users.append(self.templates['user'].format(
                username='user',
    ...
    def generate_home_manager_config(self, 
                                    username: str,
                                    description: str) -> str:
        """
  ...
  home.sessionVariables = {
    EDITOR = "{editor}";
        ...
        if 'neovim' in description_lower or 'nvim' in description_lower:
            editor = 'nvim'
            programs.append('''  programs.neovim = {
    enable = true;
    viAlias = true;
    vimAlias = true;
  };''')
            ...
            editor = 'emacs'
            programs.append('''  programs.emacs = {
    enable = true;
  };''')
    ...
    userEmail = "you@example.com";
        ...
        if 'alacritty' in description_lower:
            programs.append('''  programs.alacritty = {
    enable = true;
  };''')
        elif 'kitty' in description_lower:
            programs.append('''  programs.kitty = {
    enable = true;
  };''')
        
        # Format
        packages_str = '\n'.join(f'    {pkg}' for pkg in packages) if packages else '    # Add packages here'
...
{build_inputs}
        ...
        if 'python' in description_lower:
            packages.extend(['python3', 'python3Packages.pip', 'python3Packages.virtualenv'])
        ...
        build_inputs_str = '\n'.join(f'            {pkg}' for pkg in build_inputs) if build_inputs else '            # Build dependencies'
