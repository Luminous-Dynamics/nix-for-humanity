...
class EnhancedOutput:
    ...
    def show_packages(
        self, packages: List[Dict[str, Any]], title: Optional[str] = None
    ):
        ...
        table.add_column(
            "Description", style=self.colors.MUTED, overflow="ellipsis", max_width=50
        )
        ...
        for pkg in packages[:20]:
            ...
            desc = str(pkg.get("description", ""))[:80]  # Truncate long descriptions
        ...
        if len(packages) > 20:
            self.info(
                f"Showing first 20 of {len(packages)} packages. Refine your search for better results."
            )
...
"""
    ...
    def show_system_info(self, info: Dict[str, Any]):
        ...
        self.console.print(
            Panel(
                text, title="🖥️  System Information", border_style=self.colors.PRIMARY
            )
        )
    ...
    def section_header(self, title: str):
        ...
        self.console.rule(
            f"[{self.colors.PRIMARY}]{title}[/]", style=self.colors.PRIMARY
        )
