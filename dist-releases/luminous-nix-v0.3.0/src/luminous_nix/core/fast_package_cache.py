...
class FastPackageCache:
    ...
    COMMON_PACKAGES = {
        ...
        "google-chrome": {"description": "Google Chrome browser", "category": "browser"},
        
        # Editors
        "vim": {"description": "Vi IMproved text editor", "category": "editor"},
        "neovim": {"description": "Vim-fork focused on extensibility", "category": "editor"},
        ...
        "python3": {"description": "Python programming language", "category": "development"},
        ...
        # Terminal
        "alacritty": {"description": "GPU-accelerated terminal", "category": "terminal"},
    ...
    def search(self, query: str, limit: int = 30) -> Tuple[List[Dict], float, bool]:
        ...
        if cache_key in self.cache.get("searches", {}):
            ...
            logger.debug(f"Cache hit for '{query}': {len(results)} results in {elapsed_ms:.1f}ms")
            self._save_stats()
    ...
    def get_stats(self) -> Dict:
        ...
        stats["cache_size_kb"] = self.cache_file.stat().st_size / 1024 if self.cache_file.exists() else 0
    ...
    def warm_cache(self, common_queries: List[str] = None):
        ...
        queries = common_queries or [
            "browser", "editor", "terminal", "development",
            "python", "javascript", "rust", "git", "docker"
        ]
...
def benchmark_fast_cache():
    ...
    for query in queries:
        ...
        print(f"  First:  {time1:>6.1f}ms - {len(results1)} results (cached: {cached1})")
        
        # Second search (should be cached)
        results2, time2, cached2 = cache.search(query)
        print(f"  Second: {time2:>6.1f}ms - {len(results2)} results (cached: {cached2})")
