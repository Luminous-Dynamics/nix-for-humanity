...
class IntentRecognizer:
    ...
    def _load_patterns(self):
        ...
        self.install_patterns = [
            # Note: "add" is handled separately to avoid conflicts with user management
            r"\b(install|get|need|want|set up)\s+(\S+)",
            r"\badd\s+(?!.*\s+to\s+)(\S+)",  # "add package" but not "add X to Y"
            r"\b(can you|please|could you)\s+(install|get)\s+(\S+)",  # removed "add" from here
        ...
        self.add_user_to_group_patterns = [
            r"\badd\s+(\S+)\s+to\s+(?:the\s+)?(\S+)(?:\s+group)?",  # "add alice to wheel" or "add alice to wheel group"
        ...
        self.change_password_patterns = [
            r"\b(change|reset|set)\s+(?:my\s+)?password$",  # "change password" without user
        ...
        self.grant_sudo_patterns = [
            r"\b(grant|give)\s+sudo(?:\s+access)?\s+to\s+(\S+)",  # "grant sudo to alice"
        ...
        self.analyze_disk_patterns = [
            r"\b(analyze|analyse|check|inspect)\s+(disk|storage)\s+(space|usage)",
        ...
        self.service_status_patterns = [
            r"\b(\S+)\s+status$",  # Simple: "nginx status"
        ...
        self.validate_config_patterns = [
            r"\bvalidate\s+config(?:uration)?",
            r"\bcheck\s+config(?:uration)?(\s+file)?",
            r"\bverify\s+config(?:uration)?",
            r"\btest\s+config(?:uration)?",
            r"\bconfig(?:uration)?\s+valid(?:ation)?",
            r"\bis\s+(?:the\s+)?config(?:uration)?\s+valid",
        ]

        # Package discovery patterns
        self.discover_package_patterns = [
            r"\bdiscover\s+(.+?)(?:\s+package|\s+editor|\s+tool|\s+app)",
            r"\bfind\s+(?:me\s+)?(?:a\s+)?(.+?)\s+(?:tool|app|application|editor|browser)",
            r"\bsuggest\s+(?:a\s+)?(.+?)(?:\s+package|\s+tool|\s+app)",
            r"\brecommend\s+(?:a\s+)?(.+?)(?:\s+for\s+me)?",
            r"\bwhat\s+(?:package\s+)?(?:should\s+i\s+use\s+)?for\s+(.+)",
        ]

        # Find by command patterns
        self.find_by_command_patterns = [
            r"\bwhat\s+package\s+(?:has|contains|provides)\s+(\S+)",
            r"\bwhich\s+package\s+(?:has|contains|provides)\s+(\S+)",
            r"\bpackage\s+(?:that\s+)?(?:has|contains|provides)\s+(\S+)",
            r"\bfind\s+package\s+with\s+(\S+)(?:\s+command)?",
            r"\b(\S+)\s+is\s+in\s+what\s+package",
        ]

        # Browse categories patterns
        self.browse_categories_patterns = [
            r"\bbrowse\s+(?:package\s+)?categories",
            r"\bshow\s+(?:me\s+)?(?:package\s+)?categories",
            r"\blist\s+(?:package\s+)?categories",
            r"\bwhat\s+(?:package\s+)?categories\s+(?:are\s+)?(?:available|there)",
            r"\bcategories\s+(?:of\s+packages)?",
        ]

        # Show popular patterns
        self.show_popular_patterns = [
            r"\bshow\s+(?:me\s+)?popular(?:\s+packages)?",
            r"\bpopular\s+packages?",
            r"\btop\s+packages?",
            r"\bmost\s+(?:used|installed|popular)\s+packages?",
            r"\bwhat\s+(?:are\s+)?(?:the\s+)?(?:most\s+)?popular\s+packages?",
            r"\btrending\s+packages?",
        ]

        # Common package aliases - including compound terms!
        self.package_aliases = {
            # Compound terms that should map to specific packages
            "text editor": "vim",
            "web browser": "firefox",
            "browser": "firefox",
            "editor": "vim",
    ...
    def _match_patterns(self, text: str) -> Intent | None:
        ...
        for pattern in self.help_patterns:
            if re.search(pattern, text, re.IGNORECASE):
                return Intent(
                    type=IntentType.HELP, entities={}, confidence=0.95, raw_text=text
                )
        ...
        for pattern in self.show_ip_patterns:
            if re.search(pattern, text, re.IGNORECASE):
                return Intent(
                    type=IntentType.SHOW_IP, entities={}, confidence=0.9, raw_text=text
                )
        ...
        for pattern in self.validate_config_patterns:
            if re.search(pattern, text, re.IGNORECASE):
                return Intent(
                    type=IntentType.VALIDATE_CONFIG,
                    entities={},
                    confidence=0.85,
                    raw_text=text,
                )

        # Check package discovery patterns
        for pattern in self.discover_package_patterns:
            if match := re.search(pattern, text, re.IGNORECASE):
                # Extract what to discover
                search_term = None
                for group in match.groups():
                    if group and group not in [
                        "discover",
        ...
        # Check find by command patterns
        for pattern in self.find_by_command_patterns:
            if match := re.search(pattern, text, re.IGNORECASE):
        ...
        # Check browse categories patterns
        for pattern in self.browse_categories_patterns:
            if re.search(pattern, text, re.IGNORECASE):
                return Intent(
        ...
        # Check show popular patterns
        for pattern in self.show_popular_patterns:
            if re.search(pattern, text, re.IGNORECASE):
                return Intent(
        ...
        for pattern in self.change_password_patterns:
            if match := re.search(pattern, text, re.IGNORECASE):
                ...
                # Return intent even without username (might be changing current user's password)
                entities = {"username": username} if username else {}
                return Intent(
                    type=IntentType.CHANGE_PASSWORD,
                    entities=entities,
                    confidence=0.9 if username else 0.85,
                    raw_text=text,
                )
        ...
        # Check more specific patterns first (analyze before general disk usage)
        ...
        for pattern in self.install_patterns:
            if match := re.search(pattern, text, re.IGNORECASE):
                ...
                if not package:
                    groups = match.groups()
                    # Try to find the package name in the groups
                ...
                if not package:
                    # If we couldn't extract a package name, skip this match
                    continue

                # Resolve aliases (including compound terms)
                package_lower = package.lower() if package else ""
                package = self.package_aliases.get(package_lower, package)
        ...
        for pattern in self.configure_patterns:
            if match := re.search(pattern, text, re.IGNORECASE):
                if len(match.groups()) >= 2:
                    service = (
                        match.group(2)
                        if match.group(1) in ["configure", "config", "set up", "enable"]
                        else match.group(3)
                    )
                else:
        ...
        # Disk/storage patterns are already checked earlier in the method
        # This is a duplicate section - removing to avoid confusion
    ...
    async def _semantic_match(self, text: str) -> Intent | None:
