...
class AsciinemaRecorder:
    ...
    async def start_recording(self, output_path: Path | None = None) -> Path:
        ...
        cmd = [
            ...
            self.config.title,
            "--idle-time-limit",
            str(self.config.idle_time_limit),
    ...
    async def upload_recording(self, cast_path: Path) -> str:
        ...
        if "https://asciinema.org" in output:
            url = [
                line for line in output.split("\n") if "https://asciinema.org" in line
            ][0]
            return url.strip()
        return ""
...
class CharmFreeze:
    ...
    async def freeze_code(
        ...
        cmd = ["freeze", str(code_path), "--output", str(output), "--theme", theme]
    ...
    async def freeze_text(
        text: str, output_path: Path | None = None, language: str = "python"
    ) -> Path:
...
"""
        ...
        for cmd in commands:
            if cmd.startswith("Type") or cmd.startswith("Sleep"):
                tape_content += f"{cmd}\n"
...
class DemoOrchestrator:
    ...
    async def _run_installation_demo(self):
        ...
        package = await self.gum.input("Enter package name:", placeholder="firefox")

        # Show processing
        output, _ = await self.gum.spin(
            ["./bin/ask-nix", f"install {package}"], f"Installing {package}..."
        )
