...
class IntentRouter:
    ...
    def __init__(self):
        ...
        self.nixos_patterns = {
            'dependency': [
                'conflict', 'collision', 'dependency', 'package conflict',
                'version mismatch', 'incompatible', 'override'
            ],
            'configuration': [
                'configuration.nix', 'setup', 'configure', 'enable service',
                'system config', 'nixos config', 'services.', 'environment.'
            ],
            'error': [
                'error:', 'failed', 'attribute missing', 'undefined',
                'not found', 'broken', 'issue', 'problem'
            ],
            'optimization': [
                'optimize', 'performance', 'faster', 'improve',
                'reduce size', 'cleanup', 'garbage collect'
            ]
        }
        
        # General patterns better suited for Ollama
        self.general_patterns = [
            'what is', 'how does', 'explain', 'why', 'when should',
            'best practice', 'difference between', 'tutorial',
            'guide', 'documentation', 'learn'
        ]
...
class AIOrchestrator:
    ...
    def process(self, 
                ...
                timeout: float = 5.0) -> OrchestrationResult:
        """
    ...
    def _process_with_hrm(self, query: str, timeout: float) -> OrchestrationResult:
        ...
        if hrm_result.confidence < confidence_threshold and self.ollama:
            logger.debug(f"HRM confidence low ({hrm_result.confidence:.2f}), checking with Ollama")
    ...
    def _process_with_ollama(self, 
                            query: str, 
                            timeout: float,
                            hrm_fallback: Optional[ReasoningResult] = None) -> OrchestrationResult:
    ...
    def _extract_constraints(self, query: str) -> List[str]:
        ...
        if any(word in query_lower for word in ['compatible', 'work with', 'alongside']):
...
def ask(query: str, 
        ...
        verbose: bool = False) -> Union[str, Dict[str, Any]]:
    """
