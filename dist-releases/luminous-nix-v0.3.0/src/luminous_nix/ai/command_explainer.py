...
class NixCommandExplainer:
    ...
    def _get_command_description(self, cmd: str) -> str:
        ...
        try:
            result = subprocess.run(
                ["whatis", cmd], capture_output=True, text=True, timeout=1
            )
    ...
    def _explain_nix_command(
        self, command: str, components: List[Dict]
    ) -> CommandExplanation:
        ...
        if base_cmd == "nix-env":
            if any(
                c["value"] in ["-i", "-iA"] for c in components if c["type"] == "option"
            ):
                packages = [
                    c["value"]
                    for c in components
                    if c["type"] in ["package", "argument"]
                ]
                effects.append(
                    f"Install packages: {', '.join(packages) if packages else 'specified packages'}"
                )
                effects.append("Packages will be added to your user profile")
                effects.append("Creates new generation that can be rolled back")
                alternatives.append(
                    f"nix profile install nixpkgs#{packages[0] if packages else 'package'}"
                )
                ...
                c["value"] in ["-q", "-qa"] for c in components if c["type"] == "option"
            ):
            ...
            action = next(
                (
                    c["value"]
                    for c in components
                    if c["value"] in ["switch", "boot", "test", "build"]
                ),
                None,
            )
            if action == "switch":
        ...
        if risk == "high":
            warnings.append(
                "⚠️ HIGH RISK: This command makes significant system changes"
            )
        elif risk == "medium":
    ...
    def _explain_generic_nix_command(
        self, command: str, components: List[Dict]
    ) -> CommandExplanation:
    ...
    def _explain_general_command(
        ...
        if base_cmd == "sudo":
            ...
            if any(
                c["value"] in ["-r", "-rf"] for c in components if c["type"] == "option"
            ):
