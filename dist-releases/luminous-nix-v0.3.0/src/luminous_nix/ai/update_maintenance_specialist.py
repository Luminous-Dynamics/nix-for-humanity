...
class UpdateMaintenanceSpecialist:
        ...
        for pattern, result in self.update_patterns.items():
            if re.search(pattern, normalized):
                return {
                    'command': result['command'],
                    'description': result['description'],
                    'confidence': result['confidence'] * 0.9,  # Slightly lower for fuzzy
                    'note': result.get('note', ''),
                    'type': 'maintenance_command'
                }
    ...
    def _is_update_query(self, query: str) -> bool:
        ...
        update_keywords = [
            ...
            'clean', 'garbage', 'rollback', 'revert',
            'generation', 'maintain', 'maintenance'
        ]
    ...
    def get_training_examples(self) -> List[Tuple[str, str]]:
        ...
        for template in templates:
            ...
            examples.append((f"please {template}", self.handle_query(template)['command']))
            examples.append((f"how to {template}", self.handle_query(template)['command']))
            examples.append((f"I want to {template}", self.handle_query(template)['command']))
    ...
    def validate_command(self, command: str) -> Dict:
        ...
        if 'collect-garbage -d' in command:
            ...
            validation['warnings'].append('This will permanently delete old generations')
        ...
        if '--upgrade' in command:
            validation['warnings'].append('This will update all packages to latest versions')
