...
class NixOSStateAnalyzer:
    ...
    def list_generations(self, limit: int = 50) -> List[SystemGeneration]:
        ...
        try:
            ...
            for line in output.split('\n'):
                ...
                match = re.match(r'(\d+)\s+(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2}:\d{2})\s*(\(current\))?', line)
                if match:
        ...
        try:
            ...
            for pkg in pkgs1_set & pkgs2_set:
                if pkgs1[pkg] != pkgs2[pkg]:
                    diff['packages_updated'].append(f"{pkg}: {pkgs1[pkg]} → {pkgs2[pkg]}")
    ...
    def _get_current_generation(self) -> int:
        ...
        try:
            output = self._run_command("sudo nix-env --list-generations -p /nix/var/nix/profiles/system | grep current")
            if output:
    ...
    def _get_running_services(self) -> List[str]:
        ...
        services = []
        try:
            output = self._run_command("systemctl list-units --type=service --state=running --no-legend")
            if output:
                for line in output.split('\n'):
                    if line.strip():
