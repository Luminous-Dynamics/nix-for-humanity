...
class SecurityAuditor:
    ...
    def audit_system(self, deep_scan: bool = False) -> SecurityAuditResult:
        ...
        try:
            ...
            vulnerabilities = self._scan_for_vulnerabilities(state.installed_packages, deep_scan)
            ...
            immediate_actions = self._generate_immediate_actions(vulnerabilities, config_results)
    ...
    def check_package_security(self, package_name: str) -> List[CVEInfo]:
        ...
        try:
            ...
            if package_name in self.vulnerable_patterns:
                for vuln_version, cves in self.vulnerable_patterns[package_name].items():
                    if self._version_vulnerable(version, vuln_version):
                        for cve_id in cves:
                            vulnerabilities.append(self._create_cve_info(
                                cve_id, package_name, version
                            ))
    ...
    def _scan_for_vulnerabilities(self, packages: List[str], deep: bool) -> List[CVEInfo]:
        ...
        # Sort by severity
        return sorted(unique_vulns, key=lambda v: self._severity_weight(v.severity), reverse=True)
    ...
    def _check_ssh_security(self) -> bool:
        ...
        try:
            ...
            if config_path.exists():
                ...
                return ("PasswordAuthentication no" in config and 
                       "PermitRootLogin no" in config)
        except:
            pass
        return False
    ...
    def _generate_immediate_actions(self, vulns: List[CVEInfo], config: Dict) -> List[str]:
        ...
        if not config.get('ssh') and config.get('ssh_installed'):
            actions.append("Harden SSH: services.openssh.passwordAuthentication = false")
    ...
    def _calculate_security_score(self, vulns: List[CVEInfo], config: Dict) -> Tuple[float, str]:
    ...
    def _parse_package_string(self, package: str) -> Tuple[Optional[str], Optional[str]]:
