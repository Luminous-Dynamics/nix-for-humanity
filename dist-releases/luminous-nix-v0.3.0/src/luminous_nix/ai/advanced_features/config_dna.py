...
class ConfigDNAAnalyzer:
        ...
        self.gene_catalog = {
            "security": [
                ConfigGene(
                    "firewall",
                    "security",
                    r"networking\.firewall\.enable\s*=\s*true",
                    "Firewall enabled",
                    "high",
                    0.7,
                ),
                ConfigGene(
                    "sudo",
                    "security",
                    r"security\.sudo\.enable\s*=\s*true",
                    "Sudo configured",
                    "high",
                    0.9,
                ),
                ConfigGene(
                    "sshd",
                    "security",
                    r"services\.openssh\.enable\s*=\s*true",
                    "SSH server enabled",
                    "medium",
                    0.5,
                ),
                ConfigGene(
                    "fail2ban",
                    "security",
                    r"services\.fail2ban\.enable\s*=\s*true",
                    "Fail2ban protection",
                    "high",
                    0.3,
                ),
            ...
            "performance": [
                ...
                ConfigGene(
                    "kernel_latest",
                    "performance",
                    r"boot\.kernelPackages\s*=.*latest",
                    "Latest kernel",
                    "low",
                    0.2,
                ),
                ConfigGene(
                    "cpu_gov",
                    "performance",
                    r"powerManagement\.cpuFreqGovernor",
            ...
            "desktop": [
                ConfigGene(
                    "xorg",
                    "desktop",
                    r"services\.xserver\.enable\s*=\s*true",
                    "X11 enabled",
                    "high",
                    0.5,
                ),
                ...
                ConfigGene(
                    "plasma",
                    "desktop",
                    r"services\.xserver\.desktopManager\.plasma",
                    "KDE Plasma desktop",
                    "high",
                    0.2,
                ),
                ConfigGene(
                    "gnome",
                    "desktop",
                    r"services\.xserver\.desktopManager\.gnome",
                    "GNOME desktop",
                    "high",
                    0.2,
                ),
            ...
            "development": [
                ConfigGene(
                    "docker",
                    "development",
                    r"virtualisation\.docker\.enable\s*=\s*true",
                    "Docker enabled",
                    "medium",
                    0.4,
                ),
                ...
                ConfigGene(
                    "direnv",
                    "development",
                    r"programs\.direnv\.enable\s*=\s*true",
                ...
                ConfigGene(
                    "lorri",
                    "development",
                    r"services\.lorri\.enable\s*=\s*true",
                    "Lorri for Nix shells",
                    "medium",
                    0.1,
                ),
            ],
            "server": [
                ConfigGene(
                    "nginx",
                    "server",
                    r"services\.nginx\.enable\s*=\s*true",
                    "Nginx web server",
                    "high",
                    0.2,
                ),
                ConfigGene(
                    "postgresql",
                    "server",
                    r"services\.postgresql\.enable\s*=\s*true",
                    "PostgreSQL database",
                    "high",
                    0.1,
                ),
                ConfigGene(
                    "prometheus",
                    "server",
                    r"services\.prometheus\.enable\s*=\s*true",
                ...
                ConfigGene(
                    "containers",
                    "server",
                    r"virtualisation\.oci-containers",
    ...
    def analyze_dna(
        self, config_path: str = "/etc/nixos/configuration.nix"
    ) -> ConfigDNA:
        """
    ...
    def compare_dna(self, config1: str, config2: str) -> Dict[str, Any]:
        ...
        try:
            ...
            similarity = (
                len(common) / max(len(genes1), len(genes2)) if genes1 or genes2 else 0
            )
            ...
            return {
                ...
                "evolution_diff": self._compare_evolution(
                    dna1.evolution_stage, dna2.evolution_stage
                ),
                    ...
                    for i, commit in enumerate(commits[:10]):  # Last 10 commits
                        lineage.append(
                            ConfigLineage(
                                ...
                                parent_hash=commits[i + 1].split()[0]
                                if i + 1 < len(commits)
                                else None,
                                mutations=[
                                    commit.split(" ", 1)[1]
                                ...
                                fitness_score=0.5
                                + (i * 0.05),  # Assume improving over time
                            )
                        )
    ...
    def _calculate_complexity(self, content: str, genes: List[ConfigGene]) -> float:
        ...
        complexity = (
            min(factors["lines"] / 10, 30)
            + factors["genes"] * 3  # Lines contribute max 30
            + factors["imports"] * 5  # Each gene adds 3
            + factors["overlays"] * 10  # Imports add complexity
            + factors["modules"] * 8  # Overlays are complex
            + factors["functions"]  # Modules add complexity
            * 2  # Functions add some complexity
        )
    ...
    def _find_similar_configs(
        self, fingerprint: str, genes: List[ConfigGene]
    ) -> List[str]:
    ...
    def _detect_mutations(
        self, genes: List[ConfigGene], inherited: List[str]
    ) -> List[str]:
    ...
    def _calculate_stability(
        self, lineage: ConfigLineage, mutation_rate: float
    ) -> float:
    ...
    def _suggest_beneficial_mutations(
        self, genes: List[ConfigGene], profile: str
    ) -> List[str]:
    ...
    def _detect_harmful_patterns(
        self, content: str, genes: List[ConfigGene]
    ) -> List[str]:
