...
class EnsembleModel:
    ...
    def predict(self, query: str) -> Dict:
        ...
        ensemble_confidence = min(0.95, category_scores[best_category] / sum(self.weights))
...
class AttentionVisualizer:
    ...
    def get_attention_weights(self, query: str) -> Dict:
        ...
        important_keywords = ['install', 'update', 'config', 'enable', 'search', 'python', 'rust']
    ...
    def explain_prediction(self, query: str, prediction: Dict) -> str:
        ...
        explanation += ", ".join([f"'{token}' ({weight:.1%})" for token, weight in top_tokens])
...
def test_transformer_model():
    ...
    for query in test_queries:
        ...
        print(f"  Category: {result['category']:<10} Confidence: {result['confidence']:.1%}")
...
def test_ensemble():
    ...
    for item in test_queries:
        ...
        print(f"{correct} {item['query']:<30} → {result['category']:<10} ({result['confidence']:.1%})")
