...
class PackageRecommender:
    ...
    def __init__(self):
        ...
        self.package_graph = {
            ...
            "neovim": {
                ...
                "complementary": ["tmux", "fzf", "ripgrep", "tree-sitter", "language-servers"],
    ...
    def recommend(self, package: str, context: Optional[Dict] = None) -> RecommendationSet:
        """
    ...
    def _recommend_known_package(self, package: str, context: Optional[Dict]) -> RecommendationSet:
        ...
        for alt in pkg_data.get("alternatives", []):
            if alt in self.package_graph:
                ...
                alternatives.append(PackageRecommendation(
                    name=alt,
            ...
            for pkg, data in self.package_graph.items():
                if pkg != package and data.get("category") == my_category:
                    if pkg not in pkg_data.get("alternatives", []):
                        similar.append(PackageRecommendation(
                            name=pkg,
        ...
        for comp in pkg_data.get("complementary", []):
            complementary.append(PackageRecommendation(
                name=comp,
        ...
        for upgrade in pkg_data.get("upgrade", []):
            upgrade_path.append(PackageRecommendation(
                name=upgrade,
        ...
        explanation = self._generate_explanation(package, alternatives, similar, complementary)
        
        return RecommendationSet(
            alternatives=sorted(alternatives, key=lambda x: x.similarity_score, reverse=True)[:5],
    ...
    def _recommend_unknown_package(self, package: str, context: Optional[Dict]) -> RecommendationSet:
    ...
    def _find_similar_names(self, package: str) -> List[str]:
        ...
        for known_pkg in self.package_graph:
            ...
            elif len(package) < 10 and self._edit_distance(package_lower, known_pkg) <= 2:
                similar.append(known_pkg)
    ...
    def _recommend_by_category(self, category: str, original_query: str) -> RecommendationSet:
        """Recommend packages from a specific category"""
        packages = []
        for pkg, data in self.package_graph.items():
            if data.get("category") == category:
                packages.append(PackageRecommendation(
                    name=pkg,
                    description=self._get_description(pkg),
    ...
    def _generate_explanation(self, package: str, alternatives, similar, complementary) -> str:
    ...
    def format_recommendations(self, recommendations: RecommendationSet) -> str:
        ...
        if recommendations.alternatives:
            ...
            for rec in recommendations.alternatives[:3]:
                output.append(f"  • {rec.name} ({rec.similarity_score:.0%} similar) - {rec.reason}")
                output.append(f"    {rec.description}")
