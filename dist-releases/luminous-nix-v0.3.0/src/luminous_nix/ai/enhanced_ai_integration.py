...
class EnhancedAI:
    ...
    def ask(self, question: str, 
            ...
            auto_personality: bool = True) -> str:
        """
        ...
        if result.get("answer"):
            ...
            if result.get("validation") and not result["validation"]["valid"]:
                self.console.print("[yellow]⚠ Response may contain inaccuracies[/yellow]")
...
"""
    ...
    def rebuild_corpus(self):
        ...
        self.console.print(f"[green]✅ Corpus rebuilt: {result['documents']} documents[/green]")
        
        # Reload in refiner
        self.smart_client.refiner.documents = self.smart_client.refiner._load_corpus_documents()
...
def integrate_with_cli():
    ...
    @click.option('--personality', '-p', 
                  ...
                  default='friend', help='AI personality mode')
    @click.option('--feedback', '-f', is_flag=True, 
                  help='Provide feedback to improve responses')
    ...
    @click.argument('mode', type=click.Choice(['teacher', 'expert', 'friend', 'grandma']))
