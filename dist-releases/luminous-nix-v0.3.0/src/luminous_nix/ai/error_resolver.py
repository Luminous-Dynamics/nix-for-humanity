...
class NixOSErrorResolver:
    ...
    def _handle_attribute_missing(self, error: str, match: re.Match) -> ErrorSolution:
        ...
        if corrected:
            solutions.append(
                f"The package name might be '{corrected}' instead of '{package_name}'"
            )
    ...
    def _analyze_generic_error(self, error: str) -> ErrorSolution:
        ...
        if "/etc/nixos/" in error:
            ...
            commands.append(
                "sudo nano /etc/nixos/configuration.nix  # Edit configuration"
            )
    ...
    def explain_error(self, error_solution: ErrorSolution) -> str:
        ...
        output = []
        output.append(
            f"🔍 **Error Type**: {error_solution.error_type.replace('_', ' ').title()}"
        )
