...
from ..core.nix_ast_parser import ASTNode, NixASTParser, get_parser
...
class NixKnowledgeGraph:
    ...
    def __init__(self, parser: NixASTParser | None = None):
        """
    ...
    def build_from_file(self, file_path: str | Path) -> bool:
        """
    ...
    def build_from_directory(self, dir_path: str | Path) -> bool:
        """
    ...
    def _process_packages(
        self, binding: ASTNode, parent_node: GraphNode, source_file: str
    ):
    ...
    def _process_service(
        self, binding: ASTNode, parent_node: GraphNode, source_file: str, attr_path: str
    ):
        ...
        if len(parts) >= 2:
            ...
            edge = GraphEdge(
                source_id=parent_node.id, target_id=service_id, type=EdgeType.CONFIGURES
            )
            self._add_edge(edge)
    ...
    def _process_service_options(
        self, binding: ASTNode, service_node: GraphNode, source_file: str
    ):
    ...
    def _process_user(
        ...
        if len(parts) >= 3:
            ...
            edge = GraphEdge(
                source_id=parent_node.id, target_id=user_id, type=EdgeType.HAS
            )
            self._add_edge(edge)
    ...
    def _process_network(
        ...
        if network_id not in self.nodes:
            ...
            edge = GraphEdge(
                source_id=parent_node.id, target_id=network_id, type=EdgeType.HAS
            )
            self._add_edge(edge)

    def _process_imports_list(
        self, binding: ASTNode, parent_node: GraphNode, source_file: str
    ):
    ...
    def _process_function(self, ast: ASTNode, parent_node: GraphNode, source_file: str):
        ...
        edge = GraphEdge(
            source_id=parent_node.id, target_id=func_id, type=EdgeType.CONTAINS
        )
        self._add_edge(edge)
    ...
    def _process_import(self, ast: ASTNode, parent_node: GraphNode, source_file: str):
        ...
        for child in ast.children:
            if child.type in ["path", "string"]:
                ...
                edge = GraphEdge(
                    source_id=parent_node.id, target_id=import_id, type=EdgeType.IMPORTS
                )
                self._add_edge(edge)

    # === Utility Methods ===

    def _get_attr_path(self, binding: ASTNode) -> str | None:
    ...
    def _get_binding_value(self, binding: ASTNode) -> str | None:
    ...
    def get_dependencies(self, node_id: str) -> list[GraphNode]:
        ...
        dependencies = []
        for edge in self.edges:
            if edge.source_id == node_id and edge.type in [
                EdgeType.DEPENDS_ON,
                EdgeType.REQUIRES,
                EdgeType.IMPORTS,
            ]:
    ...
    def get_dependents(self, node_id: str) -> list[GraphNode]:
        ...
        dependents = []
        for edge in self.edges:
            if edge.target_id == node_id and edge.type in [
                EdgeType.DEPENDS_ON,
                EdgeType.REQUIRES,
            ]:
    ...
    def get_subgraph(
    ...
    ) -> tuple[set[GraphNode], list[GraphEdge]]:
        ...
        # Get nodes
        subgraph_nodes = {
            self.nodes[nid] for nid in nodes_to_include if nid in self.nodes
        }
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "average_degree": len(self.edges) * 2 / len(self.nodes)
            if self.nodes
            else 0,
