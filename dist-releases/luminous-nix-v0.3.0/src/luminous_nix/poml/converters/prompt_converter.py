...
class PromptToPomlConverter:
    ...
    def convert_to_poml(self, prompt_location: PromptLocation) -> ET.Element:
        ...
        ET.SubElement(metadata, 'description').text = self._generate_description(prompt_location)
    ...
    def _add_model_hints(self, model_hints: ET.Element, prompt_location: PromptLocation):
        ...
        if 'classify' in prompt_text or 'json' in prompt_text:
            ...
            ET.SubElement(model_hints, 'preferred-models').text = 'gpt-3.5-turbo, mistral-7b'
            ...
            # General purpose
            ET.SubElement(model_hints, 'preferred-models').text = 'gpt-3.5-turbo, mistral-7b, llama-2'
    ...
    def _add_system_prompt(self, prompt: ET.Element, prompt_location: PromptLocation):
        ...
        if 'You are' in prompt_location.prompt_text:
            ...
            if 'intent' in prompt_location.file_path.stem:
                system.text = 'You are an intent classification expert for NixOS operations.'
            else:
                system.text = 'You are a helpful NixOS assistant.'
    
    def _add_structured_content(self, prompt: ET.Element, prompt_location: PromptLocation):
    ...
    def convert_file(self, file_path: Path) -> List[Path]:
        ...
        for i, prompt_loc in enumerate(prompts):
            ...
            print(f"✅ Converted prompt at {file_path.name}:{prompt_loc.line_number} -> {saved_path.name}")
    ...
    def generate_report(self) -> str:
        ...
        for conv in self.conversions:
            report.append(f"- {conv['original_file']}:{conv['line_number']} -> {conv['poml_file']}")
            if conv['variables']:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Convert string prompts to POML format')
    parser.add_argument('source', type=Path, help='Source file or directory')
    parser.add_argument('--output', type=Path, default=Path('converted_prompts'), help='Output directory')
    parser.add_argument('--report', action='store_true', help='Generate conversion report')
