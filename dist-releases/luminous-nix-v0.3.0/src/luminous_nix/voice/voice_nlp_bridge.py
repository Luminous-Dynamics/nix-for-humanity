...
# ARCHIVED: from ..consciousness.adaptive_persona import AdaptivePersona
...
class VoiceNLPBridge:
    ...
    def _preprocess_voice_input(self, text: str) -> str:
        ...
        if not any(
            processed.startswith(cmd)
            for cmd in ["install", "remove", "search", "list", "show"]
        ):
    ...
    def _voice_to_intent(self, text: str) -> Intent:
        ...
        return Intent(type=intent_type, query=query, metadata=metadata)
    ...
    def _detect_voice_intent_type(self, text: str) -> IntentType:
        ...
        # Search patterns
        if any(
            word in text_lower for word in ["search", "find", "look for", "what is"]
        ):
    ...
    def _handle_voice_control(self, text: str) -> str:
        ...
        if "louder" in text_lower:
            self.voice.config.voice_volume = min(
                1.0, self.voice.config.voice_volume + 0.1
            )
            ...
            self.voice.config.voice_volume = max(
                0.1, self.voice.config.voice_volume - 0.1
            )
            self.voice.speaker.setProperty("volume", self.voice.config.voice_volume)
