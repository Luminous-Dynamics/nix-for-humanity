...
class PackageIndexBuilder:
            ...
            if result.returncode == 0 and result.stdout:
                ...
                for path, info in raw_packages.items():
                    ...
                    self.packages[name] = EnhancedPackageInfo(
                        ...
                        platforms=[
                            "x86_64-linux"
                        ],  # Default, would need to check properly
                        size_mb=None,
                        homepage=None,
                        license=None,
                        maintainers=[],
    ...
    def save_to_database(self):
        ...
        # Create indices
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_popularity ON packages(popularity_score DESC)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_categories ON packages(categories)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_description ON packages(description)"
        )
