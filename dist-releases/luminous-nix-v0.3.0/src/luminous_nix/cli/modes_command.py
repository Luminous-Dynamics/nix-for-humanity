...
from ..ui.mode_animator import ModeAnimator, AnimationType
...
@click.option('--animation-type', type=click.Choice([a.value for a in AnimationType]), 
              default='fade', help='Type of transition animation')
def switch(mode: str, dry_run: bool, output_json: bool, animate: bool, animation_type: str):
    ...
    try:
        ...
        if output_json:
            ...
            click.echo()
            click.echo(f"From: {transition.from_mode.value} → To: {transition.to_mode.value}")
            click.echo()
            ...
            if transition.success:
                ...
                if not dry_run:
                    click.echo()
                    click.echo(click.style(f"✨ {mode.upper()} mode activated!", fg='green', bold=True))
            else:
...
def current(output_json: bool):
    ...
    try:
        ...
        if output_json:
            ...
            click.echo(click.style(f"🎭 Current Mode: {current_mode.value.upper()}", fg='cyan', bold=True))
            click.echo()
            click.echo(f"📝 {profile.description}")
            click.echo()
...
def wizard(quick: bool, export_config: bool):
    ...
    try:
        ...
        click.echo(click.style("✨ Your Perfect Mode Configuration", fg='green', bold=True))
        click.echo()
