...
from ..ai.advanced_features.predictive_health import (
    PredictiveHealthMonitor, HealthMetric, HealthStatus
)
...
def check(output_json: bool):
    ...
    try:
        ...
        if output_json:
            ...
            click.echo(click.style(f"{status_icon} System Health: {report.overall_status.value.upper()}", 
            ...
            if report.estimated_time_to_failure:
                ...
                click.echo(click.style(f"⏰ Estimated time to critical issue: {days} days", 
...
def predict(metric: str, days: int, output_json: bool):
    ...
    try:
        ...
        if output_json:
            ...
            click.echo(click.style(f"🔮 Prediction: {metric.upper()}", fg='cyan', bold=True))
            click.echo()
...
def risks(output_json: bool):
    ...
    try:
        ...
        if output_json:
            ...
            for factor, score in sorted(report.risk_factors.items(), 
...
def optimize(show_all: bool):
    ...
    try:
        ...
        click.echo(click.style("🚀 System Optimization Suggestions", fg='cyan', bold=True))
        click.echo()
    ...
    try:
        ...
        while True:
            try:
                ...
                click.echo(f"{status_icon} {report.overall_status.value.upper()} | Score: {report.health_score:.0f}/100")
