...
# ARCHIVED: from luminous_nix.consciousness.adaptive_persona import (
    DynamicPersona,
    EmotionalState,
    Interaction,
...
class VoiceEnabledCLI:
    ...
    def respond(
        self, text: str, user_id: str = "default", speak: bool | None = None
    ) -> str:
        """
        ...
        if should_speak:
            self.voice_system.speak_with_emotion(
                adapted_text, self.current_persona, play_audio=True
            )
    ...
    def _adapt_response_text(self, text: str, persona: DynamicPersona) -> str:
        ...
        if persona.current_mood == EmotionalState.FRUSTRATED:
            ...
            # Celebrate success
            if any(
                word in text.lower() for word in ["complete", "success", "installed"]
            ):
    ...
    def update_emotional_state(
        self, user_id: str, interaction_type: str, success: bool
    ):
        """
        ...
        if interaction_type == "error":
            ...
            self.persona_system# learn_from_interaction removed - no learning
# user_id, interaction)
...
# user_id, interaction)
    ...
    def speak_notification(
        self, message: str, urgency: str = "normal", user_id: str = "default"
    ):
        """
        ...
        # Speak notification
        self.voice_system.speak_with_emotion(
            message, self.current_persona, play_audio=True
        )
...
def integrate_voice_with_cli(cli_instance, enable_voice: bool = True):
    ...
    original_respond = (
        cli_instance.respond if hasattr(cli_instance, "respond") else None
    )
