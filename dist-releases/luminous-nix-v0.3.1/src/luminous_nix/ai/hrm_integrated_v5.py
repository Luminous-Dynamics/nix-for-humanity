...
class HRMIntegratedV5:
    ...
    def process_query(self, query: str) -> Dict:
        ...
        if self.service_specialist.can_handle(query):
            ...
            if (
                service_result
                and service_result["confidence"]
                >= self.confidence_thresholds["specialist"]
            ):
                ...
                return self._add_metadata(
                    result, time.time() - start_time, "service_specialist"
                )
        ...
        if self.home_manager_specialist.can_handle(query):
            ...
            if (
                hm_result
                and hm_result["confidence"] >= self.confidence_thresholds["specialist"]
            ):
                self.metrics["specialist_handled"] += 1
                result = self._format_specialist_result(hm_result, "home_manager")
                self._update_cache(query, result)
                return self._add_metadata(
                    result, time.time() - start_time, "home_manager_specialist"
                )
        ...
        if self.flake_specialist.can_handle(query):
            ...
            if (
                ...
                self._update_cache(query, result)
                return self._add_metadata(
                    result, time.time() - start_time, "flake_specialist"
                )
        ...
        if (
            dev_result
            and dev_result["confidence"] >= self.confidence_thresholds["specialist"]
        ):
            self.metrics["specialist_handled"] += 1
            result = self._format_specialist_result(dev_result, "dev")
            self._update_cache(query, result)
            return self._add_metadata(
                result, time.time() - start_time, "dev_specialist"
            )
        ...
        if (
            update_result
            and update_result["confidence"] >= self.confidence_thresholds["specialist"]
        ):
            self.metrics["specialist_handled"] += 1
            result = self._format_specialist_result(update_result, "update")
            self._update_cache(query, result)
            return self._add_metadata(
                result, time.time() - start_time, "update_specialist"
            )
        ...
        if (
            ...
            >= self.confidence_thresholds["transformer"]
        ):
    ...
    def _format_transformer_result(self, result: Dict) -> Dict:
        ...
        return {
            ...
            "command": self._infer_command(
                result.get("category", ""), result.get("query", "")
            ),
            "confidence": result.get("confidence", 0.0),
            "model": "transformer",
    ...
    def _infer_command(self, category: str, query: str) -> str:
        ...
        if category == "install":
            ...
            words = (
                query_lower.replace("search", "").replace("find", "").strip().split()
            )
            if words:
    ...
    def _estimate_accuracy(self) -> float:
        ...
        return (
            specialist_acc + transformer_acc + ensemble_acc + fallback_acc + cache_acc
        )
    ...
    def benchmark(self, test_queries: List[str]) -> Dict:
        ...
        for query in test_queries:
            ...
            print(
                f"{status} {query:<30} → {result['category']:<10} "
        ...
        return {"accuracy": accuracy, "metrics": metrics, "results": results}
