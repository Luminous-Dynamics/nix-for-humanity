...
class ConfigDNAVisualizer:
    ...
    def visualize_evolution_tree(self, lineage: List) -> str:
        ...
        for i, gen in enumerate(lineage[:10]):  # Show last 10 generations
            ...
            lines.append(
                f"{indent}  Fitness: [{fitness_bar:<10}] {gen.fitness_score:.0%}"
            )
            lines.append("")
    ...
    def visualize_health_chart(self, dna: ConfigDNA) -> str:
        ...
        # Overall health bar
        health_pct = (
            100 - len(dna.harmful_patterns) * 10 + len(dna.beneficial_mutations) * 5
        )
    ...
    def visualize_full_report(
        self, dna: ConfigDNA, lineage: Optional[List] = None
    ) -> DNAVisualization:
        ...
        return DNAVisualization(
            type=VisualizationType.HELIX, ascii_art="\n".join(sections), summary=summary
        )
