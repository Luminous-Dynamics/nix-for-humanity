...
class AdvancedFeaturesCLI:
    ...
    def rollback_analyze(self, symptoms: Optional[str]) -> int:
        ...
        if not self.json_output:
            print(f"{Colors.HEADER}🔄 Analyzing System for Safe Rollback...{Colors.ENDC}")
        ...
        if self.json_output:
            ...
            print(f"{Colors.BOLD}Recommended Rollback:{Colors.ENDC} Generation {analysis.recommended_generation}")
    ...
    def rollback_check(self, generation: int) -> int:
        ...
        if not self.json_output:
            print(f"{Colors.HEADER}🔍 Checking Generation {generation} Safety...{Colors.ENDC}")
        ...
        if self.json_output:
            ...
            print(f"{Colors.BOLD}Safe to Rollback:{Colors.ENDC} {'✅ Yes' if safe else '❌ No'}")
            print(f"{Colors.BOLD}Recommendation:{Colors.ENDC} {safety.get('recommendation', 'N/A')}")
    ...
    def rollback_find_working(self, component: str) -> int:
        ...
        if not self.json_output:
            print(f"{Colors.HEADER}🔎 Finding Last Working Generation for '{component}'...{Colors.ENDC}")
    ...
    def storage_analyze(self, aggressive: bool) -> int:
        ...
        if self.json_output:
            ...
            print(f"{Colors.BOLD}Reclaimable Space:{Colors.ENDC} {format_size(analysis.reclaimable_gb)}")
            ...
            print(f"{Colors.BOLD}Confidence:{Colors.ENDC} {analysis.confidence:.0%}")
            print(f"{Colors.BOLD}Estimated Time:{Colors.ENDC} {analysis.estimated_time_minutes:.0f} minutes")
    ...
    def storage_cleanup(self, aggressive: bool) -> int:
        ...
        if self.dry_run:
            print(f"{Colors.YELLOW}DRY RUN - Commands will be shown but not executed{Colors.ENDC}")
    ...
    def storage_optimize(self, target_gb: float) -> int:
        ...
        if not self.json_output:
            print(f"{Colors.HEADER}💾 Optimizing to Free {format_size(target_gb)}...{Colors.ENDC}")
        ...
        if self.json_output:
            ...
            if plan.get('steps'):
                ...
                for step in plan['steps']:
                    ...
                    risk_color = Colors.GREEN if risk == 'none' else Colors.YELLOW if risk == 'low' else Colors.RED
                    print(f"  • {step['action']}: {format_size(step['space_gb'])} ({risk_color}{risk} risk{Colors.ENDC})")
    ...
    def storage_find_large(self, min_size_mb: int) -> int:
        """Find large packages"""
        if not self.json_output:
            print(f"{Colors.HEADER}🔍 Finding Packages Larger Than {min_size_mb}MB...{Colors.ENDC}")
    ...
    def security_audit(self, deep: bool) -> int:
        ...
        if not self.json_output:
            ...
            print(f"{Colors.HEADER}🔐 Running Security Audit ({scan_type})...{Colors.ENDC}")
        ...
        if self.json_output:
            ...
            print(f"{Colors.BOLD}Packages Scanned:{Colors.ENDC} {audit.total_packages_scanned}")
            
            # Vulnerability summary
            total_vulns = audit.critical_count + audit.high_count + audit.medium_count + audit.low_count
            if total_vulns > 0:
                print(f"\n{Colors.BOLD}Vulnerabilities Found ({total_vulns}):{Colors.ENDC}")
                if audit.critical_count > 0:
                    print(f"  • {Colors.RED}Critical: {audit.critical_count}{Colors.ENDC}")
                ...
                if audit.vulnerabilities_found:
                    ...
                    for vuln in audit.vulnerabilities_found[:5]:
                        print(f"  • {vuln.cve_id}: {vuln.package} ({format_severity(vuln.severity)})")
            ...
            for name, enabled in configs:
                status = f"{Colors.GREEN}✅{Colors.ENDC}" if enabled else f"{Colors.RED}❌{Colors.ENDC}"
                print(f"  • {name}: {status}")
            ...
            if vulnerabilities:
                ...
                for vuln in vulnerabilities:
                    ...
                    if vuln.fixed_version:
                        print(f"    {Colors.GREEN}Fix: Update to {vuln.fixed_version}{Colors.ENDC}")
                    elif vuln.workaround:
                        print(f"    {Colors.YELLOW}Workaround: {vuln.workaround}{Colors.ENDC}")
            else:
    ...
    def security_harden(self) -> int:
        ...
        if not self.json_output:
            print(f"{Colors.HEADER}🛡️ Generating Hardening Configuration...{Colors.ENDC}")
        ...
        if self.json_output:
            ...
            if hardening.get('impact'):
                ...
                for aspect, impact in hardening['impact'].items():
                    color = Colors.GREEN if impact == 'minimal' else Colors.YELLOW if impact == 'moderate' else Colors.RED
        ...
        if self.json_output:
            ...
            if updates:
                ...
                for update in updates[:10]:
                    print(f"  • {update.get('package', 'Unknown')}: {update.get('version', 'N/A')}")
