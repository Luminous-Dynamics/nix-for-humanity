...
class FAISSMemoryStore:
    ...
    def _init_index(self):
        ...
        logger.info(
            f"Initialized FAISS {self.index_type} index with dimension {self.dimension}"
        )
    ...
    def add_memory(
        ...
        memory = Memory(
            id=memory_id, text=text, embedding=embedding, metadata=metadata or {}
        )
    ...
    def search(
        ...
        distances, indices = self.index.search(
            query_embedding.reshape(1, -1), min(k, len(self.memories))
        )
    ...
    def update_relevance(self, memory_id: str, delta: float):
        ...
        if memory_id in self.memories:
            ...
            self.memories[memory_id].relevance_score = max(
                0.0, min(1.0, self.memories[memory_id].relevance_score)
            )
...
class SemanticLearningSystem:
    ...
    def find_similar_interactions(self, query: str, k: int = 3) -> list[dict[str, Any]]:
    ...
    def get_learning_insights(self) -> dict[str, Any]:
        ...
        helpful_count = sum(
            1 for m in self.memory.memories.values() if m.metadata.get("helpful")
        )

        most_accessed = sorted(
            self.memory.memories.values(), key=lambda m: m.access_count, reverse=True
        )[:5]
