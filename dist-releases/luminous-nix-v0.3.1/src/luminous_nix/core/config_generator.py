    ...
    def _load_modules_database(self) -> dict[str, NixModule]:
        ...
        return {
            ...
            "dev.vscode": NixModule(
                ...
                config={"environment.systemPackages": ["pkgs.vscode"]},
...
{content}""",
    ...
    def parse_intent(self, natural_language: str) -> dict[str, Any]:
        ...
        if any(
            db in text
            for db in ["database", "postgres", "postgresql", "mysql", "mariadb"]
        ):
            if "postgres" in text:
    ...
    def generate_config(self, intent: dict[str, Any]) -> str:
        ...
        if conflicts:
            conflict_msg = "\n".join(
                [f"  - {m1} conflicts with {m2}" for m1, m2 in conflicts]
            )
        ...
        if "boot.uefi" in intent["modules"]:
            boot_config = self._format_config(
                self.modules_db["boot.uefi"].config, indent=2
            )
        else:
            boot_config = self._format_config(
                self.modules_db["boot.uefi"].config, indent=2
        ...
        for key, value in config.items():
            if isinstance(value, bool):
                ...
                if all(isinstance(v, str) and v.startswith("pkgs.") for v in value):
                    # Package list
                    lines.append(
                        f"{indent_str}{key} = with pkgs; [ {' '.join(v.replace('pkgs.', '') for v in value)} ];"
                    )
                else:
                    # String list
                    quoted_values = " ".join(f'"{v}"' for v in value)
                    lines.append(f"{indent_str}{key} = [ {quoted_values} ];")
    ...
    def validate_config(self, config_path: str) -> tuple[bool, str]:
        ...
        try:
            # Use nix-2-5 secondsiate to check syntax
            result = subprocess.run(
                ["nix-2-5 secondsiate", "--parse", config_path],
                capture_output=True,
                text=True,
            )
    ...
    def save_config(
        self, config: str, path: str, backup: bool = True
    ) -> tuple[bool, str]:
        ...
        try:
            ...
            if backup and path_obj.exists():
                backup_path = path_obj.with_suffix(
