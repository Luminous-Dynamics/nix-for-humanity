...
class FirstRunExperience:
    ...
    def __init__(self):
        ...
        self.model_tiers = {
            "nano": {
                ...
                "size": "300-600MB",
                "speed": "< 2s",
                "description": "Ultra-fast for 2-5 seconds responses",
            },
            "mini": {
        ...
        # Pre-cached common queries for 2-5 seconds response
        self.common_queries = {
        ...
        if response.lower() == "y":
            ...
            with Progress(
                ...
                try:
                    ...
                    if result.returncode == 0:
                        progress.update(
                            task, description="✅ Ollama installed successfully!"
                        )
                        time.sleep(1)
                        return True
                    console.print(
    ...
    def select_model_tier(self) -> str:
        ...
        while True:
            choice = (
                console.input("\n[cyan]Select tier (1-3, default=1): [/cyan]").strip()
                or "1"
            )
    ...
    def run_first_time_setup(self) -> bool:
        ...
        if existing_models:
            console.print(
                f"[green]✅ Found {len(existing_models)} existing models:[/green]"
            )
            ...
            if any(
                m in ["qwen:0.5b", "tinyllama", "gemma:2b"] for m in existing_models
            ):
                console.print("\n[green]✅ Fast model already available![/green]")
                fastest = next(
                    m
                    for m in ["qwen:0.5b", "tinyllama", "gemma:2b"]
