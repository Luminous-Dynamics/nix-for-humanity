...
class AdaptiveBehaviorSystem:
    ...
    def __init__(self, user_id: str | None = None):
        ...
        self.profile_path = (
            Path.home() / ".luminous-nix" / "adaptive" / f"{self.user_id}.json"
        )
        self._load_profile()
    ...
    def _adapt_profile(self, observation: Observation):
        ...
        if observation.type == ObservationType.COMMAND_EXECUTED:
            ...
            if observation.details.get("time_to_execute", 999) < 5:
                self.profile.speed_preference = min(
                    1.0, self.profile.speed_preference + rate
                )
                self.profile.technical_level = min(
                    1.0, self.profile.technical_level + rate * 0.5
                )
            ...
            if read_time > 3:
                self.profile.risk_tolerance = max(
                    0.0, self.profile.risk_tolerance - rate * 0.5
                )
                self.profile.detail_preference = min(
                    1.0, self.profile.detail_preference + rate
                )

        elif observation.type == ObservationType.HELP_REQUESTED:
            # Asking for help adjusts technical level
            self.profile.technical_level = max(0.0, self.profile.technical_level - rate)
            self.profile.documentation_engagement = min(
                1.0, self.profile.documentation_engagement + rate
            )

        elif observation.type == ObservationType.ERROR_RECOVERED:
            # Successfully recovering from errors builds confidence
            self.profile.error_recovery_skill = min(
                1.0, self.profile.error_recovery_skill + rate
            )
            self.profile.technical_level = min(
                1.0, self.profile.technical_level + rate * 0.3
            )

        elif observation.type == ObservationType.EXPLORATION:
            # Exploring new commands shows curiosity
            self.profile.exploration_tendency = min(
                1.0, self.profile.exploration_tendency + rate
            )
    ...
    def _update_learning_velocity(self):
        ...
        if len(recent) > 5:
            ...
            successes = sum(
                ...
                for o in recent
                if o.type
                in [ObservationType.COMMAND_EXECUTED, ObservationType.ERROR_RECOVERED]
            )
    ...
    def get_explanation_level(self) -> str:
        ...
        if self.profile.technical_level < 0.7:
    ...
    def suggest_next_action(self) -> str | None:
        ...
        if last_action.type == ObservationType.HELP_REQUESTED:
            return "Try a simple command like 'ask-nix search firefox'"
        if self.profile.exploration_tendency > 0.7:
...
if __name__ == "__main__":
    ...
    adaptive.observe(
        ObservationType.PREVIEW_READ, details={"read_time": 5.2, "scrolled": True}
    )
