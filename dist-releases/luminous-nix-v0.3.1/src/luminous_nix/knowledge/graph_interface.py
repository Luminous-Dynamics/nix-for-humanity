...
from .nix_knowledge_graph import (
    EdgeType,
    NixKnowledgeGraph,
    NodeType,
)
...
class GraphInterface:
    ...
    def __init__(
        self, graph: NixKnowledgeGraph, max_depth: int = 5, max_results: int = 100
    ):
        """
        ...
        logger.info(
            f"GraphInterface initialized (max_depth={max_depth}, max_results={max_results})"
        )
    ...
    def query(self, query_type: QueryType, **params) -> QueryResult:
        ...
        try:
            ...
            return QueryResult(success=True, data=result, query_time_ms=query_time)
            ...
            return QueryResult(
                success=False, data=None, error=str(e), query_time_ms=query_time
            )
    ...
    def natural_language_query(self, query: str) -> QueryResult:
        ...
        if "what services" in query_lower or "list services" in query_lower:
            return self.query(QueryType.FIND_SERVICES)
        if "depends on" in query_lower:
        ...
        # Default response
        return QueryResult(
            success=False, data=None, error=f"Could not understand query: {query}"
        )

    # === Query Handlers ===

    def _find_node(self, name: str, node_type: str | None = None) -> dict | None:
        """Find a node by name"""
    ...
    def _get_node_properties(self, node_id: str) -> dict | None:
    ...
    def _get_related(self, node_id: str, edge_type: str | None = None) -> list[dict]:
        """Get related nodes"""
        related = []
    ...
    def _find_path(self, source_id: str, target_id: str) -> list[str] | None:
    ...
    def _find_packages(self, pattern: str | None = None) -> list[dict]:
    ...
    def _analyze_impact(self, node_id: str) -> dict:
        ...
        return {
            ...
            "dependents": [
                d.to_dict() for d in dependents[:10]
            ],  # Limit for readability
            "impact_assessment": self._assess_impact_level(
                len(dependents), len(affected_nodes)
            ),
        }
