...
class POMLProcessor:
    ...
    def __init__(self, template_dir: Path = None):
        ...
        self.logger.info(f"POML Processor v2 initialized with templates from {template_dir}")
        ...
        for example in element.findall('example'):
            ...
            if input_elem is not None and output_elem is not None:
                input_text = self._substitute_variables(
                    (input_elem.text or "").strip(),
                    variables
                )
                output_text = self._substitute_variables(
                    (output_elem.text or "").strip(),
                    variables
                )
    ...
    def _extract_cache_settings(self, prompt_elem: ET.Element) -> Optional[Dict[str, Any]]:
...
def main():
    ...
    context = POMLContext(
        variables={
            ...
            'available_intents': ['install_package', 'remove_package', 'search_package'],
            'persona': 'developer'
        },
        persona='developer'
    )
