...
class HeadlessTUI:
    ...
    async def execute_script(
        self, script: str | Path | list[TUICommand]
    ) -> dict[str, Any]:
        """
        ...
        for i, command in enumerate(command_objects):
            print(
                f"  [{i+1}/{len(command_objects)}] {command.description or command.type.value}: {command.value}"
            )
    ...
    def save_recording(self, filepath: str):
        ...
        commands = [
            {"type": cmd.type.value, "value": cmd.value, "description": cmd.description}
...
EXAMPLE_SEARCH_TEST = [
    ...
    {
        "type": "assert",
        "value": {"property": "search_results_count", "expected": ">0"},
        "description": "Assert results found",
    },
...
EXAMPLE_INSTALL_TEST = [
    ...
    {
        "type": "assert",
        "value": {"property": "install_preview_visible", "expected": True},
        "description": "Assert install preview shown",
    },
    ...
    {"type": "wait", "value": 2.0, "description": "Wait for installation"},
    {
        ...
        "value": {
            ...
            "expected": "Installation simulated successfully",
        },
