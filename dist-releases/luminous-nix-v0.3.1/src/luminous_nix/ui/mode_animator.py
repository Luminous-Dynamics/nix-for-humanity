...
class ModeAnimator:
    ...
    async def animate_transition(
        ...
        # Get animation type
        animation_type = self.mode_themes.get(
            to_mode, {}
        ).get('animation', AnimationType.FADE)
        ...
        with Live(console=self.console, refresh_per_second=30) as live:
            ...
            while time.time() - start_time < duration:
                ...
                if progress < 0.5:
                    ...
                    mode_text = f"[bold bright_green]{to_mode.upper()}[/bold bright_green]"
                ...
                if progress > 0.5:
                    ...
                    for i, char in enumerate(mode_text):
                        if random.random() < (progress - 0.5) * 2:
                            grid[height // 2][start_x + i] = f"[bold {colors[0]}]{char}[/bold {colors[0]}]"
        ...
        with Live(console=self.console, refresh_per_second=30) as live:
            for step in range(steps + 1):
                ...
                for i in range(10):
                    ...
                    if offset < width:
                        from_part = from_panel[i][offset:] if i < len(from_panel) else " " * (width - offset)
                        line += from_part[:width - offset]
                    
                    # New mode sliding in
                    if offset > 0:
                        to_part = to_panel[i][:offset] if i < len(to_panel) else " " * offset
                        line += to_part
                    
                    display_lines.append(line)
                
                display = "\n".join(display_lines)
                live.update(Panel(display, title=f"[bold]Activating {to_mode.upper()} Mode[/bold]"))
                await asyncio.sleep(step_duration)
    ...
    def get_transition_steps(self, from_mode: str, to_mode: str) -> List[Tuple[str, float]]:
...
async def demo():
    ...
    animator.create_progress_animation(
        'minimal',
        'gaming',
        animator.get_transition_steps('minimal', 'gaming')
    )
