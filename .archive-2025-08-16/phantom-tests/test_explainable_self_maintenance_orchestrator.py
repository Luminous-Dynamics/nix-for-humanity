...
class TestExplainableSelfMaintenanceOrchestrator:
    ...
    async def orchestrator(self, mock_config, temp_dir):
        ...
        with patch('nix_for_humanity.infrastructure.explainable_self_maintenance_orchestrator.SelfMaintenanceSystem') as mock_maintenance, \
             patch('nix_for_humanity.infrastructure.explainable_self_maintenance_orchestrator.CausalMaintenanceExplainer') as mock_explainer, \
             patch('nix_for_humanity.infrastructure.explainable_self_maintenance_orchestrator.ConstitutionalAIValidator') as mock_constitutional, \
             patch('nix_for_humanity.infrastructure.explainable_self_maintenance_orchestrator.InvisibleExcellenceEngine') as mock_excellence:
    ...
    async def test_orchestrator_initialization(self, mock_config, temp_dir):
        ...
        with patch('nix_for_humanity.infrastructure.explainable_self_maintenance_orchestrator.SelfMaintenanceSystem') as mock_maintenance:
            ...
            assert orchestrator.config.transparency_level == MaintenanceTransparencyLevel.FULL_DISCLOSURE
            assert orchestrator.config.constitutional_ai_enabled is True
            assert orchestrator.config.trust_building_enabled is True

    @pytest.mark.asyncio
    async def test_explainable_maintenance_execution_success(self, orchestrator, sample_maintenance_action, sample_decision_context):
        ...
        mock_decision = ExplainableMaintenanceDecision(
            ...
            causal_explanation={
                ...
                "confidence": 0.95,
                "reasoning_chain": ["Updates detected", "Security assessment", "Risk evaluation"]
            },
        ...
        orchestrator.causal_explainer.create_explainable_decision.return_value = mock_decision
        orchestrator.constitutional_validator.validate_action.return_value = True
        orchestrator.self_maintenance.execute_maintenance.return_value = {"success": True, "duration": 285}
        
        # Execute maintenance
        result = await orchestrator.execute_explainable_maintenance(
            sample_maintenance_action, 
    ...
    @pytest.mark.asyncio
    async def test_constitutional_ai_blocking(self, orchestrator, sample_maintenance_action, sample_decision_context):
    ...
    @pytest.mark.asyncio
    async def test_multi_stakeholder_explanations(self, orchestrator, sample_maintenance_action, sample_decision_context):
        ...
        orchestrator.causal_explainer.create_explainable_decision.return_value = mock_decision
        orchestrator.constitutional_validator.validate_action.return_value = True
        orchestrator.self_maintenance.execute_maintenance.return_value = {"success": True}
        ...
        assert StakeholderType.SACRED_TRINITY in result.decision.stakeholder_explanations
    ...
    async def test_transparency_level_adaptation(self, mock_config, temp_dir):
        ...
        with patch('nix_for_humanity.infrastructure.explainable_self_maintenance_orchestrator.SelfMaintenanceSystem') as mock_maintenance:
            ...
            assert orchestrator.config.transparency_level == MaintenanceTransparencyLevel.AMBIENT_AWARENESS
    ...
    async def test_flow_state_protection(self, orchestrator, sample_maintenance_action):
        ...
        flow_context = MaintenanceDecisionContext(
            ...
            user_context={"user_in_flow_state": True, "flow_duration": timedelta(minutes=45)},
        ...
        mock_decision = ExplainableMaintenanceDecision(
            ...
            stakeholder_explanations={StakeholderType.END_USER: "Maintenance deferred to protect your focus."},
            constitutional_compliance=True,
            trust_building_elements={"flow_state_respect": "Deferred to preserve deep work session."}
        )
        orchestrator.causal_explainer.create_explainable_decision.return_value = mock_decision
        orchestrator.constitutional_validator.validate_action.return_value = True
        ...
        assert "deferred" in result.decision.trust_building_elements["flow_state_respect"].lower()

    @pytest.mark.asyncio
    async def test_trust_building_through_vulnerability(self, orchestrator, sample_maintenance_action, sample_decision_context):
    ...
    async def test_invisible_excellence_mode_integration(self, mock_config, temp_dir):
        ...
        with patch('nix_for_humanity.infrastructure.explainable_self_maintenance_orchestrator.SelfMaintenanceSystem') as mock_maintenance, \
            ...
            assessment = await orchestrator.invisible_excellence.assess_transcendence_readiness(
                "test_action", 
                {"user_mastery": "advanced"}
            )
    ...
    @pytest.mark.asyncio
    async def test_error_handling_and_recovery(self, orchestrator, sample_maintenance_action, sample_decision_context):
        """Test comprehensive error handling and recovery mechanisms."""
        # Mock causal explainer to raise an exception
        orchestrator.causal_explainer.create_explainable_decision.side_effect = Exception("Explanation generation failed")
        ...
        orchestrator.self_maintenance.execute_maintenance.side_effect = Exception("Maintenance failed")
    ...
    @pytest.mark.asyncio
    async def test_sacred_trinity_governance_validation(self, orchestrator, sample_maintenance_action, sample_decision_context):
        ...
        trinity_explanation = result.decision.stakeholder_explanations[StakeholderType.SACRED_TRINITY]
        assert "Human vision" in trinity_explanation
        assert "Claude" in trinity_explanation  
        assert "Local LLM" in trinity_explanation
        assert result.decision.trust_building_elements["sacred_trinity_consensus"] is not None

    # Performance and Integration Tests

    @pytest.mark.asyncio
    async def test_orchestrator_performance_under_load(self, orchestrator, sample_maintenance_action, sample_decision_context):
        ...
        mock_decision = ExplainableMaintenanceDecision(
            ...
            stakeholder_explanations={StakeholderType.END_USER: "Performance test action"},
            constitutional_compliance=True,
            trust_building_elements={}
        )
        ...
        for i in range(5):
            ...
            task = orchestrator.execute_explainable_maintenance(action, sample_decision_context)
            tasks.append(task)
    ...
    def test_consciousness_first_principle_validation(self, orchestrator):
        ...
        assert any("rollback" in violation for violation in validation_result["violations"])
        assert any("override" in violation for violation in validation_result["violations"])
