...
class ComprehensiveTestSuite:
    ...
    def test_functional(self) -> bool:
        ...
        print(
            f"  {'✅' if has_session else '❌'} Session tracking: {insights['session']['total_queries']} queries"
        )
        print(f"  {'✅' if has_analytics else '❌'} Analytics data collected")
        print(
            f"  {'✅' if has_recommendations else '❌'} Cache recommendations available"
        )
        ...
        print(
            f"  {'✅' if has_predictions else '❌'} Predictions generated: {len(response.predictions)}"
        )
        if has_predictions:
        ...
        print(
            f"  {'✅' if monitoring_active else '❌'} Channels monitored: {update_stats['channels_monitored']}"
        )
    ...
    def test_edge_cases(self) -> bool:
        ...
        for query, description in edge_cases:
            try:
                response = intelligence.intelligent_search(
                    query, use_all_features=False
                )
    ...
    def test_performance(self) -> bool:
        ...
        test_scenarios = [
            ...
            (
                "Complex query",
                "install web browser with developer tools and privacy features",
                True,
            ),
        ...
        for scenario_name, query, use_features in test_scenarios:
            ...
            for _ in range(10):
                start = time.time()
                response = intelligence.intelligent_search(
                    query, use_all_features=use_features
                )
        ...
        avg_performance = statistics.mean(
            [m["avg_ms"] for m in self.performance_metrics]
        )
    ...
    def test_stress(self) -> bool:
        ...
        if valid_times:
            ...
            print(
                f"  95th percentile: {statistics.quantiles(valid_times, n=20)[18]:.1f}ms"
            )
    ...
    def test_learning(self) -> bool:
        ...
        for base_query, expected in test_cases:
            ...
            print(
                f"  After '{base_query}': {matches}/{len(expected)} correct predictions"
            )
        ...
        print(
            f"  {'✅' if has_hot_packages else '❌'} Hot packages identified: {len(recommendations['packages_to_cache'])}"
        )
    ...
    def test_network(self) -> bool:
        ...
        print(
            f"  {'✅' if shared else '❌'} Knowledge shared: Source = {response2.source}"
        )
        ...
        print("\nNetwork Statistics:")
        print(
            f"  Node 1: {stats1['peer_count']} peers, {stats1['queries_shared']} shared"
        )
        print(
            f"  Node 2: {stats2['peer_count']} peers, {stats2['queries_received']} received"
        )
    ...
    def test_error_recovery(self) -> bool:
        ...
        print(
            f"  {'✅' if works_after_db_stress else '❌'} System works after database stress"
        )
        ...
        try:
            ...
            print("  ❌ Failed with corrupted cache")
        ...
        print(
            f"  {'✅' if responsive else '❌'} Still responsive after 100 queries ({elapsed:.1f}ms)"
        )
    ...
    def test_real_world(self) -> bool:
        ...
        for query in new_user_queries:
            ...
            print(
                f"  {'✅' if has_results else '❌'} '{query[:30]}...' → {len(result['results'])} results"
            )
        ...
        for query in admin_queries:
            ...
            has_results = len(result["results"]) > 0
            print(
                f"  {'✅' if has_results else '❌'} '{query}' → {len(result['results'])} results"
            )
            scenarios_passed.append(has_results)
