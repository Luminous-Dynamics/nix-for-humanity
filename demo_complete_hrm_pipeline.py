...
try:
    from luminous_nix.ai.hrm_training_pipeline import (
        ...
        TrainingConfig, NixOSDataGenerator, ProductionHRMTrainer
    )
    from luminous_nix.ai.hrm_benchmarking_suite import (
        run_comprehensive_benchmark,
        HRMBenchmarkSuite, BenchmarkConfig
    )
   ...
   • Response caching and optimization
    ...
    try:
        ...
        if 'test_val_strategy_accuracy' in training_results:
            print(f"  Test Accuracy: {training_results['test_val_strategy_accuracy']:.1%}")
    ...
    try:
        ...
        print(f"  Production Readiness: {benchmark_results['production_readiness']:.1%}")
...
def run_quick_demo():
    ...
    try:
        ...
        for i, query in enumerate(test_queries):
            ...
            if solution:
                preview = solution.split('\n')[0][:50] + "..." if len(solution) > 50 else solution
