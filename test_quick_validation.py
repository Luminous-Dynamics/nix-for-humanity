...
def test_basic_operations():
    ...
    for query in test_queries:
        try:
            ...
            has_results = response.results is not None and isinstance(response.results, list)
            ...
            print(f"  {status} '{query}': {len(response.results)} results in {response.response_time_ms:.1f}ms")
            
            if not passed:
                all_passed = False
                if not has_results:
                    print(f"     ERROR: No results or wrong type")
                if not has_intent:
                    print(f"     ERROR: No intent")
                if not has_time:
                    print(f"     ERROR: Invalid time")
    ...
    try:
        ...
        if cli_passed:
            ...
            print(f"  ❌ CLI search failed")
            if not has_query:
                print(f"     ERROR: No query in result")
            if not has_results:
                print(f"     ERROR: No results in result")
            if not has_time:
                print(f"     ERROR: No response_time in result")
