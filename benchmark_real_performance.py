...
from luminous_nix.core.package_index import PackageIndex, get_package_index
...
def benchmark_index(
    query: str, iterations: int = 3, index: PackageIndex = None
) -> dict:
...
def print_results(results: list):
    ...
    for query, methods in queries.items():
        ...
        for m in methods:
            speedup = (
                baseline["median_ms"] / m["median_ms"] if m["median_ms"] > 0 else 999
            )
...
def main():
    ...
    for query in queries:
        ...
        subprocess_result = benchmark_subprocess(
            query, iterations=1
        )  # Only 1 iteration because it's slow
    ...
    if subprocess_times and index_times:
        ...
        print("   (Not 10,000x, but still significant!)")
