...
class PerformanceProfiler:
        ...
        for name, times in self.timings.items():
            ...
            if avg_time > 100:  # Over 100ms average
                self.bottlenecks.append({
                    ...
                    'avg_ms': avg_time,
                    'max_ms': max_time,
                    'recommendation': self._get_timing_recommendation(name, avg_time)
                })
        ...
        for name, memory in self.memory_usage.items():
            ...
            if avg_memory > 10:  # Over 10MB
                self.bottlenecks.append({
                    ...
                    'avg_mb': avg_memory,
                    'max_mb': max_memory,
                    'recommendation': self._get_memory_recommendation(name, avg_memory)
                })
    ...
    def generate_report(self) -> str:
        ...
        for name, times in sorted(self.timings.items(), key=lambda x: statistics.mean(x[1]), reverse=True):
        ...
        for name, memory in sorted(self.memory_usage.items(), key=lambda x: statistics.mean(x[1]) if x[1] else 0, reverse=True):
            if memory:
        ...
        if self.bottlenecks:
            for bottleneck in sorted(self.bottlenecks, key=lambda x: x.get('avg_ms', x.get('avg_mb', 0)), reverse=True):
                severity_icon = "🔴" if bottleneck['severity'] == 'high' else "🟡"
                report.append(f"{severity_icon} {bottleneck['name']}")
                
                if bottleneck['type'] == 'timing':
                    report.append(f"   Time: {bottleneck['avg_ms']:.1f}ms avg, {bottleneck['max_ms']:.1f}ms max")
                else:
                    report.append(f"   Memory: {bottleneck['avg_mb']:.1f}MB avg, {bottleneck['max_mb']:.1f}MB max")
...
def profile_core_operations():
    ...
    for query in test_queries:
        ...
        with profiler.profile_section("cache_lookup"):
            cache_results, cache_time, source = intelligence.base_cache.search_hybrid(query)
        ...
        with profiler.profile_section("network_check"):
            ...
            if hasattr(intelligence.collaborative, 'search_network'):
                ...
                network_results = intelligence.collaborative.search(query) if hasattr(intelligence.collaborative, 'search') else []
    ...
    with profiler.profile_section("batch_analytics"):
        ...
        recommendations = intelligence.analytics.get_smart_cache_recommendations(limit=100)
    ...
    results = {
        'timings': {k: {'avg': statistics.mean(v), 'max': max(v), 'count': len(v)} 
                   for k, v in profiler.timings.items()},
        'memory': {k: {'avg': statistics.mean(v) if v else 0, 'max': max(v) if v else 0} 
...
def profile_specific_bottleneck(component: str):
    ...
    if component == "semantic":
        ...
        test_queries = [
            "vim", "neovim", "emacs",  # Simple
            "text editor for python", "IDE for web development",  # Complex
            "instal fierrfox", "pythn developmnt"  # Typos
        ]
        
        for query in test_queries:
            result = profiler.profile_function(
                intelligence.semantic.understand,
                query
            )
        ...
        for i in range(100):
            ...
            result = profiler.profile_function(
                intelligence.base_cache.search_hybrid,
                query
            )
        ...
        for event in events:
            profiler.profile_function(
                intelligence.analytics.track_event,
                event
            )
...
def main():
    ...
    if all_timings:
        ...
        print(f"  • 95th percentile: {statistics.quantiles(all_timings, n=20)[18]:.2f}ms")
