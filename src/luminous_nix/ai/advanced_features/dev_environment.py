...
class DevEnvironmentGenerator:
    ...
    def analyze_project(
        self, project_path: str = ".", use_poml: bool = True
    ) -> EnvironmentSpec:
        """
                ...
                with open(file_path) as f:
                    ...
                    for pattern in patterns:
                        if pattern.lower() in content:
                            db_name = (
                                pattern.split("_")[0] if "_" in pattern else pattern
                            )
  ...
  postgres_data:"""
    ...
    def _generate_aliases(self, project_type: ProjectType) -> dict[str, str]:
        ...
        if project_type == ProjectType.PYTHON:
            ...
            aliases.update(
                {"start": "npm start", "test": "npm test", "build": "npm run build"}
            )
        elif project_type == ProjectType.RUST:
            aliases.update(
                {
    ...
    def _poml_enhanced_analysis(self, path: Path) -> Optional[EnvironmentSpec]:
        ...
        try:
            ...
            poml_path = (
                Path(__file__).parent.parent.parent / "agents" / "devenv_analysis.poml"
            )
            ...
            result = processor.process(
                str(poml_path), project_files=project_files, file_contents=file_contents
            )
