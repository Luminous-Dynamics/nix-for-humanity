...
class RollbackIntelligence:
    ...
    def __init__(self):
        ...
        self.breaking_patterns = {
            ...
            "bootloader": {
                ...
                "detection": lambda diff: any(
                    "boot" in pkg.lower() for pkg in diff.get("packages_updated", [])
                ),
            },
            "graphics_driver": {
                "risk": "high",
                ...
                "detection": lambda diff: any(
                    gpu in pkg
                    for gpu in ["nvidia", "amd", "intel-graphics"]
            ...
            "networking": {
                ...
                "detection": lambda diff: any(
                    "network" in pkg.lower() or "wifi" in pkg.lower()
                    for pkg in diff.get("packages_updated", [])
                ),
            },
            "audio": {
                ...
                "detection": lambda diff: any(
                    audio in pkg
                    for audio in ["pulse", "pipewire", "alsa"]
                    for pkg in diff.get("packages_updated", [])
                ),
            },
            "desktop_environment": {
                ...
                "detection": lambda diff: any(
                    de in pkg
                    for de in ["gnome", "kde", "plasma", "xfce"]
                    for pkg in diff.get("packages_updated", [])
                ),
            },
        }
    ...
    def analyze_system_failure(
        self, symptoms: Optional[str] = None
    ) -> RollbackAnalysis:
        """
        ...
        try:
            ...
            if not generations:
                return self._create_fallback_analysis(
                    current_gen, "No generation history available"
                )
            ...
            return self._create_fallback_analysis(
                self.analyzer._get_current_generation(), f"Analysis error: {str(e)}"
            )
    ...
    def find_last_working_generation(self, failing_component: str) -> Optional[int]:
        ...
        try:
            ...
            logger.error(
                f"Failed to find working generation for {failing_component}: {e}"
            )
            return None
        ...
        try:
            ...
            if diff["packages_removed"]:
                summary_parts.append(
                    f"Removed {len(diff['packages_removed'])} packages"
                )
            if diff["packages_updated"]:
                summary_parts.append(
                    f"Updated {len(diff['packages_updated'])} packages"
                )
            if diff["kernel_changed"]:
                summary_parts.append("Kernel changed")
            if diff["services_changed"]:
                summary_parts.append(
                    f"Services changed: {', '.join(diff['services_changed'][:3])}"
                )
    ...
    def _analyze_symptoms(
        self, symptoms: str, breaking_changes: List[str]
    ) -> Optional[str]:
    ...
    def _create_fallback_analysis(
        self, current_gen: int, reason: str
    ) -> RollbackAnalysis:
