...
class NixOSQueryDataset(Dataset):
    ...
    def _load_data(self, data_path: str):
        ...
        logger.info(
            f"Loaded {len(self.data)} queries with {len(self.category_map)} categories"
        )
...
class NeuralModelTrainer:
    ...
    def train(
        self, train_loader: DataLoader, val_loader: DataLoader, epochs: int = 10
    ) -> Dict:
        """Full training loop"""
...
class RealNeuralQueryProcessor:
    ...
    def create_default_model(self):
        ...
        self.model = RealNixOSNeuralNetwork(self.vocab_size, self.num_categories).to(
            self.device
        )
    ...
    def _category_to_command(self, category: str, query: str) -> str:
        ...
        if category == "install":
            ...
            words = [
                w for w in query_lower.split() if w not in ["search", "find", "list"]
            ]
            if words:
...
def train_real_model(
    data_path: str = "data/training/comprehensive_training_data.json",
    ...
    model = RealNixOSNeuralNetwork(
        vocab_size=len(dataset.vocab), num_categories=len(dataset.category_map)
    )
