...
zramSwap.enable = true;"""
    ...
    def _generate_service_config(
        self, service: str, ssl: bool = False, production: bool = False
    ) -> str:
...
'';"""
    ...
    def build_complete_dataset(self) -> Path:
        ...
        summary = {
            ...
            "task_distribution": {
                "dependency": sum(
                    1 for e in all_examples if e.task_type == "dependency"
                ),
                "configuration": sum(
                    1 for e in all_examples if e.task_type == "configuration"
                ),
                "error": sum(1 for e in all_examples if e.task_type == "error"),
                "optimization": sum(
                    1 for e in all_examples if e.task_type == "optimization"
                ),
            },
        }
