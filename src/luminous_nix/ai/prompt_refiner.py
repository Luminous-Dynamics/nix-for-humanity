...
class PromptRefiner:
    ...
    def __init__(self, corpus_dir: str = "corpus"):
        ...
        self.patterns = {
            ...
            PromptType.EXPLAIN: [r"what is", r"how does", r"explain", r"tell me about"],
    ...
    def _get_relevant_context(
        self, user_input: str, prompt_type: PromptType
    ) -> List[str]:
        ...
        context.append(
            "NixOS is a Linux distribution built on the Nix package manager. Configuration is declarative in configuration.nix."
        )

        # Add type-specific context
        if prompt_type == PromptType.INSTALL_PACKAGE:
            context.append(
                "Packages can be installed with: nix-env -iA nixos.package, or added to environment.systemPackages in configuration.nix"
            )
            ...
            context.append(
                "Services are configured in configuration.nix under the 'services' attribute set."
            )
            ...
            context.append(
                "Common troubleshooting: Check nixos-rebuild test for errors, view logs with journalctl, check disk space with df -h"
            )
    ...
    def _generate_constraints(self, prompt_type: PromptType) -> List[str]:
        ...
        if prompt_type == PromptType.INSTALL_PACKAGE:
            ...
            constraints.append(
                "Always mention that changes require nixos-rebuild switch"
            )
    ...
    def _refine_text(self, user_input: str, prompt_type: PromptType) -> str:
        ...
        if prompt_type == PromptType.INSTALL_PACKAGE:
            ...
            if (
                "enable" not in user_input.lower()
                and "configure" not in user_input.lower()
            ):
                refined = f"How do I configure {user_input} service on NixOS?"

        # Ensure it's a clear question
        if not refined.endswith("?") and prompt_type in [
            PromptType.EXPLAIN,
    ...
    def _calculate_confidence(
        self, user_input: str, context: List[str], examples: List[str]
    ) -> float:
    ...
    def validate_response(
        ...
        if prompt_type == PromptType.INSTALL_PACKAGE:
            if (
                "nix-env -iA" not in response
                and "environment.systemPackages" not in response
            ):
...
class SmartOllamaClient:
    ...
    def ask(self, question: str, validate: bool = True) -> Dict[str, Any]:
        ...
        if refined_prompt.confidence < 0.5:
            self.console.print(
                f"[yellow]⚠ Low confidence ({refined_prompt.confidence:.0%}) - answer may be less accurate[/yellow]"
            )

        # Get response from Ollama
        if self.ollama and self.ollama.is_available():
            response = self.ollama.generate(
                refined_prompt.to_ollama_format(), model="mistral"
            )
            ...
            if validate:
                validation = self.refiner.validate_response(
                    answer, refined_prompt.prompt_type
                )
