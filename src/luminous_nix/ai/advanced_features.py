...
class MultiStepPlanner:
    ...
    def _load_plan_templates(self):
        ...
        self.plans_db = {
            'python_dev_environment': Plan(
                ...
                steps=[
                    {'action': 'install', 'packages': ['python312', 'python312Packages.pip']},
                    {'action': 'install', 'packages': ['python312Packages.virtualenv', 'python312Packages.poetry']},
                    {'action': 'install', 'packages': ['python312Packages.black', 'python312Packages.ruff', 'python312Packages.mypy']},
                    {'action': 'configure', 'file': 'shell.nix', 'template': 'python_dev'},
            ...
            'web_server_ssl': Plan(
                ...
                steps=[
                    ...
                    {'action': 'create', 'file': '/etc/nginx/sites-available/default', 'template': 'nginx_ssl'},
            ...
            'gaming_setup': Plan(
                ...
                steps=[
                    {'action': 'configure', 'hardware': {'opengl': True, 'pulseaudio': True}},
                    {'action': 'install', 'packages': ['steam', 'steam-run', 'steamcmd']},
                    ...
                    {'action': 'restart', 'required': True, 'reason': 'Graphics driver changes'},
                ],
    ...
    def create_plan(self, goal: str) -> Plan:
        ...
        for key, plan in self.plans_db.items():
            if key in goal.lower() or any(word in goal.lower() for word in key.split('_')):
                return plan
    ...
    def _extract_packages(self, text: str) -> List[str]:
        ...
        known_packages = ['firefox', 'chrome', 'vim', 'emacs', 'git', 'docker', 
                         'python', 'nodejs', 'rust', 'go', 'java', 'vscode']
...
class LearningSystem:
    ...
    def record_correction(self, correction: Correction):
        ...
        conn.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (correction.original_query, correction.original_response,
              correction.user_correction, correction.correct_response,
              correction.category, correction.timestamp))
        conn.commit()
    ...
    def _learn_pattern(self, correction: Correction, conn):
        ...
        pattern = self._extract_pattern(correction.original_query, correction.correct_response)
        if pattern:
            conn.execute('''
    ...
    def apply_learning(self, query: str) -> Optional[str]:
        ...
        for pattern, replacement, confidence in cursor:
            if pattern in query.lower():
                ...
                # Update usage count
                conn.execute('UPDATE patterns SET usage_count = usage_count + 1 WHERE pattern = ?', (pattern,))
    ...
    def _load_knowledge_base(self):
        ...
        return {
            'permission_denied': {
                ...
                'preventions': ['Always use nix-shell for development', 'Avoid sudo for package installation']
            },
            ...
            'command_not_found': {
                ...
                'preventions': ['Use nix-shell for project dependencies', 'Add to configuration.nix']
            },
            ...
            'build_failure': {
                ...
                'preventions': ['Use flakes for reproducible builds', 'Pin nixpkgs version']
            },
            ...
            'disk_space': {
                ...
                'preventions': ['Regular garbage collection', 'Set up automatic GC in configuration.nix']
            },
        }
...
class AdvancedAI:
    ...
    def record_feedback(self, query: str, response: str, correction: str):
        ...
        self.learning.record_correction(Correction(
...
def example_advanced_ai():
    ...
    result = ai.process_complex_request("Set up complete Python development environment with testing tools")
