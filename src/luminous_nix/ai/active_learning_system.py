...
class ActiveLearningSystem:
    ...
    def record_feedback(self, query: str, prediction: Dict, feedback: Dict) -> Dict:
        ...
        feedback_entry = {
            ...
            'correct_category': feedback.get('correct_category', prediction.get('category')),
            'correct_command': feedback.get('correct_command', prediction.get('command')),
    ...
    def _learn_pattern(self, query: str, category: str, command: str):
        ...
        action_words = ['install', 'update', 'search', 'configure', 'enable', 
                       'disable', 'remove', 'list', 'create', 'setup']
        ...
        if key_terms:
            ...
            if len(self.pattern_corrections[pattern]) >= self.learning_config['min_feedback_for_update']:
    ...
    def _store_pattern(self, pattern: str, category: str, command: str):
        ...
        if existing:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, 1)
            """, (pattern, category, command, self._calculate_success_rate(pattern, category)))
    ...
    def get_learning_metrics(self) -> Dict:
        ...
        if self.metrics['total_feedback'] > 0:
            accuracy = self.metrics['positive_feedback'] / self.metrics['total_feedback']
        ...
        return {
            ...
            'positive_rate': self.metrics['positive_feedback'] / max(1, self.metrics['total_feedback']),
    ...
    def simulate_user_feedback(self, num_interactions: int = 50) -> Dict:
        ...
        test_queries = [
            ...
            ("update all packages", "update", "nix-channel --update && nix-env -u", 0.85),
            ("search markdown editor", "search", "nix search nixpkgs markdown", 0.9),
            ("enable docker service", "config", "systemctl enable docker", 0.88),
            ("python with tensorflow", "dev", "nix-shell -p python3 python3Packages.tensorflow", 0.92),
        ...
        for i in range(num_interactions):
            ...
            # Simulate prediction
            prediction = {
                'category': correct_cat if random.random() < base_accuracy else 'unknown',
                'command': correct_cmd if random.random() < base_accuracy else 'nix search',
        ...
        print(f"  High-confidence patterns: {len(improvements['high_confidence_patterns'])}")
...
def test_active_learning():
    ...
    if improvements['frequent_corrections']:
        ...
        for correction in improvements['frequent_corrections'][:3]:
            ...
            print(f"    Correct: {correction['correct_category']} → {correction['correct_command'][:30]}")
