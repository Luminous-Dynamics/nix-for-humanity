...
class HRMwithSimpleRL:
    ...
    def learn_from_experience(self, experience: Experience):
        ...
        if experience.done:
            ...
            next_state_key = (
                self.get_state_key(experience.next_state)
        ...
        # Q-learning update
        self.q_table[state_key][experience.action] = current_q + self.alpha * (
            target - current_q
        )
        ...
        if experience.reward > 0:
            ...
            self.metrics.success_rate = self.metrics.success_rate * 0.95 + 0.05
        else:
        ...
        self.metrics.avg_reward = (
            ...
            if self.metrics.recent_rewards
            else 0
        )
    ...
    def get_solution(
        ...
        """
        # Create state
        state = {"query": query, "context": context or {}, "timestamp": time.time()}
    ...
    def get_learning_stats(self) -> Dict[str, Any]:
        ...
        for action in self.action_space:
            ...
            if stats["total"] > 0:
                ...
                action_performance[
                    action
                ] = f"{success_rate:.1f}% ({stats['total']} uses)"
...
def demonstrate_learning():
    ...
    for episode in range(50):
        ...
        rating = min(
            1.0,
            base_rating + learning_bonus + strategy_bonus + random.uniform(-0.2, 0.2),
        )
        success = rating > 0.4
