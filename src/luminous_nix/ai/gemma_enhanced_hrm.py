...
class GemmaEnhancedHRM(nn.Module if TORCH_AVAILABLE else object):
    ...
    def __init__(self, gemma_dim=768, hrm_features=256, hidden_dim=512):
        ...
        if TORCH_AVAILABLE:
            ...
            self.semantic_gate = nn.Sequential(nn.Linear(512, 1), nn.Sigmoid())

            self.feature_gate = nn.Sequential(nn.Linear(256, 1), nn.Sigmoid())
    ...
    def _fallback_forward(self, gemma_embedding, hrm_features):
        ...
        intent_idx = min(
            int(combined_score * len(INTENT_CLASSES)), len(INTENT_CLASSES) - 1
        )
...
class HRMFeatureExtractor:
    ...
    def extract(self, query: str) -> np.ndarray:
        ...
        features[204] = (
            1.0 if any(cmd in query for cmd in ["install", "remove", "update"]) else 0.0
        )
        features[205] = (
            1.0 if any(char.isdigit() for char in query) else 0.0
        )  # Has numbers
...
class GemmaHRMPredictor:
    ...
    def __init__(self, model_path: Optional[Path] = None):
        ...
        self.stats = {"predictions": 0, "cache_hits": 0, "avg_latency": 0}
                    ...
                    results.append(
                        {
                            ...
                            "strategy": STRATEGIES[
                                torch.argmax(outputs["strategy"][i]).item()
                            ],
                        }
                    )
        else:
...
def test_gemma_hrm():
    ...
    batch_queries = ["install vim", "install emacs", "install vscode"]
