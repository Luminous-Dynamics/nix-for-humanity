...
class OllamaSimulator:
    ...
    def query(self, prompt: str) -> Tuple[str, float, float]:
        ...
        if "dependency" in prompt.lower():
            ...
            response = (
                "This error might be caused by missing packages or outdated channels."
            )
            confidence = 0.65
        else:
...
def benchmark_dependency_resolution() -> List[BenchmarkResult]:
    ...
    # Test case
    task_description = (
        "Resolve conflict between python311 and python312 with numpy versions"
    )
...
def print_comparison_table(results: List[BenchmarkResult]):
    ...
    for task_name, methods in tasks.items():
        ...
        if hrm and ollama:
            ...
            print(f"  {'-'*25} {'-'*15} {'-'*15} {'-'*20}")
            print(
                f"  {'Response Time (ms)':<25} {hrm.response_time:<15.1f} {ollama.response_time:<15.1f} {speed_improvement:.1f}x faster"
            )
            print(
                f"  {'Model Size (MB)':<25} {hrm.model_size:<15.0f} {ollama.model_size:<15.0f} {size_reduction:.1f}x smaller"
            )
            print(
                f"  {'Accuracy':<25} {hrm.accuracy:<15.0%} {ollama.accuracy:<15.0%} +{accuracy_gain:.0f}% better"
            )
            print(
                f"  {'Confidence':<25} {hrm.confidence:<15.0%} {ollama.confidence:<15.0%}"
            )
            print(
                f"  {'Memory Usage (MB)':<25} {hrm.memory_usage:<15.0f} {ollama.memory_usage:<15.0f}"
            )
...
def calculate_overall_stats(results: List[BenchmarkResult]):
    ...
    if hrm_results and ollama_results:
        ...
        avg_ollama_time = sum(r.response_time for r in ollama_results) / len(
            ollama_results
        )

        avg_hrm_accuracy = sum(r.accuracy for r in hrm_results) / len(hrm_results)
        avg_ollama_accuracy = sum(r.accuracy for r in ollama_results) / len(
            ollama_results
        )
        ...
        print(
            f"  • Size: HRM is {ollama_results[0].model_size/hrm_results[0].model_size:.1f}x smaller"
        )
        print(
            f"  • Accuracy: HRM is {(avg_hrm_accuracy-avg_ollama_accuracy)*100:.0f}% more accurate"
        )
        ...
        print(
            f"  • Savings: {(ollama_results[0].model_size - hrm_results[0].model_size)/1000:.1f}GB disk space"
        )
