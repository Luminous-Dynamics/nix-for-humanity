...
class NeuralHRMReasoner:
    ...
    def _load_model(self) -> bool:
        ...
        try:
            if not self.model_path.exists():
                ...
                alt_path = Path("/srv/luminous-dynamics/11-meta-consciousness/luminous-nix") / "models" / "hrm-nixos-v1" / "hrm_trained.pt"
                if alt_path.exists():
    ...
    def get_stats(self) -> Dict:
        ...
        avg_time = self.total_time / max(1, self.total_inferences) if self.total_inferences > 0 else 0
        
        return {
            'model_accuracy': self.accuracy,
            'total_inferences': self.total_inferences,
            'average_inference_time_ms': avg_time,
            'model_type': 'Neural Network (BiLSTM)' if self.loaded else 'Fallback (keywords)',
            'parameters': sum(p.numel() for p in self.model.parameters()) if self.loaded else 0,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print(f"Using: {'Neural Network' if result['using_neural_network'] else 'Fallback'}")
    else:
        # Run test suite
        test_queries = [
            "install firefox",
