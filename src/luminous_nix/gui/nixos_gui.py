...
from luminous_nix.execution.safe_executor import ExecutionMode, SafeExecutor
...
class NixOSGUI:
    ...
    def create_qt_gui(self):
        ...
        class MainWindow(QMainWindow):
            ...
            def create_packages_tab(gui_self):
                ...
                gui_self.search_input.setPlaceholderText(
                    "Enter package name or description..."
                )
                ...
                for category in categories:
                    ...
                    btn.clicked.connect(
                        lambda checked, c=category: gui_self.browse_category(c)
                    )
                ...
                gui_self.package_list.setHeaderLabels(
            ...
            def create_configuration_tab(gui_self):
                ...
                gui_self.config_selector.currentTextChanged.connect(
                    gui_self.load_configuration
                )
            ...
            def create_generations_tab(gui_self):
                ...
                gui_self.generation_list.setHeaderLabels(
            ...
            def create_ai_assistant_tab(gui_self):
                ...
                for action in quick_actions:
                    ...
                    btn.clicked.connect(
                        lambda checked, a=action: gui_self.quick_ai_query(a)
                    )
            ...
            def search_packages(gui_self):
                ...
                from luminous_nix.package_aliases import (
                    suggest_similar,
                )
                ...
                for alias, package in suggestions:
                    item = QTreeWidgetItem(
                        [package, "latest", "Available", f"Alias: {alias}"]
                    )
            ...
            def browse_category(gui_self, category):
                ...
                for alias, package in packages.items():
                    item = QTreeWidgetItem(
                        [package, "latest", "Available", f"Category: {category}"]
                    )
                    gui_self.package_list.addTopLevelItem(item)

                gui_self.status_bar.showMessage(
                    f"Showing {len(packages)} {category} packages"
                )
            ...
            def install_selected(gui_self):
                ...
                if not selected:
                    QMessageBox.warning(
                        gui_self, "No Selection", "Please select packages to install"
                    )
                    return
                ...
                if reply == QMessageBox.StandardButton.Yes:
                    for package in packages:
                        ...
                        result = gui_self.parent_gui.executor.execute(
                            cmd, ExecutionMode.CONFIRMED
                        )
                        if result.success:
            ...
            def run_health_check(gui_self):
                ...
                checks = [
                    ...
                    (
                        "Memory Usage",
                        gui_self.parent_gui.system_info["memory_usage"] < 80,
                    ),
                ...
                for check_name, is_healthy in checks:
                    ...
                    if not is_healthy:
                        if "Disk" in check_name:
                            gui_self.recommendations_list.addItem(
            ...
            def remove_selected(gui_self):
                pass

            def update_selected(gui_self):
                pass

            def load_configuration(gui_self):
                pass

            def save_configuration(gui_self):
                pass

            def test_configuration(gui_self):
                pass

            def apply_configuration(gui_self):
                pass

            def switch_generation(gui_self):
                pass

            def delete_old_generations(gui_self):
                pass

            def show_generation_diff(gui_self):
                pass

            def run_garbage_collection(gui_self):
                pass

            def optimize_store(gui_self):
                pass
    ...
    def create_web_gui(self):
        ...
        st.set_page_config(page_title="Luminous Nix", page_icon="🌟", layout="wide")
        ...
        page = st.sidebar.radio(
            "", ["Packages", "Configuration", "Health", "AI Assistant"]
        )
    ...
    def web_packages_page(self):
        ...
        with col1:
            search = st.text_input(
                "Search packages", placeholder="Enter package name..."
            )
        with col2:
            if st.button("🔍 Search"):
        ...
        for i, cat in enumerate(
            ["Browsers", "Editors", "Development", "Media", "Security", "System"]
        ):
            with categories[i]:
                if st.button(cat):
