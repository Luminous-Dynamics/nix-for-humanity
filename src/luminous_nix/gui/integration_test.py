...
from component_synthesis_engine import ComponentRequirements, ComponentSynthesizer
...
class IntegrationTestSuite:
    ...
    async def test_complete_flow(self):
        ...
        optimized_requirements = self.learning_engine.optimize_for_user(
            context.user_id, {"base": "requirements"}
        )
        print("   ✅ Requirements optimized with preferences")
    ...
    def test_natural_language_parsing(self):
        ...
        for request in test_requests:
            ...
            print(
                f"   Type: {intent.interface_type.value if intent.interface_type else 'None'}"
            )
    ...
    def test_component_synthesis(self):
        ...
        for i, reqs in enumerate(requirement_sets, 1):
            ...
            # Test evolution
            evolved = synthesizer.evolve_component(component.id, {"satisfaction": 0.6})

            print(f"   Evolved ID: {evolved.id}")
            print("   Mutation Applied: ✅")

            results.append(
                {"component": component, "evolved": evolved, "success": True}
            )
...
def run_integration_demo():
    ...
    for request in requests:
        ...
        if container:
            ...
            print("   Rendered: ⚠️ (Textual not available)")
