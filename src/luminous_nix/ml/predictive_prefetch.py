...
class SequencePredictor:
    ...
    def train(self, sequences: list[list[str]], epochs: int = 10):
        ...
        for i in range(min(self.vocab_size, len(self.weights["output"][0]))):
            avg_cooc = (
                sum(cooccurrence[i]) / self.vocab_size if self.vocab_size > 0 else 0
            )
...
class MarkovChainPredictor:
    ...
    def predict(self, context: list[str], top_k: int = 5) -> list[tuple[str, float]]:
        ...
        predictions = [
            (query, count / total) for query, count in next_queries.most_common(top_k)
        ]
...
class PredictivePrefetchEngine:
    ...
    def predict_next(
        self, current_query: str, context: Optional[PredictionContext] = None
        ...
        if not context:
            context = PredictionContext(
                ...
                previous_queries=self.current_session[-5:]
        ...
        if len(self.current_session) > 0:
            ...
            predictions = sorted(combined.items(), key=lambda x: x[1], reverse=True)[:5]
    ...
    def _get_time_based_predictions(
        ...
        if 6 <= hour <= 10:
            predictions.extend([("python3", 0.3), ("git", 0.3), ("vim", 0.2)])

        # Afternoon (14-17): Productivity tools
        elif 14 <= hour <= 17:
            predictions.extend([("firefox", 0.3), ("libreoffice", 0.2), ("slack", 0.2)])

        # Evening (19-23): Entertainment
        elif 19 <= hour <= 23:
            predictions.extend([("spotify", 0.3), ("vlc", 0.2), ("discord", 0.2)])
