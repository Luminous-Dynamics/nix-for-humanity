...
class UsageAnalytics:
    ...
    def _init_database(self):
        ...
        with self.lock:
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_events_query ON usage_events(query)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_events_session ON usage_events(session_id)")
    ...
    def _update_avg_response_time(self, new_time: float):
        ...
        if self.current_session["avg_response_ms"] == 0:
            ...
            self.current_session["avg_response_ms"] = (
                alpha * new_time + 
                (1 - alpha) * self.current_session["avg_response_ms"]
            )
    ...
    def _store_event(self, event: UsageEvent):
        ...
        with self.lock:
            try:
                ...
                """, (
                    event.timestamp, event.event_type, event.query,
                    event.result_count, event.response_time_ms,
                    event.cache_hit, event.source, event.selected_package,
                    event.user_satisfied, event.session_id or self.session_id
                ))
                ...
                if event.selected_package:
                    ...
                    """, (event.selected_package, event.response_time_ms, event.response_time_ms))
    ...
    def get_smart_cache_recommendations(self) -> Dict[str, Any]:
        ...
        if total_accesses > 0:
            ...
            hot_packages = [
                pkg for pkg, count in self.cache_optimization["hot_packages"].most_common()
                if count >= threshold / max(1, len(self.cache_optimization["hot_packages"]))
            ]
        ...
        try:
            with self.lock:
                cursor = self.conn.cursor()
                cursor.execute("PRAGMA busy_timeout=100")  # 100ms timeout for this query
                cursor.execute("""
        ...
        try:
            ...
            recommendations["common_patterns"] = [
                {"pattern": p, "frequency": len(self.cache_optimization["query_patterns"][p])}
                for p in patterns
            ]
        ...
        if self.cache_optimization["predictive_cache"]:
            predictions = []
            for query, next_queries in self.cache_optimization["predictive_cache"].items():
                if next_queries:
    ...
    def get_usage_insights(self) -> Dict[str, Any]:
        ...
        if self.current_session["total_queries"] > 0:
            cache_hit_rate = (
                ...
                self.current_session["total_queries"] * 100
            )
    ...
    def export_analytics(self, format: str = "json") -> str:
        ...
        data["session"]["packages_selected"] = dict(data["session"]["packages_selected"])
    ...
    def _apply_cache_optimizations(self, recommendations: Dict):
        ...
        for prediction in recommendations["predictive_prefetch"][:5]:
            ...
            if prediction["probability"] > 0.5:
                ...
                threading.Thread(
                    target=self.cache.search_hybrid,
                    args=(query,),
                    daemon=True
                ).start()
