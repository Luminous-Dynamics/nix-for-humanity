...
class AIEnabledCLI:
    ...
    def _is_direct_command(self, query: str) -> bool:
        ...
        direct_commands = [
            'nix-env', 'nix-shell', 'nix-build', 'nix-channel',
    ...
    def explain_last_response(self) -> Response:
        ...
        if self.stats['ai_queries'] == 0:
            return Response(
                type=ResponseType.INFO,
                message="No AI queries processed yet."
            )
        ...
        return {
            **self.stats,
            'hrm_percentage': (
                ...
                if self.stats['ai_queries'] > 0 else 0
            ),
            'ai_percentage': (
                self.stats['ai_queries'] / 
                (self.stats['ai_queries'] + self.stats['direct_commands']) * 100
                if (self.stats['ai_queries'] + self.stats['direct_commands']) > 0 else 0
            )
        }
...
def enhanced_cli_main():
    ...
    parser.add_argument(
        '--explain',
        action='store_true',
        help='Explain AI routing decision'
    )
