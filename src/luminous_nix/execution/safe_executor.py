...
class SafeExecutor:
    ...
    def __init__(
        ...
        self.risk_patterns = {
            ...
            RiskLevel.LOW: ["nix profile install", "nix-env -i", "home-manager switch"],
            RiskLevel.SAFE: [
                "nix search",
                "nix-env -q",
                "nixos-option",
    ...
    def confirm_execution(self, command: str, risk_level: RiskLevel) -> bool:
        ...
        if risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]:
            ...
            if current_gen:
                ...
                print("💡 You can rollback with: nixos-rebuild switch --rollback")
    ...
    def execute_sandboxed(self, command: str) -> ExecutionResult:
        ...
        try:
            result = subprocess.run(
                sandbox_command, shell=True, capture_output=True, text=True, timeout=30
            )
    ...
    def execute(
        self, command: str, mode: Optional[ExecutionMode] = None
    ) -> ExecutionResult:
        """
    ...
    def rollback_last(self) -> ExecutionResult:
        ...
        if (
            last_execution.generation_before
            and last_execution.generation_after
            and last_execution.generation_before != last_execution.generation_after
        ):
    ...
    def get_execution_summary(self) -> str:
        ...
        summary_parts = [f"Executed {len(self.history)} commands"]
        ...
        risk_summary = ", ".join(
            f"{level.value}={count}" for level, count in risk_counts.items()
        )
