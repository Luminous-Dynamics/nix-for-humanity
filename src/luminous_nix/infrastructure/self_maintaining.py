...
class SelfHealingEngine:
    ...
    async def diagnose_issue(
        self, metrics: SystemMetrics, component: str
    ) -> HealthIssue | None:
...
class PredictiveOptimizer:
    ...
    def predict_resource_needs(self, horizon_minutes: int = 30) -> dict[str, float]:
        ...
        predictions["disk"] = self.metrics_history[
            -1
        ].disk_percent  # Disk changes slowly
        predictions["response_time"] = np.mean([m.response_time_ms for m in recent])

        # Detect trends
        if len(self.metrics_history) >= 30:
            cpu_trend = self._calculate_trend(
                [m.cpu_percent for m in self.metrics_history[-30:]]
            )
            memory_trend = self._calculate_trend(
                [m.memory_percent for m in self.metrics_history[-30:]]
            )
...
class SelfMaintainingInfrastructure:
    ...
    def get_infrastructure_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "active_issues": len(
                [i for i in self.healing_engine.issues.values() if not i.resolved_at]
            ),
...
async def demo_self_maintaining():
    ...
    infra.test_runner.register_test_suite(
        "basic", [test_basic_functionality, test_performance, test_async_operation]
    )
