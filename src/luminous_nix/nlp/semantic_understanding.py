...
class SemanticUnderstanding:
    ...
    def __init__(self):
        ...
        self.cache_file = (
            Path.home() / ".cache" / "luminous-nix" / "semantic_mappings.json"
        )
    ...
    def _build_modifier_patterns(self) -> list[tuple[re.Pattern, str]]:
        ...
        return [
            ...
            (
                re.compile(r"\b(light|lightweight|minimal|simple|fast)\b", re.I),
                "lightweight",
            ),
            (
                re.compile(r"\b(professional|advanced|powerful|full)\b", re.I),
                "advanced",
            ),
    ...
    def _apply_modifiers(self, category_data: dict, modifiers: list[str]) -> list[str]:
        """Apply modifiers to filter packages"""
        filtered = []
        ...
        for modifier in modifiers:
            ...
            elif modifier == "cli" and "terminal" in category_data.get(
                "subcategories", {}
            ):
    ...
    def suggest_query_improvements(self, query: str) -> list[str]:
        ...
        if len(query_lower.split()) < 2:
            suggestions.append(
                f"Try being more specific: '{query} for terminal' or '{query} with GUI'"
            )
        ...
        if not matched_categories:
            suggestions.append(
                "Try using category terms like: editor, browser, terminal, development"
            )
        ...
        if not has_modifiers:
            suggestions.append(
                "Add modifiers like: lightweight, GUI, terminal, advanced"
            )
