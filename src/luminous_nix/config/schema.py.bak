...
class ConfigSchema:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "ConfigSchema":
        ...
        if "aliases" in data:
            # Handle nested aliases structure from YAML
            aliases_data = data["aliases"]
            if isinstance(aliases_data, dict):
                # Check if it's the nested format (aliases.aliases and aliases.shortcuts)
                if "aliases" in aliases_data or "shortcuts" in aliases_data:
                    config.aliases = CustomAliases(
                        aliases=aliases_data.get("aliases", {}),
                        shortcuts=aliases_data.get("shortcuts", {}),
                    )
                else:
                    # Assume the whole dict is aliases
                    config.aliases = CustomAliases(aliases=aliases_data)
