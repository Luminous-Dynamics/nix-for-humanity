...
def format_severity(severity: Severity) -> str:
    ...
    return click.style(
        severity.value.upper(), fg=colors.get(severity, "white"), bold=True
    )
...
def audit(ctx, deep, json_output):
    ...
    if json_output:
        ...
        total_vulns = (
            ...
            + audit_result.medium_count
            + audit_result.low_count
        )
        ...
        if total_vulns > 0:
            ...
            if audit_result.critical_count > 0:
                click.echo("  • ", nl=False)
                click.secho(
                    f"Critical: {audit_result.critical_count}", fg="red", bold=True
                )
            ...
            if audit_result.medium_count > 0:
                click.echo("  • ", nl=False)
                click.secho(
                    f"Medium: {audit_result.medium_count}", fg="blue", bold=True
                )
...
def check(ctx, package, json_output):
    ...
    if json_output:
        ...
        if vulnerabilities:
            click.secho(
                f"⚠️ {len(vulnerabilities)} vulnerabilities found:",
                fg="yellow",
                bold=True,
            )
...
def updates(ctx, json_output):
    ...
    if json_output:
        ...
        if updates_list:
            click.secho(
                f"⚠️ {len(updates_list)} security updates available:",
                fg="yellow",
                bold=True,
            )
