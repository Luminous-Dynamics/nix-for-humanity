    ...
    try:
        ...
        if json_output:
            result = {
                ...
                "issues": [
                    (issue.value, severity) for issue, severity in analysis.issues
                ],
            ...
            click.echo(
                click.style("📊 System Performance Profile", fg="cyan", bold=True)
            )
            ...
            if analysis.issues:
                ...
                for issue, severity in analysis.issues[:3]:
                    ...
                    click.echo(
                        f"  • {issue.value} ({click.style(f'{severity*100:.0f}%', fg=severity_color)} severity)"
                    )
            ...
            if analysis.quick_wins:
                ...
                for win in analysis.quick_wins[:2]:
                    ...
                    click.echo(
                        f"    Impact: {win['impact']}, Time: {win['time_to_implement']}"
                    )
            ...
            click.echo(
                f"Estimated boot time reduction: {analysis.estimated_boot_reduction:.1f}s"
            )
            click.echo(
                f"Estimated memory savings: {analysis.estimated_memory_savings:.1f}GB"
            )
        ...
        if json_output:
            ...
            if optimizations["optimizations"]:
                ...
                for i, opt in enumerate(optimizations["optimizations"], 1):
                    ...
                    click.echo(
                        f"   Difficulty: {_format_difficulty(opt['difficulty'])}"
                    )

            if "estimated_final_boot_time" in optimizations:
                reduction = (
                    optimizations["current_boot_time"]
                    - optimizations["estimated_final_boot_time"]
                )
                click.echo(
                    f"\n✨ Estimated final boot time: {optimizations['estimated_final_boot_time']:.1f}s"
                )
                click.echo(
                    f"   Potential reduction: {reduction:.1f}s ({reduction/optimizations['current_boot_time']*100:.0f}%)"
                )
...
def rebuild(ctx, json_output):
    ...
    try:
        ...
        if json_output:
            ...
            click.echo(
                f"Current rebuild time: {optimizations['current_rebuild_time']:.0f}s"
            )
...
def resources(ctx, json_output):
    ...
    try:
        ...
        if json_output:
            ...
            click.echo("\n🧠 Memory:")
            click.echo(
                f"  Usage: {memory['current_usage_gb']:.1f}GB ({click.style(memory['status'], fg=memory_color)})"
            )
            ...
            click.echo("\n⚡ CPU:")
            click.echo(
                f"  Usage: {cpu['average_usage']:.0f}% ({click.style(cpu['status'], fg=cpu_color)})"
            )
...
def _format_difficulty(difficulty: str) -> str:
    ...
    colors = {"easy": "green", "moderate": "yellow", "complex": "red", "high": "red"}
