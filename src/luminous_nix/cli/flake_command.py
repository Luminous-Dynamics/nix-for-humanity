...
def analyze(ctx, config_path, json_output):
    ...
    try:
        ...
        if json_output:
            ...
            click.echo(click.style("🔍 Configuration Analysis", fg="cyan", bold=True))
            click.echo(f"Type: {analysis.config_type}")
            click.echo(
                f"Complexity: {_format_complexity(analysis.migration_complexity)}"
            )
            click.echo(f"Estimated effort: {analysis.estimated_effort_hours} hours")

            if (
                analysis.has_overlays
                or analysis.has_home_manager
    ...
    try:
        ...
        if dry_run:
            ...
            if json_output:
                ...
                if result["success"]:
                    click.echo(
                        click.style("✅ Migration successful!", fg="green", bold=True)
                    )
...
def improve(ctx, flake_path, json_output):
    ...
    try:
        ...
        if json_output:
            ...
            if suggestions:
                click.echo(
                    click.style("💡 Improvement suggestions:", fg="cyan", bold=True)
                )
