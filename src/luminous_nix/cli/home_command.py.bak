...
from nix_for_humanity.core.home_manager import HomeConfig, HomeManager
...
@click.option(
    "--shell", type=click.Choice(["bash", "zsh", "fish"]), help="Shell preference"
)
@click.option(
    "--theme",
    type=click.Choice(["dracula", "nord", "solarized-dark", "gruvbox"]),
...
@click.argument(
    "dotfile_type",
    type=click.Choice(
        ["vim", "neovim", "tmux", "git", "bash", "zsh", "fish", "alacritty", "kitty"]
    ),
)
@click.option(
    "--source",
...
@click.option(
    "--backup/--no-backup", default=True, help="Backup existing config before adding"
)
def add(dotfile_type: str, source: str | None, backup: bool):
    ...
    click.echo(
        click.style(f"✨ Adding {dotfile_type} configuration", fg="cyan", bold=True)
    )
    ...
    click.echo(click.style("\n💡 Tip:", fg="green"))
    click.echo("  Run 'ask-nix home apply' to update your home.nix")


@home.command()
@click.argument(
    "theme_name", type=click.Choice(["dracula", "nord", "solarized-dark", "gruvbox"])
)
@click.option(
    "--apps",
    "-a",
...
def theme(theme_name: str, apps: list[str], preview: bool):
    ...
    if not result["success"]:
        click.echo(
            click.style(f"❌ {result.get('error', 'Failed to apply theme')}", fg="red")
        )
...
def sync(source: str, target: str, configs: builtins.list[str]):
