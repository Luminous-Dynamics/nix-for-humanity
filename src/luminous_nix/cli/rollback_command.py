...
def analyze(ctx, symptoms, json_output):
    ...
    if json_output:
        ...
        click.echo(
            f"Recommended Rollback: Generation {analysis.recommended_generation}"
        )
...
def check(ctx, generation, json_output):
    ...
    if json_output:
        ...
        click.secho(
            f"🔍 Checking Generation {generation} Safety...", fg="cyan", bold=True
        )
        click.echo()
        ...
        if safe:
            click.secho("Safe to Rollback: ✅ Yes", fg="green")
        else:
            click.secho("Safe to Rollback: ❌ No", fg="red")
...
def find_working(ctx, component, json_output):
    ...
    if json_output:
        ...
        click.secho(
            f"🔎 Finding Last Working Generation for '{component}'...",
            fg="cyan",
            bold=True,
        )
        click.echo()
        ...
        if generation is not None:
            ...
            click.secho("⚠️ No previous working generation found", fg="yellow")
