...
def analyze(config_path: str, output_json: bool):
    ...
    try:
        ...
        if output_json:
            ...
            click.echo(
                click.style("🧬 Configuration DNA Analysis", fg="cyan", bold=True)
            )
            click.echo()
...
def compare(config1: str, config2: str, output_json: bool):
    ...
    try:
        ...
        if output_json:
            ...
            click.echo(
                click.style("🧬 Configuration DNA Comparison", fg="cyan", bold=True)
            )
            click.echo()
            ...
            if comparison["common_genes"]:
                click.echo(
                    click.style(
                        f"🤝 Common Genes ({len(comparison['common_genes'])}):",
                        fg="green",
                    )
                )
            ...
            if comparison["unique_to_first"]:
                ...
                click.echo(
                    click.style(
                        f"1️⃣ Unique to First ({len(comparison['unique_to_first'])}):",
                        fg="yellow",
                    )
                )
            ...
            if comparison["unique_to_second"]:
                ...
                click.echo(
                    click.style(
                        f"2️⃣ Unique to Second ({len(comparison['unique_to_second'])}):",
                        fg="yellow",
                    )
                )
...
@click.option(
    "--type",
    "viz_type",
    type=click.Choice(["helix", "chromosome", "fingerprint", "health", "full"]),
    default="helix",
...
@click.option(
    "--format",
    type=click.Choice(["json", "yaml", "nix", "compressed"]),
    default="json",
    help="Export format",
)
...
def export(config_path: str, format: str, output: Optional[str], include_raw: bool):
    ...
    try:
        ...
        dna_export = manager.export_dna(
            config_path, format=format_map[format], include_raw=include_raw
        )
...
@click.option(
    "--preserve-custom", is_flag=True, default=True, help="Keep custom settings"
)
def import_dna(
    dna_file: str, validate: bool, apply_to: Optional[str], preserve_custom: bool
):
...
@click.option(
    "--strategy",
    type=click.Choice(["best_of_both", "hybrid_vigor", "selective", "random_mix"]),
...
def breed(
    parent1: str, parent2: str, strategy: str, output: Optional[str], visualize: bool
):
