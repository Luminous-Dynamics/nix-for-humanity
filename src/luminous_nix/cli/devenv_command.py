...
def analyze(ctx, project_path, json_output):
    ...
    try:
        ...
        if json_output:
            ...
            click.echo(click.style("🔍 Project Analysis", fg="cyan", bold=True))
        ...
        if json_output:
            ...
            if output:
                ...
                output_path = Path(project_path) / (
                    "flake.nix" if flake else "shell.nix"
                )
            ...
            click.echo(
                click.style(f"✅ Generated {output_path.name}!", fg="green", bold=True)
            )
            ...
            if docker and spec.docker_compose:
                ...
                if docker_path.exists():
                    if click.confirm("docker-compose.yml exists. Overwrite?"):
                        ...
                        click.echo(
                            click.style("✅ Generated docker-compose.yml!", fg="green")
                        )
                else:
                    with open(docker_path, "w") as f:
                        f.write(spec.docker_compose)
...
def create(ctx, stack, output, json_output):
    ...
    try:
        ...
        if json_output:
            click.echo(
                json.dumps(
                    {"shell_nix": spec.shell_nix, "aliases": spec.aliases}, indent=2
                )
            )
        else:
            if output:
                ...
                click.echo(
                    click.style(f"🔧 shell.nix for {stack}:", fg="cyan", bold=True)
                )
                click.echo("─" * 50)
