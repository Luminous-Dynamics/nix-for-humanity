...
@click.option(
    "--filter", type=str, help="Filter by change type (package/service/kernel)"
)
    ...
    try:
        ...
        if format == "json":
            ...
            for event in events:
                ...
                changes_str = (
                    str(event.details.get("changes", 0)) if event.details else "0"
                )
        ...
        if format != "json":
            console.print()
            console.print(
                f"[dim]Showing {len(events)} most recent configurations[/dim]"
            )
            ...
            if current:
                console.print(
                    f"[green]Current generation: {current.generation}[/green]"
                )
        ...
        if snapshot.changes_from_previous:
            ...
            for change in snapshot.changes_from_previous:
                ...
                if details and change.details:
                    if change.added:
                        console.print(
                            f"    [green]Added: {', '.join(change.added[:5])}[/green]"
                        )
                    if change.removed:
                        console.print(
                            f"    [red]Removed: {', '.join(change.removed[:5])}[/red]"
                        )
...
def diff(gen1: int, gen2: int, unified: bool, summary: bool):
    ...
    try:
        if summary:
            ...
            console.print(
                f"Time difference: {time_diff.days} days, {time_diff.seconds // 3600} hours"
            )
...
@click.option(
    "--strategy",
    type=click.Choice(["full", "selective", "merge", "cherry_pick"]),
    default="full",
    help="Restoration strategy",
)
...
def find_similar(generation: int, threshold: float, limit: int):
    ...
    try:
        ...
        if not similar:
            console.print(
                f"[yellow]No similar configurations found (threshold: {threshold:.0%})[/yellow]"
            )
            return
    ...
    try:
        ...
        if output_json:
            ...
            if patterns.get("total_generations"):
                console.print(
                    f"[bold]Total Generations:[/bold] {patterns['total_generations']}"
                )
            ...
            if patterns.get("change_frequency") is not None:
                console.print(
                    f"[bold]Average Changes/Week:[/bold] {patterns['change_frequency']:.1f}"
                )
            ...
            if patterns.get("stability_periods"):
                ...
                for period in patterns["stability_periods"][:3]:
                    console.print(
                        f"  • {period['duration_days']} days ({period['start'].strftime('%Y-%m-%d')} - {period['end'].strftime('%Y-%m-%d')})"
                    )
