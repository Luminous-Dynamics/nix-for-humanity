...
def generate(description, output, preview):
    ...
    console.print(f"[bold blue]🔧 Generating configuration for:[/bold blue] {natural_language}")
    
    # Use System Orchestrator for configuration generation
    orchestrator = SystemOrchestrator()
    
    # Generate configuration using AST components if available
    if orchestrator.has_capability('ast_config_generation'):
        config_content = orchestrator.generate_config_ast(natural_language)
        intent = orchestrator.get_last_intent()
    else:
    ...
    if conflicts:
        ...
        intent["modules"] = [m for m in intent["modules"] if m == choice or m not in [c for conflict in conflicts for c in conflict]]
    ...
    if not preview:
        ...
        if success:
            ...
            console.print(f"  sudo nixos-rebuild switch")
        else:
...
def explain(config_file):
    ...
    console.print(Panel(explanation, title="Configuration Explanation", border_style="green"))
...
def diff(old_config, new_config):
    ...
    console.print(f"[bold blue]📊 Comparing configurations:[/bold blue]")
    ...
    if diff_output:
        ...
        console.print(Panel(syntax, title="Configuration Differences", border_style="yellow"))
    else:
...
def wizard():
    ...
    # Server configuration
        ...
        if Confirm.ask("\nInclude web server?", default=True):
            web = Prompt.ask("Which web server?", choices=["nginx", "apache"], default="nginx")
            intent["modules"].append(f"web.{web}")
        
        if Confirm.ask("\nInclude database?", default=False):
            db = Prompt.ask("Which database?", choices=["postgresql", "mysql"], default="postgresql")
    ...
    if Confirm.ask("\nSave this configuration?", default=True):
        ...
        if success:
            ...
            console.print(f"2. Test: sudo nixos-rebuild test -I nixos-config={save_path}")
            console.print(f"3. Apply: sudo cp {save_path} /etc/nixos/configuration.nix && sudo nixos-rebuild switch")
        else:
            console.print(f"\n[bold red]❌ {message}[/bold red]")
