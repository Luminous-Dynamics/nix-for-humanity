...
from ..core.intelligent_system import LuminousNixIntelligence, IntelligentResponse
...
class LuminousNixAPI:
    ...
    def search(self, query: str, limit: int = 10) -> APIResponse:
        ...
        try:
            ...
            response = self.intelligence.intelligent_search(query, use_all_features=True)
            ...
            metadata = {
                ...
                'suggested_packages': response.intent.suggested_packages if response.intent else [],
        ...
        try:
            if permanent:
                command = {
                    ...
                    'steps': [
                        f"1. Edit /etc/nixos/configuration.nix",
                        f"2. Add '{package}' to environment.systemPackages",
                        f"3. Run: sudo nixos-rebuild switch"
                    ],
                ...
                command = {
                    'method': 'temporary',
                    'steps': [f"Run the following command:"],
            ...
            return APIResponse(
                success=True,
                data=command,
                message=f"Install command for {package}"
            )
        ...
        try:
            ...
            formatted_insights = {
                'session': {
                    ...
                    'average_response_ms': insights['session'].get('avg_response_ms', 0),
                    'cache_hit_rate': insights['session'].get('cache_hit_rate', 0)
                },
                'performance': {
                    'queue_size': insights['session']['queue_stats']['queue_size'],
                    'writes_completed': insights['session']['queue_stats']['writes_completed'],
                    'writes_failed': insights['session']['queue_stats']['writes_failed']
                },
                'recommendations': {
                    'hot_packages': insights['recommendations']['packages_to_cache'][:5],
                    'common_patterns': insights['recommendations']['common_patterns'][:5]
                }
            }
            ...
            return APIResponse(
                ...
                message="Current system insights"
            )
    ...
    def get_popular_packages(self, limit: int = 10) -> APIResponse:
        ...
        """
        try:
            recommendations = self.intelligence.analytics.get_smart_cache_recommendations(limit)
        ...
        try:
            ...
            for package in packages:
                if self.intelligence.update_monitor.has_update(package):
                    update_info = self.intelligence.update_monitor.get_update_info(package)
                    if update_info:
                        updates.append({
                            'package': package,
    ...
    def health_check(self) -> APIResponse:
        ...
        try:
            ...
            try:
                stats = self.intelligence.collaborative.get_stats()
                health['network'] = 'healthy' if stats['status'] == 'operational' else 'degraded'
            except:
