...
try:
    ...
    logger.info("FAISS not available. Using NumPy for similarity search (slower but works)")
...
class SemanticCache:
    ...
    def __init__(self, encoder=None, cache_dir: Optional[Path] = None, 
                 max_cache_size: int = 10000, similarity_threshold: float = 0.9):
        """
        ...
        self.cache_dir = cache_dir or Path.home() / ".cache" / "luminous-nix" / "semantic"
    ...
    def _load_cache(self):
        ...
        for row in rows:
            query_hash, query, embedding_blob, intent, confidence, response_json, timestamp, access_count = row
    ...
    def store(self, query: str, intent: str, confidence: float, 
              response: Dict[str, Any]) -> CachedResult:
        """
    ...
    def _evict_lru(self):
        ...
        sorted_results = sorted(
            ...
            key=lambda r: (r.access_count, r.timestamp)
        )
...
def test_semantic_cache():
    ...
    print(f"  Query: 'add firefox browser'")
    print(f"  Hit: {result is not None}")
    if result:
    ...
    print(f"  Query: 'find text editor'")
    print(f"  Hit: {result is not None}")
    if result:
    ...
    print(f"  1000 exact lookups: {exact_time*1000:.1f}ms ({exact_time/1000*1000:.3f}ms per lookup)")
    ...
    print(f"  100 semantic lookups: {semantic_time*1000:.1f}ms ({semantic_time/100*1000:.1f}ms per lookup)")
