...
class GemmaEncoder:
    ...
    def __init__(self, dimension: int = DEFAULT_DIMENSION, use_gpu: bool = False):
        ...
        self.use_gpu = use_gpu and torch.cuda.is_available() if GEMMA_AVAILABLE else False
        self.model = None
        self.device = None
    ...
    def encode_query(self, text: str, dimension: Optional[int] = None) -> Optional[np.ndarray]:
        """
    ...
    def encode_documents(self, texts: List[str], dimension: Optional[int] = None) -> Optional[np.ndarray]:
        """
...
class SemanticIntentRecognizer:
    ...
    def _extract_entities(self, query: str, intent: str) -> Dict[str, str]:
        ...
        if intent in ["install", "search", "remove", "info"]:
            ...
            stopwords = {"install", "search", "remove", "find", "package", 
                        "for", "the", "a", "an", "on", "nixos", "system",
                        "info", "about", "information", "tell", "me"}
