...
class SacredTrinityDB:
        ...
        def write_worker():
            ...
            while not self.stop_writer.is_set():
                try:
                    ...
                    should_flush = (
                        len(batch) >= 100
                        or (time.time() - last_flush > 1 and batch)
                        or (self.write_queue.empty() and batch)
                    )
    ...
    def _load_to_memory(self):
        ...
        with self.cache_lock:
            try:
                ...
                # Load analytics
                cursor.execute(
                    "SELECT metric_name, metric_value, metric_data FROM analytics"
                )
                ...
                self.memory_cache = {"recent_events": [], "kv": {}, "analytics": {}}
    ...
    def track_event(self, event_type: str, data: Any, session_id: Optional[str] = None):
        ...
        with self.cache_lock:
            ...
            self.memory_cache["recent_events"] = self.memory_cache["recent_events"][
                :100
            ]
    ...
    def set_value(self, key: str, value: Any):
        ...
        self.write_queue.put({"type": "kv", "data": (key, json.dumps(value))})
    ...
    def update_metric(self, metric_name: str, value: float, data: Optional[Any] = None):
        ...
        with self.cache_lock:
            ...
            self.memory_cache["analytics"][metric_name] = {"value": value, "data": data}
...
class AnalyticsDB(SacredTrinityDB):
    ...
    def increment_package_stat(self, package: str, stat_type: str = "search"):
        ...
        # Queue the update
        self.write_queue.put({"type": "custom", "sql": sql, "params": (package,)})
...
class CacheDB(SacredTrinityDB):
    ...
    def set_with_ttl(self, key: str, value: Any, ttl_seconds: int):
        ...
        with self.cache_lock:
            ...
            self.memory_cache["ttl_cache"][key] = {"value": value, "expiry": expiry}
