...
from ..ai.advanced_features.ml_health_predictor import HealthMetric, MLHealthPredictor
    ...
    def create_layout(self) -> Layout:
        ...
        if self.state.current_view == DashboardView.OVERVIEW:
            ...
            layout["left"].split_column(
                Layout(name="system_info"), Layout(name="mode_status")
            )

            layout["center"].split_column(
                Layout(name="health_overview"), Layout(name="predictions")
            )

            layout["right"].split_column(
                Layout(name="dna_mini"), Layout(name="storage_mini")
            )

        elif self.state.current_view == DashboardView.HEALTH:
            layout["body"].split_row(
                Layout(name="metrics", ratio=2), Layout(name="predictions", ratio=1)
            )

            layout["metrics"].split_column(
                Layout(name="health_charts"), Layout(name="health_table")
            )

        elif self.state.current_view == DashboardView.DNA:
            layout["body"].split_row(
                Layout(name="dna_visual", ratio=2), Layout(name="dna_details", ratio=1)
            )
...
[dim]View: {view_name} | Time: {current_time} | Refresh: {self.state.refresh_rate}s[/dim]
        ...
        layout["header"].update(
            Panel(Align.center(header_text.strip()), border_style="cyan")
        )
...
GPU: {profile.gpu_profile}
        ...
        return Panel(
            content.strip(), title="[bold]Active Mode[/bold]", border_style="green"
        )
    ...
    def update_health_overview(self) -> Panel:
        ...
        health_display.append("")
        health_display.append(
            f"[bold]Risk Score: [{risk_color}]{profile.risk_score:.1f}/100[/{risk_color}][/bold]"
        )
    ...
    def update_predictions(self) -> Panel:
        ...
        return Panel(
            content, title="[bold]ML Predictions[/bold]", border_style="yellow"
        )
    ...
    def update_storage_mini(self) -> Panel:
        ...
        if analysis.optimization_potential > 1e9:
            content_lines.append(
                f"[yellow]Can free: {analysis.optimization_potential / 1e9:.1f} GB[/yellow]"
            )

        return Panel(
            "\n".join(content_lines), title="[bold]Storage[/bold]", border_style="blue"
        )
    ...
    def _update_health_view(self, layout: Layout):
        ...
        if self.state.selected_metric:
            ...
            layout["metrics"]["health_charts"].update(
                Panel(chart, title="Metric History")
            )
        else:
            layout["metrics"]["health_charts"].update(
    ...
    def _update_dna_view(self, layout: Layout):
        ...
        try:
            ...
            layout["dna_visual"].update(
                Panel(
                    helix,
                    title="[bold]Configuration DNA[/bold]",
...
[bold]Dominant Genes:[/bold]
    ...
    def _update_storage_view(self, layout: Layout):
        ...
        for category, size in analysis.by_category.items():
            percentage = (
                (size / analysis.total_used) * 100 if analysis.total_used > 0 else 0
            )
            storage_table.add_row(
                category, f"{size / 1e9:.2f} GB", f"{percentage:.1f}%"
            )
    ...
    def _update_rollback_view(self, layout: Layout):
        ...
        for gen in analysis.generations[:10]:
            ...
            node = timeline.add(
                f"{icon} Gen {gen.generation} - {gen.date}", style=style
            )
        ...
        layout["body"]["main_content"].update(Columns([timeline, rec_panel]))
    ...
    def _update_ai_status_view(self, layout: Layout):
        ...
        try:
            ...
            ai_status.append(
                ("[yellow]⚠️[/yellow]", "Ollama", "Error", "Check installation")
            )
        ...
        ai_status.append(
            (
                ...
                "POML",
                "Active",
                f"{len(self._get_poml_templates())} templates",
            )
        )
...
Net Recv:   {net_io.bytes_recv / 1e9:.2f} GB
    ...
    def _collect_metrics(self):
        ...
        self.ml_predictor.collect_metric(
            HealthMetric.CPU_USAGE, psutil.cpu_percent(interval=0.1)
        )

        self.ml_predictor.collect_metric(
            HealthMetric.MEMORY_USAGE, psutil.virtual_memory().percent
        )

        self.ml_predictor.collect_metric(
            HealthMetric.DISK_USAGE, psutil.disk_usage("/").percent
        )
