...
class ThreeTierCache:
    ...
    def __init__(self, db_path: str = "cache/hrm_cache.db",
                 memory_size: int = 100,
    ...
    def _populate_patterns(self):
        ...
        patterns = [
            # Install patterns
            (r'(install|add|get)\s+(\w+)', 'nix-env -iA nixpkgs.{package}', 'install', 10),
            (r'how\s+(do\s+i|to)\s+install\s+(\w+)', 'nix-env -iA nixpkgs.{package}', 'install', 9),
            
            # Search patterns
            (r'(search|find|look\s+for)\s+(\w+)', 'nix search nixpkgs {package}', 'search', 8),
            (r'what\s+(\w+)\s+are\s+available', 'nix search nixpkgs {package}', 'search', 7),
            
            # Configuration patterns
            (r'(configure|setup|enable)\s+(\w+)', 'services.{service}.enable = true;', 'configure', 8),
            
            # Error patterns
            (r'error.*collision', 'Use priority or overlays to resolve conflicts', 'error', 9),
            (r'attribute.*not\s+found', 'Check package name with: nix search', 'error', 8),
            
            # Update patterns
            (r'(update|upgrade)\s+system', 'sudo nixos-rebuild switch --upgrade', 'update', 10),
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            'total_hit_rate': (self.stats['l1_hits'] + self.stats['l2_hits'] + 
                              self.stats['l3_hits']) / total,
