...
class UpdateMonitor:
    ...
    def __init__(self, check_interval: int = 300):
        ...
        self.package_versions: Dict[str, Dict[str, str]] = {}  # channel -> package -> version
        ...
        # Persistence
        self.state_path = Path.home() / ".cache" / "luminous-nix" / "update_monitor.json"
    ...
    def _save_state(self):
        ...
        try:
            state = {
                'channels': [
                    asdict(ch) for ch in self.channels.values()
                ],
                'package_versions': self.package_versions,
                'watched_packages': list(self.watched_packages),
                'update_history': [
                    asdict(u) for u in list(self.update_history)[-100:]
                ]
            }
            ...
            while not self.stop_monitoring.is_set():
                try:
                    ...
                    for channel_name, channel in list(self.channels.items()):
                        ...
                        if time.time() - channel.last_checked > channel.update_frequency:
    ...
    def _get_channel_revision(self, channel: ChannelInfo) -> Optional[str]:
        ...
        if hours_passed > 0:
            return hashlib.md5(f"{channel.name}{hours_passed}".encode()).hexdigest()[:12]
    ...
    def _find_package_updates(
        ...
        for pkg_name, new_version in simulated_updates.items():
            ...
            if old_version != new_version:
                ...
                if (pkg_name in self.watched_packages or
                    self._is_security_update(pkg_name, new_version)):
                    ...
                    update = PackageUpdate(
                        ...
                        timestamp=time.time(),
                        severity=self._determine_severity(pkg_name, old_version, new_version)
                    )
    ...
    def _is_security_update(self, package: str, version: str) -> bool:
        ...
        security_packages = [
            "kernel", "linux", "systemd", "openssl", "openssh",
    ...
    def add_channel(self, name: str, url: str, check_frequency: int = 3600):
        ...
        channel = ChannelInfo(
            ...
            url=url,
            last_checked=0,
            update_frequency=check_frequency
        )
    ...
    def get_statistics(self) -> Dict:
        ...
        with self.update_lock:
            ...
            now = time.time()
            recent_24h = sum(
                1 for u in self.update_history
...
class SmartUpdateNotifier:
    ...
    def _on_update(self, update: PackageUpdate):
        ...
        if update.severity == "security" and self.notify_security:
            should_notify = True
        elif update.package_name in self.monitor.watched_packages and self.notify_watched:
            should_notify = True
                ...
                try:
                    ...
                    if self.notification_queue:
                        if self.batch_notifications:
                            ...
                            if urgent or time.time() - self.last_notification > self.batch_interval:
    ...
    def _send_notifications(self):
        ...
        if "important" in updates_by_severity:
            ...
            messages.append(
                f"⚠️ {len(important_updates)} important updates available"
            )
        
        if "normal" in updates_by_severity:
            normal_updates = updates_by_severity["normal"]
            messages.append(
                f"📦 {len(normal_updates)} package updates available"
            )
    ...
    def search_with_updates(self, query: str) -> Tuple[List[Dict], List[PackageUpdate]]:
        ...
        for result in results:
            ...
            if pkg_name:
                ...
                pkg_updates = self.monitor.get_update_history(
                    package=pkg_name,
                    limit=1
                )
                if pkg_updates:
