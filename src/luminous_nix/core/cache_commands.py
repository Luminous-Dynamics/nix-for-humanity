...
class CacheCommands:
    ...
    def _handle_cache_status(self) -> Response:
        ...
        if stats.get("total_searches", 0) > 0:
            ...
            message += (
                f"  • Fuzzy matches: {stats['fuzzy_matches']} ({fuzzy_rate:.1f}%)\n"
            )
        ...
        return Response(success=True, text=message.strip(), data=info)
    ...
    def _handle_cache_warm(self) -> Response:
        ...
        message += (
            f"  • Total cached: {len(self.cache.cache.get('packages', {}))} packages\n"
        )
    ...
    def _handle_cache_test(self) -> Response:
        ...
        return Response(success=True, text=message.strip(), data={"results": results})
...
Common packages are always cached for immediate results.
        ...
        return Response(success=True, text=message.strip())
