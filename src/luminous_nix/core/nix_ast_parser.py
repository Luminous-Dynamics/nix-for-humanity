...
class NixASTParser:
    ...
    def _initialize_parser(self, grammar_path: Path | None = None):
        ...
        try:
            ...
            if grammar_path and grammar_path.exists():
                ...
                Language.build_library(
                    str(grammar_path.parent / "build" / "nix.so"), [str(grammar_path)]
                )
                self.language = Language(
                    str(grammar_path.parent / "build" / "nix.so"), "nix"
                )
            else:
    ...
    def parse(self, code: str) -> ASTNode | None:
        """
    ...
    def parse_file(self, file_path: str | Path) -> ASTNode | None:
        """
    ...
    def find_nodes_by_type(
        self, ast: ASTNode, node_type: str | NixNodeType
    ) -> list[ASTNode]:
        """
    ...
    def validate_syntax(self, code: str) -> tuple[bool, list[dict[str, Any]]]:
        ...
        if not self.parser:
            return False, [
                {"message": "Parser not initialized", "line": 0, "column": 0}
            ]
    ...
    def find_attribute(self, ast: ASTNode, attribute_path: str) -> ASTNode | None:
        """
    ...
    def modify_attribute(
        self, ast: ASTNode, attribute_path: str, new_value: str
    ) -> str:
        """
    ...
    def pretty_print(self, ast: ASTNode, indent: int = 0) -> str:
        ...
        # Print current node
        lines.append(
            f"{indent_str}{ast.type}: {repr(ast.text[:50] if len(ast.text) > 50 else ast.text)}"
        )
    ...
    def get_error_location(self, ast: ASTNode, error_line: int) -> ASTNode | None:
        """
        ...
        def find_at_line(node: ASTNode) -> ASTNode | None:
...
def get_parser() -> NixASTParser | None:
