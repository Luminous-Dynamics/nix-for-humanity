...
class GrandmaResponse:
    ...
    technical: str | None = None  # Hidden technical details
...
class GrandmaMode:
    ...
    def install_program(
        self, program_name: str, confirm: bool = False
    ) -> GrandmaResponse:
        """
    ...
    def search_programs(self, search_term: str) -> GrandmaResponse:
        ...
        try:
            ...
            for pkg, description in self.commonly_used_programs.items():
                if (
                    search_term.lower() in pkg.lower()
            ...
            if friendly_results:
                return GrandmaResponse(
                    ...
                    suggestions=[
                        "To install one, just tell me: 'install [program name]'"
                    ],
                )
            ...
            if result.returncode == 0 and result.stdout:
                ...
                for prog in programs[:30]:  # Limit to first 30 to not overwhelm
                    ...
                    if any(
                        ...
                        for x in ["firefox", "chrome", "browser", "email", "thunder"]
                    ):
                        categories["Internet"].append(clean_name)
                    elif any(
                        x in clean_name.lower()
                        for x in ["office", "writer", "calc", "word"]
                    ):
                        categories["Office"].append(clean_name)
                    elif any(
                        x in clean_name.lower()
                        for x in ["video", "audio", "music", "player", "vlc"]
                    ):
                        categories["Media"].append(clean_name)
                    elif any(
                        x in clean_name.lower()
                        for x in ["git", "vim", "emacs", "compiler"]
                    ):
                ...
                return GrandmaResponse(
                    ...
                    message=message,
                    suggestions=[
                        "To remove any program, just tell me: 'remove [program name]'"
                    ],
                )
            return GrandmaResponse(
                success=True,
                ...
                suggestions=[
                    "Try installing Firefox for web browsing: 'install firefox'"
                ],
            )
    ...
    def _suggest_by_use_case(self, search: str) -> list[str]:
        ...
        return [
            "Try describing what you want to do, like 'browse internet' or 'edit photos'"
        ]
