...
class SecurityAuditor:
    ...
    def _load_trusted_commands(self) -> Set[str]:
        ...
        return {
            'nix', 'nix-env', 'nix-channel', 'nix-store', 'nix-build',
            'nixos-rebuild', 'nixos-version', 'nix-shell', 'nix-instantiate',
            'nix-collect-garbage', 'nix-hash', 'nix-prefetch-url',
            'home-manager', 'systemctl', 'journalctl', 'ip', 'ping',
            'df', 'free', 'ps', 'which', 'whereis', 'ls', 'cat',
            'echo', 'true', 'false', 'date', 'whoami', 'hostname'
        }
    ...
    def audit_file_operation(self, path: str, operation: str) -> AuditResult:
        ...
        if normalized_path:
            ...
            sensitive_paths = [
                ...
                '/root', '/home/*/.ssh', '/var/log/auth.log'
            ]
    ...
    def audit_configuration(self, config_data: Dict[str, Any], format: str = "yaml") -> AuditResult:
        ...
        secret_patterns = [
            ...
            (r'(private_key|privatekey)\s*[:=]\s*["\']?[^"\'\s]+', "Exposed private key"),
        ]
    ...
    def _detect_suspicious_behavior(self, input_str: str) -> bool:
        ...
        suspicious_indicators = [
            ...
            re.compile(r'(rm|delete|remove).*\b(fail|error|denied)\b.*\1', re.IGNORECASE),
    ...
    def _calculate_security_score(self, violations: List[SecurityViolation], 
    ...
    def _generate_recommendations(self, violations: List[SecurityViolation], 
        ...
        if AuditCategory.COMMAND_INJECTION in categories:
            recommendations.append("Avoid shell execution; use direct API calls instead")
            
        if AuditCategory.PATH_TRAVERSAL in categories:
            recommendations.append("Use absolute paths and validate all file operations")
        ...
        if warnings:
            recommendations.append("Address security warnings to improve overall posture")
