...
class FlakeManager:
    ...
    async def init(self, path: Optional[Path] = None) -> NativeOperationResult:
        """Initialize a new flake in the current directory"""
        return await self.ops.execute_flake_operation(
            FlakeOperationType.INIT_FLAKE,
            flake_path=path
        )
        
    async def update(self, path: Optional[Path] = None) -> NativeOperationResult:
        """Update all flake inputs to latest versions"""
        return await self.ops.execute_flake_operation(
            FlakeOperationType.UPDATE_FLAKE,
            flake_path=path
        )
        
    async def show(self, path: Optional[Path] = None) -> NativeOperationResult:
        """Show flake outputs and structure"""
        return await self.ops.execute_flake_operation(
            FlakeOperationType.SHOW_FLAKE,
            flake_path=path
        )
        
    async def check(self, path: Optional[Path] = None) -> NativeOperationResult:
        """Validate flake configuration"""
        return await self.ops.execute_flake_operation(
            FlakeOperationType.CHECK_FLAKE,
            flake_path=path
        )
        
    async def build(self, path: Optional[Path] = None) -> NativeOperationResult:
        """Build the flake configuration"""
        return await self.ops.execute_flake_operation(
            FlakeOperationType.BUILD_FLAKE,
            flake_path=path
        )
        
    async def edit(self, path: Optional[Path] = None, editor: Optional[str] = None) -> NativeOperationResult:
...
class ProfileManager:
    ...
    async def switch(self, profile_name: str) -> NativeOperationResult:
        ...
        return await self.ops.execute_profile_operation(
            ProfileOperationType.SWITCH_PROFILE,
    ...
    async def create(self, profile_name: str) -> NativeOperationResult:
        ...
        return await self.ops.execute_profile_operation(
            ProfileOperationType.CREATE_PROFILE,
            profile_name=profile_name
        )
        
    async def backup(self, profile_name: Optional[str] = None) -> NativeOperationResult:
        """Backup current or specified profile"""
        return await self.ops.execute_profile_operation(
            ProfileOperationType.BACKUP_PROFILE,
            profile_name=profile_name
        )
...
class RemoteBuilder:
    ...
    async def build(self, host: str, user: Optional[str] = None) -> NativeOperationResult:
    ...
    async def deploy(self, host: str, user: Optional[str] = None) -> NativeOperationResult:
    ...
    async def copy_system(self, host: str, system_path: Optional[str] = None) -> NativeOperationResult:
    ...
    async def check_builders(self) -> NativeOperationResult:
        ...
        return await self.ops.execute_remote_operation(
            RemoteOperationType.CHECK_REMOTE
        )
...
class ImageBuilder:
    ...
    async def container(self, container_type: str = "docker") -> NativeOperationResult:
        ...
        return await self.ops.execute_image_operation(
            ImageOperationType.BUILD_CONTAINER,
            options={'type': container_type}
        )
        
    async def sd_card(self, board: Optional[str] = None) -> NativeOperationResult:
        """Build SD card image for embedded devices"""
        options = {'board': board} if board else {}
        return await self.ops.execute_image_operation(
            ImageOperationType.BUILD_SD_IMAGE,
            options=options
        )
...
class ReplLauncher:
    ...
    async def launch(self, flake: bool = False, path: Optional[Path] = None) -> NativeOperationResult:
    ...
    async def launch_with_context(self, context: Dict[str, Any]) -> NativeOperationResult:
