...
def search_packages(query: tuple, limit: int, persona: Optional[str]):
    ...
    table = Table(
        title=f"Package Matches for '{query_str}'",
        box=box.ROUNDED,
        show_lines=True
    )
...
@click.option("--persona", "-p", help="Use specific persona style")
def package_info(package: str, persona: Optional[str]):
...
[cyan]Platforms:[/cyan] {', '.join(info.platforms)}"""
    ...
    # Persona response
    response = persona_mgr.format_response(
        f"Package information for {package}",
        {"action": "info", "package": package}
    )
...
@click.option("--persona", "-p", help="Use specific persona style")
def find_by_command(command: str, persona: Optional[str]):
    ...
    if suggestions:
        console.print(f"💡 Quick install the most likely match:")
...
@click.option("--persona", "-p", help="Use specific persona style")
def browse_packages(category: Optional[str], persona: Optional[str]):
    ...
    if category:
        ...
        console.print("   [green]ask-nix discover browse --category development[/green]")
...
@click.option("--persona", "-p", help="Use specific persona style")
def popular_packages(category: Optional[str], persona: Optional[str]):
    ...
    if not popular:
        console.print(f"[yellow]No popular packages found{' in ' + category if category else ''}[/yellow]")
        return
