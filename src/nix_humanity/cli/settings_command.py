...
from nix_humanity.config.schema import Personality, ResponseFormat, NLPEngine
...
def show(format, show_defaults):
    ...
    if show_defaults:
        ...
        console.print(f"[dim]Configuration from: {manager.config_path or 'defaults'}[/dim]")
    ...
    if config_str:
        ...
        console.print(Panel(syntax, title=f"Configuration ({format})", border_style="blue"))
    else:
...
def wizard(profile):
    ...
    config.ui.show_commands = Confirm.ask(
        "Show underlying Nix commands?",
        default=config.ui.show_commands
    )
    
    config.ui.use_colors = Confirm.ask(
        "Use colored output?",
        default=config.ui.use_colors
    )
    ...
    config.performance.fast_mode = Confirm.ask(
        "Enable fast mode (speed over accuracy)?",
        default=config.performance.fast_mode
    )
    ...
    config.privacy.local_only = Confirm.ask(
        ...
        default=config.privacy.local_only
    )
    ...
    config.learning.enabled = Confirm.ask(
        "Enable learning from your interactions?",
        default=config.learning.enabled
    )
    ...
    config.voice.enabled = Confirm.ask(
        ...
        default=config.voice.enabled
    )
    
    if config.voice.enabled:
        config.voice.wake_word = Prompt.ask(
            "Wake word",
            default=config.voice.wake_word
        )
...
def profiles():
    ...
    if built_in:
        table = Table(title="Built-in Profiles", show_header=True, header_style="bold magenta")
        ...
        for name, profile in built_in:
            ...
            if "ui" in overrides:
                if "default_personality" in overrides["ui"]:
                    features.append(f"Personality: {overrides['ui']['default_personality']}")
    ...
    if custom:
        table = Table(title="Custom Profiles", show_header=True, header_style="bold magenta")
        table.add_column("Profile", style="cyan", no_wrap=True)
        table.add_column("Description", style="white")
...
def delete_profile(name):
    ...
    if manager.profile_manager.delete_profile(name):
        ...
        console.print(f"[red]❌ Failed to delete profile (may be built-in)[/red]")
...
def add_alias(alias, command):
    ...
    if manager.add_alias(alias, command):
        ...
        console.print(f"[red]❌ Failed to add alias[/red]")
...
def import_config(file, format):
    ...
    try:
        ...
        if manager.import_config(content, format):
            ...
            console.print(f"[red]❌ Failed to import configuration[/red]")
