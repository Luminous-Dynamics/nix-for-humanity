...
class AdaptiveComplexityManager:
    ...
    def update_mastery(self, user_id: str, command_success: bool, 
        ...
        if command_success:
            ...
            confidence_penalty = 0.01 if mastery.stage == ComplexityStage.SANCTUARY else 0.03
            mastery.confidence_score = max(0.0, mastery.confidence_score - confidence_penalty)
            
        # Update error rate (rolling average of last 20 commands)
        total_commands = mastery.successful_commands + 1  # Approximate
        mastery.error_rate = ((mastery.error_rate * (total_commands - 1)) + 
                             (0 if command_success else 1)) / total_commands
    ...
    def _check_stage_progression(self, mastery: UserMastery) -> ComplexityStage:
        ...
        if mastery.stage == ComplexityStage.SANCTUARY:
            ...
            if (mastery.successful_commands >= self.SANCTUARY_TO_GYMNASIUM['min_commands'] and
                mastery.error_rate <= self.SANCTUARY_TO_GYMNASIUM['max_error_rate'] and
                mastery.session_count >= self.SANCTUARY_TO_GYMNASIUM['min_sessions'] and
                mastery.confidence_score >= self.SANCTUARY_TO_GYMNASIUM['min_confidence']):
                return ComplexityStage.GYMNASIUM
                
        elif mastery.stage == ComplexityStage.GYMNASIUM:
            # Check progression to Open Sky
            if (mastery.successful_commands >= self.GYMNASIUM_TO_OPEN_SKY['min_commands'] and
                mastery.error_rate <= self.GYMNASIUM_TO_OPEN_SKY['max_error_rate'] and
                mastery.session_count >= self.GYMNASIUM_TO_OPEN_SKY['min_sessions'] and
                mastery.confidence_score >= self.GYMNASIUM_TO_OPEN_SKY['min_confidence'] and
                mastery.time_in_stage >= self.GYMNASIUM_TO_OPEN_SKY['min_time_in_stage']):
                return ComplexityStage.OPEN_SKY
                
            # Check regression to Sanctuary (high error rate)
            elif mastery.error_rate > 0.5 and mastery.confidence_score < 0.3:
    ...
    def get_stage_description(self, stage: ComplexityStage) -> str:
        ...
        descriptions = {
            ComplexityStage.SANCTUARY: 
                "🛡️ Sanctuary Mode - Protected and guided experience for comfortable learning",
            ComplexityStage.GYMNASIUM:
                "🎯 Gymnasium Mode - Exploring features and building mastery",
            ComplexityStage.OPEN_SKY:
                "☁️ Open Sky Mode - Invisible excellence, the interface disappears"
        }
        ...
        progress = {
            ...
            'progress_metrics': {
                'commands': {
                    ...
                    'percentage': min(100, (mastery.successful_commands / thresholds['min_commands']) * 100)
                },
                'accuracy': {
                    'current': 1 - mastery.error_rate,
                    'required': 1 - thresholds['max_error_rate'],
                    'percentage': min(100, ((1 - mastery.error_rate) / (1 - thresholds['max_error_rate'])) * 100)
                },
                'confidence': {
                    'current': mastery.confidence_score,
                    'required': thresholds['min_confidence'],
                    'percentage': min(100, (mastery.confidence_score / thresholds['min_confidence']) * 100)
                }
            },
