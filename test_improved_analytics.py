    ...
    def frequent_reader():
        ...
        for i in range(50):
            ...
            try:
                ...
                if i % 10 == 0:
                    print(f"  Read {i}: {elapsed:.1f}ms (found {len(recommendations['packages_to_cache'])} recommendations)")
...
def test_optimizer_doesnt_lock():
    ...
    for i in range(100):
        event = UsageEvent(
            timestamp=time.time(),
            event_type="search",
            query=f"firefox" if i % 10 == 0 else f"query_{i}",
            result_count=5,
    ...
    print(f"  Time since last optimization: {status['time_since_last']:.1f}s" if status['time_since_last'] else "  No optimization yet")
