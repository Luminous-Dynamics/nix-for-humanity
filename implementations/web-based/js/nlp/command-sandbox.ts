...
export class CommandSandbox {
  ...
  async execute(
    ...
    try {
      ...
      const result = await this.executeWithRestrictions(
        command,
        args,
        sandboxDir,
        opts,
  ...
  ): Promise<Omit<SandboxResult, 'executionTime' | 'sanitizedCommand'>> {
   ...
   */
  private buildSandboxEnvironment(options: Required<SandboxOptions>): Record<string, string> {
   ...
   */
  private sanitizeCommand(command: string, args: string[]): string {
   ...
   */
  validateCommand(command: string, args: string[]): { safe: boolean; reason?: string } {
