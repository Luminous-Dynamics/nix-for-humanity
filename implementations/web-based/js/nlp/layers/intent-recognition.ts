...
interface PatternMatcher {
  patterns: RegExp[];
  type: string;
  extractEntities: (match: RegExpMatchArray) => Entity[];
}

// Pattern definitions for intent recognition
const INTENT_PATTERNS: PatternMatcher[] = [
  // Search patterns - MUST come before install patterns for proper matching
  {
    type: 'search',
    patterns: [
      /^(?:search|find|look for|search for)\s+(.+)$/i,
      /^(?:can you|please|could you)\s+(?:search|find|look for)\s+(.+)$/i,
      /^(?:show me|find me)\s+(?:packages? for\s+)?(.+)$/i,
      /^what (?:packages?|programs?|software) (?:are available )?for\s+(.+)$/i
    ],
 ...
 * Pure function
 */
function extractServiceAction(input: string): string {
  if (/\b(?:is|check|status)\b/i.test(input)) return 'status';
  if (/\bstart|turn on|launch\b/i.test(input)) return 'start';
  if (/\bstop|turn off|kill\b/i.test(input)) return 'stop';
  if (/\brestart\b/i.test(input)) return 'restart';
  if (/\benable\b/i.test(input)) return 'enable';
  if (/\bdisable\b/i.test(input)) return 'disable';
 ...
 * Pure function
 */
function hasTypos(input: string): boolean {
 ...
 * Pure function
 */
export function suggestIntent(input: string): string | null {
