...
export class AmbiguityResolver {
  ...
  resolve(intents: Intent[]): AmbiguityResolution | null {
    ...
    if (topIntent.confidence - secondIntent.confidence > 0.3) return null;
   ...
   */
  private generateClarification(intents: Intent[]): AmbiguityResolution {
   ...
   */
  private clarifyIntentType(intents: Intent[]): AmbiguityResolution {
    const options: ClarificationOption[] = intents.map(intent => {
   ...
   */
  private clarifyEntities(intents: Intent[]): AmbiguityResolution {
   ...
   */
  private clarifyPackages(intents: Intent[]): AmbiguityResolution {
    const options: ClarificationOption[] = [];
    
    // Extract unique packages
    const packages = new Map<string, Intent>();
   ...
   */
  private clarifyProblems(intents: Intent[]): AmbiguityResolution {
    const options: ClarificationOption[] = [];
    
    // Extract unique problems
    const problems = new Map<string, Intent>();
    for (const intent of intents) {
   ...
   */
  private getIntentLabel(intent: Intent): string {
   ...
   */
  private getIntentExample(intent: Intent): string {
