...
export class NixWebSocketClient {
  ...
  connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      if (typeof window !== 'undefined' && (window as any).io) {
        this.socket = (window as any).io(this.serverUrl);
  ...
  private setupEventHandlers() {
    ...
    // Execution error
    this.socket.on('execution-error', (data: any) => {
      const execution = this.executions.get(data.sessionId);
      if (execution) {
    ...
    // Warnings
    this.socket.on('execution-warning', (data: any) => {
   ...
   */
  async executeCommand(command: any): Promise<CommandExecution> {
   ...
   * Request rollback
   */
  async rollback(generation: number): Promise<boolean> {
    if (this.connected && this.socket) {
      return new Promise((resolve) => {
        const handler = (data: any) => {
   ...
   */
  private simulateExecution(execution: CommandExecution) {
    setTimeout(() => {
      execution.status = 'running';
