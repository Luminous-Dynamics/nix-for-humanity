...
def test_update_monitoring():
    ...
    for update in updates[:5]:
        print(f"   • {update.package_name}: {update.old_version} → {update.new_version}")
...
def test_smart_notifications():
    ...
    for severity, updates in by_severity.items():
        emoji = "🔒" if severity == "security" else "⚠️" if severity == "important" else "📦"
    ...
    for query in test_queries:
        ...
        if updates:
            for update in updates[:2]:
                print(f"      • {update.package_name}: {update.new_version} ({update.severity})")
...
def test_concurrent_updates():
    ...
    for i in range(5):
        thread = threading.Thread(
            target=lambda: monitor.check_for_updates(force=True)
        )
