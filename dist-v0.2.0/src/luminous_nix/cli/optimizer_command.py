...
@click.option(
    "--level",
    type=click.Choice(["safe", "balanced", "aggressive", "custom"]),
    default="balanced",
            ...
            if opt_type:
                ...
                if filtered:
                    ...
                    for i, suggestion in enumerate(filtered[:10], 1):
                        ...
                        console.print(
                            f"   Impact: {suggestion.rule.impact} | Risk: {suggestion.rule.risk}"
                        )
                        console.print()
                else:
                    console.print(
                ...
                if plan.suggestions:
                    ...
                    for i, suggestion in enumerate(plan.suggestions[:5], 1):
                        ...
                        console.print(
                            f"   Impact: {suggestion.rule.impact} | Risk: [{risk_color}]{suggestion.rule.risk}[/{risk_color}]"
                        )
...
@click.option(
    "--selections", multiple=True, type=int, help="Specific suggestion numbers to apply"
)
@click.option(
    "--risk",
    type=click.Choice(["low", "medium", "high"]),
    default="low",
    help="Maximum risk level to apply",
)
@click.option(
    "--backup/--no-backup", default=True, help="Create backup before applying"
)
@click.option("--dry-run", is_flag=True, help="Preview changes without applying")
@click.option("--force", is_flag=True, help="Skip confirmation")
def apply(
    config: str, selections: tuple, risk: str, backup: bool, dry_run: bool, force: bool
):
        ...
        if output_json:
            ...
            if "boot_time" in benchmarks:
                ...
                status = (
                    "[green]Good[/green]"
                    if boot_time < 30
                    else "[yellow]Could improve[/yellow]"
                    if boot_time < 60
                    else "[red]Slow[/red]"
                )
            ...
            if "package_count" in benchmarks:
                ...
                status = (
                    "[green]Minimal[/green]"
                    if pkg_count < 300
                    else "[yellow]Normal[/yellow]"
                    if pkg_count < 600
                    else "[red]Bloated[/red]"
                )
...
def compare(config1: str, config2: str, verbose: bool):
    ...
    try:
        ...
        winner = (
            ...
            if len(plan2.suggestions) < len(plan1.suggestions)
            else "Tie"
        )
        table.add_row(
            "Optimization Needed",
        ...
        if len(plan1.suggestions) < len(plan2.suggestions):
            ...
            console.print(
                "[yellow]Both configurations have similar optimization levels[/yellow]"
            )
