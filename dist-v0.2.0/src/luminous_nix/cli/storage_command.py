...
def analyze(ctx, aggressive, json_output):
    ...
    if json_output:
        ...
        if analysis.old_generations:
            click.echo()
            click.secho(f"Old Generations ({len(analysis.old_generations)}):", bold=True)
...
def cleanup(ctx, aggressive, dry_run, yes):
    ...
    if not dry_run and not yes:
        response = click.confirm(
            click.style("Proceed with cleanup?", fg='yellow'),
            default=False
        )
        if not response:
    ...
    for cmd in analysis.cleanup_commands:
        if cmd.startswith("#"):
            ...
            if dry_run:
                ...
                if result != 0:
                    click.secho(f"⚠️ Command failed with exit code {result}", fg='yellow')
...
def optimize(ctx, target_gb, json_output):
    ...
    if not json_output:
        click.secho(f"💾 Optimizing to Free {format_size(target_gb)}...", fg='cyan', bold=True)
    ...
    if json_output:
        ...
        if plan.get('steps'):
            ...
            for step in plan['steps']:
                ...
                click.echo(f"  • {step['action']}: {format_size(step['space_gb'])} (", nl=False)
...
def find_large(ctx, min_size, json_output):
    ...
    if not json_output:
        click.secho(f"🔍 Finding Packages Larger Than {min_size}MB...", fg='cyan', bold=True)
