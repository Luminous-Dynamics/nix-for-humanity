...
class SacredConfirmationDialog:
    ...
    def ask(
        ...
        try:
            ...
            return self._show_gui_dialog(
                title, message, command, details, allow_remember
            )
        except Exception:
    ...
    def _show_gui_dialog(
        self, title: str, message: str, command: str, details: str, allow_remember: bool
    ) -> ConfirmationResult:
        ...
        message_label = ttk.Label(
            main_frame, text=message, wraplength=400, font=("Arial", 12)
        )
        ...
        if command:
            cmd_frame = ttk.LabelFrame(
                main_frame, text="Command to execute:", padding="10"
            )
            cmd_frame.grid(
                row=1, column=0, columnspan=2, pady=(0, 10), sticky=(tk.W, tk.E)
            )
        ...
        cancel_btn = ttk.Button(
            button_frame, text="Cancel (Esc)", command=self._on_cancel, width=15
        )
        cancel_btn.grid(row=0, column=0, padx=(0, 10))

        # Confirm button
        confirm_btn = ttk.Button(
            button_frame, text="Confirm (Enter)", command=self._on_confirm, width=15
        )
    ...
    def _show_terminal_prompt(
        self, title: str, message: str, command: str, details: str
    ) -> ConfirmationResult:
        """Fallback to terminal prompt if GUI unavailable"""
        print(f"\n{'=' * 60}")
        print(f"⚠️  {title}")
    ...
    def _on_cancel(self):
        """Handle cancellation"""
        self.result = ConfirmationResult(confirmed=False, remember_choice=False)
        if self.root:
            self.root.quit()
            self.root.destroy()
...
if __name__ == "__main__":
    # Test the dialog
    result = confirm.install_package("firefox", "nix profile install nixpkgs#firefox")
