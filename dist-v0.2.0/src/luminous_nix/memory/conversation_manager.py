...
class ConversationMemory:
    ...
    def __init__(self, 
        ...
        self.storage_path = storage_path or Path.home() / ".cache" / "luminous-nix" / "conversations"
    ...
    def get_conversation_summary(self) -> str:
        ...
        summary_parts = [
            f"Session {self.session_id} - {len(self.history)} turns"
        ]
        ...
        try:
            ...
            if time.time() - data.get('timestamp', 0) < 86400:
                ...
                if time.time() - data.get('timestamp', 0) < 3600:  # Within 1 hour
                    ...
                    self.history = [ConversationTurn.from_dict(turn) for turn in history_data]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'session_id': self.session_id,
            'success_rate': sum(1 for t in self.history if t.success) / len(self.history) 
...
class ContextEnhancer:
    ...
    def personalize_response(self, response: str, query: str) -> str:
        ...
        if context['user_patterns']['common_tasks']:
            top_task = max(context['user_patterns']['common_tasks'].items(), 
                          key=lambda x: x[1])[0]
...
if __name__ == "__main__":
    ...
    for query in queries:
        ...
        if context['relevant_history']:
            print(f"🔍 Found relevant history: {context['relevant_history'][0]['query'][:30]}...")
