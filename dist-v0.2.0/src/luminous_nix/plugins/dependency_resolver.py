...
class DependencyGraph:
    ...
    def get_all_dependencies(
        self, plugin_id: str, include_optional: bool = False
    ) -> set[str]:
...
class DependencyResolver:
    ...
    def __init__(self, plugin_directory: Path | None = None):
        ...
        self.plugin_directory = (
            plugin_directory or Path.home() / ".luminous-nix" / "plugins"
        )
        self.manifests_cache: dict[str, dict] = {}

    def load_manifest(self, plugin_id: str) -> dict | None:
    ...
    def build_graph(self, plugin_ids: list[str]) -> DependencyGraph:
        ...
        for plugin_id in plugin_ids:
            ...
            if manifest:
                ...
                # Parse dependencies
                if (
                    "relationships" in manifest
                    and "dependencies" in manifest["relationships"]
                ):
                    for dep_data in manifest["relationships"]["dependencies"]:
                        dep = PluginDependency(
                            ...
                            relationship_type=RelationshipType(
                                dep_data.get("relationship_type", "requires")
                            ),
    ...
    def check_compatibility(
        self, plugin_id: str, installed_plugins: list[str]
    ) -> tuple[bool, list[str]]:
        """
        ...
        if "relationships" in manifest and "conflicts" in manifest["relationships"]:
            for conflict in manifest["relationships"]["conflicts"]:
                if conflict["plugin_id"] in installed_plugins:
                    conflicts.append(
                        f"{conflict['plugin_id']}: {conflict.get('reason', 'Unknown conflict')}"
                    )
