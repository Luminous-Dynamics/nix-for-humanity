...
class PermissionManager:
    ...
    def __init__(self, manifest: dict):
        ...
        self.governing_principle = manifest.get("consciousness", {}).get(
            "governing_principle"
        )
        self.sacred_promise = manifest.get("consciousness", {}).get(
            "sacred_promise", ""
        )
    ...
    def can_perform(self, action: str, permission: Permission) -> tuple[bool, str]:
        ...
        if self._is_forbidden(action):
            return (
                False,
                f"Action '{action}' is explicitly forbidden by plugin boundaries",
            )
        ...
        if permission not in self.granted_permissions:
            return (
                False,
                f"Plugin lacks permission '{permission.value}' for action '{action}'",
            )
        ...
        if not self._is_aligned_with_principle(permission):
            return (
                False,
                f"Permission '{permission.value}' conflicts with governing principle '{self.governing_principle}'",
            )
    ...
    def _is_aligned_with_principle(self, permission: Permission) -> bool:
        ...
        return self.get_risk_level(permission) not in [
            RiskLevel.HIGH,
            RiskLevel.CRITICAL,
        ]

    def request_permission(
        self, action: str, permission: Permission, context: dict | None = None
    ) -> PermissionRequest:
        """
...
"""
    ...
    def _explain_permission(self, permission: Permission) -> str:
        ...
        """
        explanations = {
            Permission.FILESYSTEM_READ: "📖 Read files on your system (cannot modify them)",
            Permission.FILESYSTEM_WRITE: "✏️ Create or modify files on your system",
            Permission.FILESYSTEM_WATCH: "👁️ Monitor when files change (passive observation)",
            Permission.NETWORK_LOCAL: "🏠 Communicate within your local network only",
            Permission.NETWORK_INTERNET: "🌍 Access the internet (could share data externally)",
            Permission.PROCESS_SPAWN: "🚀 Start new programs or scripts",
            Permission.PROCESS_MONITOR: "📊 See what programs are running (cannot control them)",
            Permission.SYSTEM_NOTIFICATIONS: "🔔 Show you notifications",
            Permission.SYSTEM_INFO: "ℹ️ Read system information (OS version, hardware, etc)",
            Permission.CONFIGURATION_READ: "⚙️ Read your NixOS configuration",
            Permission.CONFIGURATION_WRITE: "⚠️ MODIFY your NixOS configuration (requires rebuild)",
        }
...
"""
        ...
        if self.audit_log:
            ...
            for perm, count in sorted(
                perm_counts.items(), key=lambda x: x[1], reverse=True
            ):
...
if __name__ == "__main__":
    ...
    allowed, reason = pm.can_perform(
        "block notification", Permission.SYSTEM_NOTIFICATIONS
    )
    ...
    allowed, reason = pm.can_perform(
        "share focus data with external services", Permission.NETWORK_INTERNET
    )
