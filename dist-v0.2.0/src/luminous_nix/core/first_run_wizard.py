...
First-Run Wizard for Luminous Nix
...
class FirstRunWizard:
    ...
    def should_run(self) -> bool:
        ...
        config_path = Path.home() / ".config" / "luminous-nix" / "config.yaml"
...
This quick setup will take about 2 minutes.
    ...
    def _detect_system(self) -> SystemInfo:
        ...
        try:
            ...
            info.available_memory_mb = psutil.virtual_memory().available // (
                1024 * 1024
            )
    ...
    def _show_system_info(self) -> None:
        ...
        print(
            f"{'✅' if self.system_info.has_nix_daemon else '❌'} Nix Daemon: "
            f"{'Running' if self.system_info.has_nix_daemon else 'Not running'}"
        )

        print(
            f"{'✅' if self.system_info.home_manager_installed else '❌'} Home Manager: "
            f"{'Installed' if self.system_info.home_manager_installed else 'Not installed'}"
        )

        print(
            f"{'✅' if self.system_info.has_internet else '❌'} Internet: "
            ...
            try:
                ...
                if len(version_parts) >= 2:
                    ...
                    if major < 23 or (major == 23 and minor < 11):
                        issues.append(
                            f"NixOS {major}.{minor} is quite old. Consider upgrading."
                        )
                    elif major > 25:
                        print(
                            f"ℹ️  NixOS {major}.{minor} is newer than tested. Some features may not work."
                        )
            except Exception:
                # TODO: Add proper error handling
                pass  # Silent for now, should log error
    ...
    def _choose_privacy(self) -> str:
        ...
        while True:
            ...
            if choice in ["1", "2", "3", "4"]:
                levels = {"1": "strict", "2": "minimal", "3": "standard", "4": "full"}
                level = levels[choice]
                print(f"\n✅ Privacy level: {level}")
                return level
    ...
    def _check_accessibility(self) -> list[str] | None:
    ...
    def _show_tour(self) -> None:
        ...
        features = [
            ...
            (
                "Smart Suggestions",
                "I'll guide you through complex tasks with helpful suggestions",
            ),
            ("Safety First", "All commands are validated for safety before execution"),
            (
                "Learning System",
                "I learn your preferences to provide better help over time",
            ),
        ...
        print("\n✅ Tour complete! You're ready to start using Luminous Nix.")
    ...
    def get_quick_setup_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "privacy": {"data_collection": "minimal", "local_only": True},
            "learning": {"enabled": True, "privacy_mode": "strict"},
        }


def run_if_needed(config_manager: ConfigManager | None = None) -> bool:
