...
class ConversationState:
    ...
    def get_context_for_query(self, query: str) -> dict[str, Any]:
        ...
        if self.history:
            context["recent_history"] = [
                {"query": turn.query, "intent": turn.intent, "entities": turn.entities}
                for turn in self.history[-3:]
            ]

        # Add working memory
        context["working_memory"] = {
            k: v for k, v in self.working_memory.items() if v is not None
        }
    ...
    def get_clarification_needed(self) -> str | None:
        """
    ...
    def _detect_topic(self, query: str, intent: str | None) -> ConversationTopic:
        ...
        if any(
            word in query_lower for word in ["config", "setting", "option", "configure"]
        ):
            return ConversationTopic.CONFIGURATION
        if any(
            word in query_lower
            for word in ["manage", "system", "generation", "rollback"]
        ):
            return ConversationTopic.SYSTEM_MANAGEMENT
        if any(
            word in query_lower for word in ["how", "what", "why", "explain", "learn"]
        ):
    ...
    def _update_user_profile(self, turn: Turn):
        ...
        if turn.topic == ConversationTopic.LEARNING:
            ...
            if any(
                word in turn.query.lower() for word in ["what is", "how do i", "basic"]
            ):
                self.user_profile.skill_level = "beginner"
            # Complex questions indicate advanced
            elif any(
                word in turn.query.lower()
                for word in ["optimize", "customize", "advanced"]
            ):
    ...
    def _save_session(self):
        ...
        session_data = {
            "session_id": self.session_id,
            "started": self.history[0].timestamp.isoformat()
            if self.history
            else datetime.now().isoformat(),
            "last_updated": datetime.now().isoformat(),
            "turns": [
                turn.to_dict() for turn in self.history[-50:]
            ],  # Keep last 50 turns
    ...
    def _save_user_profile(self):
        ...
        profile_data = {
            ...
            "installed_packages": self.user_profile.installed_packages[
                -100:
            ],  # Keep last 100
