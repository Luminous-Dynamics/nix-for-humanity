...
from .llm_intent_recognizer import SmartIntentRouter
...
class EnhancedIntentPipeline:
    ...
    def __init__(self, context: IntentContext | None = None):
    ...
    def recognize(self, text: str) -> Intent:
        ...
        # Log for analysis
        logger.debug(
            f"Intent recognized in {latency:.3f}s: {intent.type} (confidence: {intent.confidence})"
        )
...
class AdaptiveIntentRecognizer:
    ...
    def _generate_explanation(self, text: str, intent: Intent) -> str:
        ...
        explanation_parts.append(
            f"I understood '{text}' as a request to: {intent.type.value.replace('_', ' ')}"
        )
        ...
        if intent.confidence > 0.9:
            ...
            explanation_parts.append(
                "I'm not entirely certain, but this seems most likely."
            )
    ...
    def get_insights(self) -> dict[str, Any]:
        ...
        insights = {
            "performance": {
                ...
                "pattern_success_rate": metrics["pattern_hits"]
                / max(1, metrics["pattern_hits"] + metrics["llm_assists"]),
                "ai_assistance_rate": metrics["llm_assists"]
                / max(1, metrics["pattern_hits"] + metrics["llm_assists"]),
            },
            "learning": {
            ...
            "capabilities": {
                ...
                "mode": "AI-enhanced"
                if self.pipeline.context.prefer_llm
                else "Pattern-first",
            },
        }
