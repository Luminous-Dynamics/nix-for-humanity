...
from .nix_ast_parser import NixASTParser, get_parser
...
class ASTConfigGenerator:
    ...
    def load_configuration(self, file_path: str | Path) -> bool:
        """
        ...
        logger.info(
            f"   Graph: {len(self.knowledge_graph.nodes)} nodes, "
    ...
    def _generate_package_changes(self, intent: ConfigIntent) -> list[ConfigChange]:
        ...
        changes = []
        package_name = intent.properties.get(
            "name", intent.target.replace("package:", "")
        )

        if intent.action == "add":
            # Check if package already exists
            result = self.graph_interface.query(
                QueryType.FIND_PACKAGES, pattern=package_name
            )

            packages = (
                result.data
                if isinstance(result.data, list)
                else result.data.get("packages", [])
            )
            if result.success and package_name in [
                p.get("name", p) if isinstance(p, dict) else p for p in packages
            ]:
    ...
    def _generate_service_changes(self, intent: ConfigIntent) -> list[ConfigChange]:
        ...
        changes = []
        service_name = intent.properties.get(
            "name", intent.target.replace("service:", "")
        )
        ...
        for change in changes:
            ...
            for dep in deps:
                if dep not in [c.path for c in resolved]:
                    resolved.append(
                        ConfigChange(path=dep, value="true", operation="set")
                    )
