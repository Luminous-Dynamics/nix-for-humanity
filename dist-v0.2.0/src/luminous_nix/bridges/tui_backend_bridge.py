...
class TUIBackendBridge:
    ...
    def __init__(self, readiness: float = 0.3):
        ...
        logger.info(
            f"TUI-Backend Bridge initialized: {self.interaction_mode.value} mode (readiness: {self.readiness:.1%})"
        )
    ...
    def _start_event_processors(self):
        ...
        self.tui_processor = threading.Thread(
            target=self._process_tui_events, daemon=True, name="TUI-Event-Processor"
        )
    ...
    def _handle_tui_event(self, event: BridgeEvent):
        ...
        if not self._can_process_event(event):
            logger.debug(
                f"Event {event.event_type} blocked at readiness {self.readiness:.1%}"
            )
    ...
    def send_from_tui(
        ...
        if self.interaction_mode == InteractionMode.CONFIRM:
            if event_type in [EventType.COMMAND_REQUEST, EventType.CONFIG_CHANGE]:
                ...
                ] = f"Execute: {payload.get('command', 'action')}?"
    ...
    def sync_state(self) -> dict[str, Any]:
        ...
        state = {
            ...
            "can_execute": self.interaction_mode
            in [InteractionMode.CONFIRM, InteractionMode.FULL_CONTROL],
    ...
    def adjust_readiness(self, delta: float):
        ...
        if new_mode != self.interaction_mode:
            logger.info(
                f"Interaction mode advancing: {self.interaction_mode.value} → {new_mode.value}"
            )
    ...
    def progressive_test(self) -> bool:
        ...
        if self.readiness >= 0.2:
            correlation_id = self.send_from_tui(EventType.HEARTBEAT, {"test": "basic"})
        ...
        if self.readiness >= 0.4:
            test_received = []
            self.register_backend_handler(
                EventType.SEARCH_QUERY, lambda e: test_received.append(e)
            )
        ...
        if self.readiness >= 0.6:
            ...
            self.register_backend_handler(
                EventType.USER_INPUT, lambda e: backend_received.append(e)
            )
            self.register_tui_handler(
                EventType.RESULT_READY, lambda e: tui_received.append(e)
            )
