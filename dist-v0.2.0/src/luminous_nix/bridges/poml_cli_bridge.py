...
class POMLtoCLIBridge:
    ...
    SAFE_COMMANDS = {
        ...
        "nix-2-5 secondsiate --eval",  # Evaluate expressions (read-only)
    }
    ...
    def bridge_execution(self, poml_result: dict[str, Any]) -> BridgeResult:
        ...
        if mode == ExecutionMode.SUGGEST:
            ...
            return BridgeResult(
                success=True, mode=mode, command=command, suggestion=suggestion
            )
        ...
        return BridgeResult(
            success=False, mode=mode, command=command, error="Unknown execution mode"
        )
