...
class BayesianHRM:
    ...
    def _detect_out_of_distribution(self, query: str) -> Tuple[bool, float]:
        ...
        if self.mean_embedding is not None:
            ...
            distance = np.linalg.norm(diff) / (np.linalg.norm(self.std_embedding) + 1e-6)
            
            # Convert to probability (scalar)
            ood_score = float(1.0 / (1.0 + np.exp(-distance + 3)))  # Sigmoid with threshold
...
def demonstrate_uncertainty():
    ...
    for query, description in test_queries:
        ...
        should_learn = active_hrm.should_request_feedback(
            query, uncertainty.total
        )
