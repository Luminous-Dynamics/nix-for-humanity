...
class HRMBenchmarkSuite:
    ...
    def benchmark_model(
        self,
        model: Union[NeuralHRM, torch.nn.Module],
        test_data: List[Dict],
        model_name: str = "HRM_Model",
    ) -> Dict[str, Any]:
    ...
    def _evaluate_accuracy(self, model, test_data: List[Dict]) -> Dict[str, float]:
        ...
        for sample in tqdm(test_data, desc="Accuracy evaluation"):
            ...
            if isinstance(model, NeuralHRM):
                ...
                pred_strategy, confidence = self._predict_with_pytorch_model(
                    model, query
                )
        ...
        if valid_indices:
            ...
            precision = precision_score(
                valid_truth, valid_preds, average="macro", zero_division=0
            )
            recall = recall_score(
                valid_truth, valid_preds, average="macro", zero_division=0
            )
    ...
    def _evaluate_performance(self, model, test_data: List[Dict]) -> Dict[str, float]:
        ...
        results = {
            ...
            "throughput_qps": 1000.0 / np.mean(inference_times)
            if inference_times
            else 0.0,
        }
    ...
    def _evaluate_calibration(self, model, test_data: List[Dict]) -> Dict[str, float]:
        ...
        if len(confidences) > 10:  # Need sufficient samples
            ...
            try:
                ...
                brier_score = np.mean(
                    (np.array(confidences) - np.array(accuracies)) ** 2
                )
    ...
    def _predict_with_pytorch_model(
        self, model: torch.nn.Module, query: str
    ) -> Tuple[str, float]:
        ...
        strategy = (
            strategies[strategy_idx] if strategy_idx < len(strategies) else "unknown"
        )
    ...
    def _generate_benchmark_plots(self, model_name: str):
        ...
        fig.suptitle(
            f"HRM Benchmark Results - {model_name}", fontsize=16, fontweight="bold"
        )
        ...
        if model_name in self.results["performance_metrics"]:
            ...
            times = np.random.normal(
                ...
                perf_data.get("std_inference_time_ms", 5),
                100,
            )
            axes[0, 1].hist(
                times, bins=20, color="skyblue", alpha=0.7, edgecolor="black"
            )
            axes[0, 1].axvline(
                perf_data["avg_inference_time_ms"],
                color="red",
                linestyle="--",
                label=f"Avg: {perf_data['avg_inference_time_ms']:.1f}ms",
            )
        ...
        # Calibration plot
        if (
            model_name in self.results["calibration_metrics"]
            and "calibration_curve" in self.results["calibration_metrics"][model_name]
        ):
            cal_data = self.results["calibration_metrics"][model_name][
                "calibration_curve"
            ]
...
"""
        ...
        for model_name in self.results["accuracy_metrics"].keys():
            ...
            time_ms = self.results["performance_metrics"][model_name][
                "avg_inference_time_ms"
            ]
            readiness = self.results["production_readiness"][model_name][
                "readiness_level"
            ]
