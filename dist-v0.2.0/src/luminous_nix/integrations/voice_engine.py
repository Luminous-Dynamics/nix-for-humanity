...
class VoiceEngine:
    ...
    async def speak(self, text: str, output_path: Path | None = None) -> bool:
        """Convert text to speech using Piper or espeak-ng"""
        if self.has_piper:
            return await self._speak_piper(text, output_path)
        if self.has_espeak and self.config.espeak_fallback:
            return await self._speak_espeak(text, output_path)
        logger.error("No TTS engine available")
        return False

    async def _speak_piper(self, text: str, output_path: Path | None = None) -> bool:
