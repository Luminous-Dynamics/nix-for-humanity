    ...
    def _check_llama_cpp(self):
        ...
        try:
            result = subprocess.run(
                ["llama-cpp", "--version"], capture_output=True, text=True
            )
            ...
            logger.warning(
                "llama.cpp not found. Install from nixpkgs or build from source"
            )

    async def download_model(
        self, model_name: str, quantization: str = "Q4_K_M"
    ) -> bool:
    ...
    async def generate(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                "http://127.0.0.1:8080/completion", json=payload
            ) as response:
                if stream:
...
class LlamaCppOrchestrator:
    ...
    async def process(self, task_type: str, prompt: str, **kwargs) -> str:
