...
def extract_nixos_patterns() -> List[Dict]:
    ...
    real_patterns = [
        ...
        ("setup postgresql on nixos", "configure", "services.postgresql.enable = true;"),
        ("configure nginx web server", "configure", "services.nginx.enable = true;"),
        ("enable ssh server", "configure", "services.openssh.enable = true;"),
        ("setup firewall rules", "configure", "networking.firewall.allowedTCPPorts = [ 22 80 443 ];"),
        
        # Error resolution queries
        ("error collision between packages", "error", "Use priority or overlays to resolve"),
        ("attribute not found", "error", "Check package name with: nix search"),
        ("cannot build derivation", "error", "Check build logs: nix log /nix/store/..."),
        ...
        ("rollback to previous generation", "rollback", "sudo nixos-rebuild switch --rollback"),
        ...
        ("create python dev environment", "shell", "nix-shell -p python3 python3Packages.pip"),
        ...
        # Common issues
        ("wifi not working nixos", "hardware", "Check: networking.wireless.enable or networking.networkmanager.enable"),
        ("sound not working", "hardware", "Check: sound.enable = true; hardware.pulseaudio.enable = true;"),
        ("graphics driver nvidia", "hardware", "services.xserver.videoDrivers = [ \"nvidia\" ];"),
        ("touchpad not working", "hardware", "services.xserver.libinput.enable = true;"),
        
        # Flakes (modern NixOS)
        ("how to use flakes", "flakes", "Add: nix.settings.experimental-features = [ \"nix-command\" \"flakes\" ];"),
        ("create nix flake", "flakes", "nix flake init"),
        ("update flake inputs", "flakes", "nix flake update"),
        
        # Home Manager
        ("install home manager", "home-manager", "Follow: https://github.com/nix-community/home-manager"),
        ("configure dotfiles with nix", "home-manager", "Use home-manager for declarative dotfiles"),
        ...
        ("run windows vm", "virtualization", "virtualisation.virtualbox.host.enable = true;"),
    ]
...
def save_training_data(queries: List[Dict], output_path: str = "data/nixos_queries.json"):
    ...
    with open(output_file, 'w') as f:
        json.dump({
            ...
            'metadata': {
                ...
                'sources': {
                    'patterns': len([q for q in unique_queries if q['source'] == 'patterns']),
                    'variations': len([q for q in unique_queries if q['source'] == 'variation']),
                    'synthetic': len([q for q in unique_queries if q['source'] == 'synthetic']),
                    'documentation': len([q for q in unique_queries if q['source'] == 'documentation'])
                },
                'categories': {
                    cat: len([q for q in unique_queries if q.get('category') == cat])
                    for cat in set(q.get('category', 'unknown') for q in unique_queries)
                },
