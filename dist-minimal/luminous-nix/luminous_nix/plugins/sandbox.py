...
class PluginSandbox:
    ...
    def __init__(self, manifest: dict, plugin_module: Any):
        ...
        self.consent_callback: Callable | None = None
    ...
    async def _monitored_execute(
        self, handler: Callable, intent_data: dict, context: dict
    ) -> Any:
        """
        ...
        def sandboxed_open(file, mode="r", *args, **kwargs):
            ...
            if not str(file_path).startswith(str(self.workspace)):
                ...
                allowed, reason = self.permission_manager.can_perform(
                    action, permission
                )
    ...
    def _create_sandboxed_context(self) -> dict[str, Any]:
        ...
        return {
            ...
            "plugin_id": self.plugin_id,
            "permissions": [
                p.value for p in self.permission_manager.granted_permissions
            ],
    ...
    def handle_consent_response(
        self, request: PermissionRequest, decision: ConsentDecision
    ):
        """
...
if __name__ == "__main__":
    ...
    async def test():
        result = await sandbox.execute(
            "start focus session", {"duration": 25, "task": "testing sandbox"}
        )
        print("\nExecution result:")
