...
class ImprovedCache:
        ...
        for key in list(self.persistent_cache.keys()):
            try:
                ...
                if isinstance(entry, dict) and "timestamp" in entry:
                    ...
                    ttl = self.ttl_policies.get(
                        entry.get("type", "search"), timedelta(hours=24)
                    )
    ...
    async def get(self, operation: str, query: str) -> Any | None:
        """
    ...
    async def put(
        self, operation: str, query: str, data: Any, operation_time_ms: float = 0
    ):
        """
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_time_saved_per_hit": f"{self.time_saved_ms/self.hits:.1f}ms"
            if self.hits > 0
            else "0ms",
        }
