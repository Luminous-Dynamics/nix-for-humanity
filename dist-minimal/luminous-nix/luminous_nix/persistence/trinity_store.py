...
class TemporalStore:
    ...
    def get_learning_trajectory(
        self, user_id: str, concept: str | None = None
    ) -> list[dict]:
        ...
        return [dict(zip([d[0] for d in self.conn.description], row, strict=False)) for row in result]
...
class SemanticStore:
    ...
    def find_similar_concepts(self, query: str, n_results: int = 5) -> list[dict]:
        ...
        results = self.concepts.query(query_texts=[query], n_results=n_results)
    ...
    def find_similar_commands(self, query: str, n_results: int = 5) -> list[dict]:
        ...
        results = self.commands.query(query_texts=[query], n_results=n_results)
...
class TrinityStore:
    ...
    def record_learning_moment(
        ...
        if self.semantic:
            self.semantic.store_command_pattern(
                command=command, intent=concept, success=success
            )
    ...
    def _calculate_readiness(
        self, user_id: str, concept: str, prerequisites: list[str]
    ) -> float:
    ...
    def suggest_next_concept(self, user_id: str) -> str | None:
