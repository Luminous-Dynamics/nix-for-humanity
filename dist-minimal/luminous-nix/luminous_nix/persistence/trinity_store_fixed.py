...
try:
    ...
    if "libstdc++" in str(e):
        if not IN_NIX_SHELL:
            logger.debug(
                "DuckDB requires system libraries. Run in nix-shell for full functionality."
            )
    else:
...
try:
    ...
    if "libstdc++" in str(e) or "sqlite3" in str(e):
        if not IN_NIX_SHELL:
            logger.debug(
                "ChromaDB requires system libraries. Run in nix-shell for full functionality."
            )
    else:
...
try:
    ...
    if "libstdc++" in str(e):
        if not IN_NIX_SHELL:
            logger.debug(
                "Kùzu requires system libraries. Run in nix-shell for full functionality."
            )
    else:
...
if not any([DUCKDB_AVAILABLE, CHROMADB_AVAILABLE, KUZU_AVAILABLE]) and not IN_NIX_SHELL:
    ...
    if not os.environ.get("TRINITY_WARNING_SHOWN"):
        logger.info(
            "💡 Tip: Run 'nix-shell' to enable Data Trinity databases for enhanced memory"
        )
...
class InMemoryFallbackStore:
    ...
    def _load_data(self) -> dict[str, Any]:
        ...
        return {"learning_events": [], "concepts": {}, "patterns": [], "memories": []}
    ...
    def search(self, query: str, limit: int = 10) -> list[dict[str, Any]]:
        ...
        for concept, metadata in self.data["concepts"].items():
            if query_lower in concept.lower() or query_lower in str(metadata).lower():
                results.append(
                    {"type": "concept", "data": {"concept": concept, **metadata}}
                )
...
class TrinityStore:
    ...
    def __init__(self, data_dir: Path = None):
        ...
        self.data_dir = (
            data_dir or Path.home() / ".local" / "share" / "luminous-nix" / "trinity"
        )
        self.data_dir.mkdir(parents=True, exist_ok=True)
        ...
        if not self.using_trinity:
            ...
            logger.debug(
                "📝 Using fallback storage (run in nix-shell for full Data Trinity)"
            )
        else:
            self.fallback = None

    def record_learning_moment(
        self,
        user_id: str,
        command: str,
        concept: str,
        ...
        if self.using_trinity:
            ...
            self.fallback.add_concept(
                concept, {"commands": [command], "context": context}
            )
    ...
    def search(self, query: str, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if self.using_trinity:
            ...
            if self.semantic:
                ...
                results.extend(
                    [{"type": "semantic", "data": r} for r in semantic_results]
                )
