...
class UnifiedLearningSystem:
    ...
    def _calculate_confidence(
        self, event: LearningEvent, patterns: list[Pattern]
    ) -> float:
    ...
    def _generate_adaptations(
        self, event: LearningEvent, profile: UserProfile, patterns: list[Pattern]
    ) -> dict[str, Any]:
        ...
        if user_events:
            recent_success = sum(1 for e in user_events[-10:] if e.success) / min(
                10, len(user_events)
            )
        else:
    ...
    def _generate_suggestions(
        self, event: LearningEvent, patterns: list[Pattern], profile: UserProfile
    ) -> list[str]:
        ...
        if not event.success and event.error:
            ...
            for pattern in self.patterns.values():
                if (
                    pattern.pattern_type == "error_resolution"
                    and pattern.trigger in event.error
                ):
        ...
        if profile.common_tasks and event.content not in profile.common_tasks[-3:]:
            suggestions.append(
                f"You often follow this with: {profile.common_tasks[-1]}"
            )
    ...
    async def query_similar(self, query: str, limit: int = 5) -> list[dict[str, Any]]:
        ...
        if self.storage and hasattr(self.storage, "semantic") and self.storage.semantic:
            ...
            try:
                results = self.storage.semantic.find_similar_commands(
    ...
    async def get_user_statistics(self, user_id: str) -> dict[str, Any]:
        ...
        if self.storage:
            try:
                ...
                understanding = self.storage.get_user_understanding(
                    user_id=user_id, query="overall progress"
                )
            except Exception as e:
        ...
        user_patterns = [
            p for p in self.patterns.values() if p.metadata.get("user_id") == user_id
        ]
        ...
        return {
            ...
            "success_rate": sum(p.success_rate for p in user_patterns)
            / len(user_patterns)
            if user_patterns
            else 0.0,
        }
    ...
    async def request_llm_guidance(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        decision = await self.llm_control.request_llm_decision(
            context=context, capability=SystemCapability.LEARNING_STRATEGY
        )
...
_global_learning_system: UnifiedLearningSystem | None = None
