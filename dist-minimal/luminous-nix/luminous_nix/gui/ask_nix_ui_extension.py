...
Examples:
  ...
  ask-nix ui create "dark theme IDE layout with file browser"
    ...
    create_parser.add_argument(
        "request", nargs="+", help="Natural language description"
    )
...
Examples:
  ...
  ask-nix ui refine "remove the sidebar"
    ...
    refine_parser.add_argument(
        "--preview", action="store_true", help="Preview after refining"
    )
...
Examples:
  ...
  ask-nix ui show --list        # List all interfaces
    ...
    show_group.add_argument(
        "--last", action="store_true", default=True, help="Show last interface"
    )
...
Examples:
  ...
  ask-nix ui export --format json --id dashboard_20240125
    ...
    export_parser.add_argument(
        "--format",
        choices=["json", "html", "python", "react"],
        default="json",
        help="Export format",
    )
...
Examples:
  ...
  ask-nix ui delete --old 30    # Delete interfaces older than 30 days
    ...
    delete_group.add_argument(
        "--all", action="store_true", help="Delete all interfaces"
    )
    delete_group.add_argument(
        "--old", type=int, help="Delete interfaces older than N days"
    )
...
Examples:
  ...
  ask-nix ui stats --report     # Generate full report
    ...
    stats_parser.add_argument(
        "--detailed", action="store_true", help="Show detailed stats"
    )
    stats_parser.add_argument(
...
Examples:
  ...
  ask-nix ui feedback --rating 8 --comment "Great but too dark"
    ...
    feedback_parser.add_argument(
        "--rating", type=int, choices=range(1, 11), help="Rating from 1-10"
    )
    ...
    if args.ui_command == "create":
        ...
        if args.report:
            ...
            if "performance" in stats and stats["performance"]:
                ...
                print(
                    f"   Avg generation time: {perf.get('avg_generation_time', 0):.2f}ms"
                )
        ...
        cli.learning.record_feedback(
            ...
            feedback_text=args.comment or " ".join(args.feedback)
            if args.feedback
            else "",
        )
