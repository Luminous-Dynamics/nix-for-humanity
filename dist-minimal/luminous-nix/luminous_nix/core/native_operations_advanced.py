...
class AdvancedNativeOperations:
    ...
    async def _build_flake(
        self, flake_path: Path | None, options: dict[str, Any]
    ) -> NativeOperationResult:
            ...
            if process.returncode == 0:
                ...
                info = {
                    ...
                    "nixosConfigurations": list(
                        flake_data.get("nixosConfigurations", {}).keys()
                    ),
                    "packages": list(
                        flake_data.get("packages", {}).get("x86_64-linux", {}).keys()
                    ),
                    "apps": list(
                        flake_data.get("apps", {}).get("x86_64-linux", {}).keys()
                    ),
                }
...
"""
    ...
    async def _list_profiles(
        self, profile_name: str | None, options: dict[str, Any]
    ) -> NativeOperationResult:
        ...
        try:
            ...
            # Custom profiles
            custom_dir = Path("/nix/var/nix/profiles/per-user") / os.environ.get(
                "USER", "root"
            )
            if custom_dir.exists():
                for profile_path in custom_dir.glob("*"):
                    if profile_path.is_link() and not profile_path.name.endswith(
                        "-link"
                    ):
                        profiles.append(
                            ProfileInfo(
    ...
    async def _switch_profile(
        ...
        try:
            ...
            await self.base_manager.async_api.executor(
                None, lambda: self.nix.set_profile(profile)
            )
    ...
    async def launch_repl(
        self, flake: bool = False, options: dict[str, Any] = None
    ) -> NativeOperationResult:
        ...
        try:
            if flake and self.advanced_available:
                ...
                return NativeOperationResult(
                    ...
                    data={"type": "flake", "command": f"nix repl {flake_path}#"},
                    duration_ms=0,
            ...
            return NativeOperationResult(
                success=True,
                operation="launch-repl",
                data={"type": "standard", "command": "nix repl '<nixpkgs>'"},
                duration_ms=0,
    ...
    async def _build_remote(
        ...
        try:
            ...
            result_path = await self.base_manager.async_api.executor(
                None,
            ...
            return NativeOperationResult(
                ...
                data={"host": host, "result": str(result_path)},
                duration_ms=0,
    ...
    async def execute_image_operation(
        self, operation: ImageOperationType, options: dict[str, Any] = None
    ) -> NativeOperationResult:
    ...
    async def _build_iso(self, options: dict[str, Any]) -> NativeOperationResult:
        ...
        try:
            ...
            build_op = NixOperation(
                type=OperationType.BUILD, options={"attribute": iso_config}
            )
    ...
    async def _backup_profile(
        self, profile_name: str | None, options: dict[str, Any]
    ) -> NativeOperationResult:
        """Backup a profile"""
        profile_name = profile_name or "system"
        backup_name = options.get(
            "backup_name",
            f"{profile_name}-backup-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
        )

        return NativeOperationResult(
            success=True,
            operation="backup-profile",
            data={"profile": profile_name, "backup": backup_name},
            duration_ms=0,
            message="Profile backup instructions ready",
            suggestions=[
    ...
    async def _diff_profiles(
        ...
        return NativeOperationResult(
            ...
            data={"profile1": profile1, "profile2": profile2},
            duration_ms=0,
    ...
    async def _copy_closure(
        ...
        try:
            ...
            await self.base_manager.async_api.executor(
                None, lambda: self.nix.copy_closure(remote, Path(closure))
            )

            return NativeOperationResult(
                success=True,
                operation="copy-closure",
                data={"host": host, "closure": closure},
                duration_ms=0,
    ...
    async def _check_remote(
        self, host: str | None, options: dict[str, Any]
    ) -> NativeOperationResult:
    ...
    async def _build_vm_image(self, options: dict[str, Any]) -> NativeOperationResult:
        ...
        return await self.base_manager.execute_native_operation(
            NativeOperationType.BUILD_VM, options=options
        )
