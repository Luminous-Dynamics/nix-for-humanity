...
class IntentAnalyzer:
        ...
        with sqlite3.connect(self.db.db_path) as conn:
            ...
            for row in cursor:
                confusion_matrix.append(
                    {"recognized": row[0], "correct": row[1], "count": row[2]}
                )
            ...
            for row in cursor:
                most_corrected.append(
                    {"query": row[0], "correct_intent": row[1], "corrections": row[2]}
                )
    ...
    def analyze_performance(self) -> dict[str, any]:
        ...
        sorted_by_overall = sorted(
            metrics, key=lambda m: m.performance_score, reverse=True
        )
        ...
        return {
            ...
            "slowest_patterns": [
                {"pattern": m.pattern, "latency_ms": m.avg_latency_ms}
...
class IntentImprovementDashboard:
    ...
    def get_summary(self) -> dict[str, any]:
        ...
        with sqlite3.connect(self.db.db_path) as conn:
            ...
            query_count = conn.execute("SELECT COUNT(*) FROM query_history").fetchone()[
                0
            ]
