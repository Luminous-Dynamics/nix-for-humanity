...
""",
    ...
    def detect_project_type(self, path: Path) -> str | None:
    ...
    def create_flake(
        self, intent: dict[str, Any], path: Path = Path(".")
    ) -> tuple[bool, str]:
        ...
        if intent["language"]:
            ...
            dev_shell = self.templates["devShell"].format(
                name="default", packages=package_list, shell_hook=shell_hook
            )
            outputs.append(dev_shell)
        ...
        if lang == "python":
            package_list = "\n".join(
                [
                    f"            {pkg}"
                    for pkg in packages
                    if pkg not in ["python3", "pip", "virtualenv"]
                ]
            )
            return self.templates["python_dev"].format(
                python_version="3",
                packages=package_list,
                extra_hook='echo "Run pip install -r requirements.txt to install dependencies"'
        ...
        if lang == "javascript":
            package_list = "\n".join(
                [
                    f"            {pkg}"
                    for pkg in packages
                    if pkg not in ["nodejs", "npm"]
                ]
            )
            return self.templates["node_dev"].format(
                node_version="18",
                packages=package_list,
                extra_hook='echo "Run npm install to install dependencies"'
        ...
        if lang == "rust":
            package_list = "\n".join(
                [
                    f"            {pkg}"
                    for pkg in packages
                    if pkg not in ["rustc", "cargo", "rustfmt"]
                ]
            )
    ...
    def show_flake_info(self, path: Path = Path(".")) -> str:
        ...
        try:
            # Get flake metadata
            result = subprocess.run(
                ["nix", "flake", "metadata"], cwd=path, capture_output=True, text=True
            )
    ...
    def convert_to_flake(self, path: Path = Path(".")) -> tuple[bool, str]:
        ...
        if success:
            return (
                True,
                f"Successfully converted to flake!\n{message}\nOriginal files preserved.",
            )
