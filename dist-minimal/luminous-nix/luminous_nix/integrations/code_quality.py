...
class SemgrepAnalyzer:
    ...
    @staticmethod
    async def analyze(
        path: Path, config: str = "auto"  # auto, python, security
    ) -> list[dict[str, Any]]:
...
class CodeQualityOrchestrator:
    ...
    async def full_quality_check(
        self, path: Path, fix: bool = False, strict: bool = False
    ) -> QualityReport:
    ...
    async def auto_fix(self, path: Path) -> dict[str, Any]:
        ...
        fixes_applied = {"ruff_fixes": 0, "formatting": False, "security_patches": 0}
...
async def github_actions_quality():
    ...
    for error in report.type_errors:
        print(
            f"::warning file={error['file']},line={error['line']}::{error['message']}"
        )

    for issue in report.security_issues:
        level = "error" if issue["severity"] == "HIGH" else "warning"
        print(
            f"::{level} file={issue['file']},line={issue['line']}::{issue['message']}"
        )
