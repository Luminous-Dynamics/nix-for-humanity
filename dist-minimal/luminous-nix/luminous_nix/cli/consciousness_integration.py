...
try:
    ...
    from ..consciousness.signal_collector import (
        IntegrationBridge,
        RealTimeSignalCollector,
    )
    ...
    from luminous_nix.consciousness.signal_collector import (
        IntegrationBridge,
        RealTimeSignalCollector,
    )
...
class ConsciousnessAwareCLI:
    ...
    def __init__(self):
        ...
        self.bridge = IntegrationBridge(
            self.signal_collector, self.consciousness_barometer
        )
    ...
    def get_adaptive_response(self, base_response: str) -> str:
        ...
        if quality == "flow":
            ...
            essential_lines = [
                l for l in lines if not l.startswith("  ") or "✓" in l or "✗" in l
            ]
        ...
        if quality == "learning":
            ...
            response = base_response
            if any(
                word in self.last_command for word in ["install", "search", "configure"]
            ):
                response += (
    ...
    def should_offer_break(self) -> bool:
        ...
        # - High cognitive load
        # - Overwhelmed state
        session_minutes = self.signal_collector.get_consciousness_signals()[
            "session_duration"
        ]
    ...
    def get_consciousness_hint(self) -> str | None:
...
def integrate_with_cli(cli_instance):
    ...
    original_execute = (
        cli_instance.execute if hasattr(cli_instance, "execute") else None
    )
