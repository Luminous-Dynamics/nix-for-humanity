    ...
    def _extract_nixos_options(self, progress, task):
        ...
        try:
            ...
            result = subprocess.run(
                ["nix-2-5 secondsiate", "--eval", "--json", 
                 "-E", "with import <nixpkgs/nixos> {}; builtins.attrNames options"],
                capture_output=True,
                text=True,
                timeout=30
            )
            ...
            if result.returncode == 0:
                ...
                common_options = [
                    "boot.loader", "networking.firewall", "services.nginx",
                    "users.users", "environment.systemPackages", "services.openssh",
                    "system.stateVersion", "time.timeZone", "i18n.defaultLocale"
                ]
            ...
            self.console.print(f"[yellow]Warning: Could not extract options: {e}[/yellow]")
    ...
    def _get_option_documentation(self, option: str) -> Optional[DocumentChunk]:
        ...
        try:
            ...
            if "services" in option:
                ...
                doc.examples.append("environment.systemPackages = with pkgs; [ firefox vim git ];")
...
"""
        ...
        self.console.print(Panel(stats_text, title="[cyan]NixOS Training Corpus[/cyan]"))
