...
class CounterfactualHRM:
    ...
    def _create_counterfactual(
        self, query: str, intervention: str, intervention_type: str
    ) -> Dict[str, Any]:
    ...
    def _analyze_failure(self, query: str, failed_solution: str) -> Dict:
        ...
        if "collision" in failed_solution.lower():
            return {
                ...
                "points": ["Multiple packages provide same file", "No priority set"],
                "type": "dependency_conflict",
            }
        elif "attribute" in failed_solution.lower():
            return {
                "root_cause": "Package not found in current channel",
                "points": ["Package name mismatch", "Channel not updated"],
    ...
    def _generate_alternatives(
        self, query: str, failure_analysis: Dict
    ) -> List[CausalPath]:
    ...
    def _find_pareto_optimal(self, solutions: List[Dict]) -> List[Dict]:
        ...
        for s1 in solutions:
            ...
            for s2 in solutions:
                ...
                if (
                    s2.get("speed", 0) >= s1.get("speed", 0)
                    and s2.get("reliability", 0) >= s1.get("reliability", 0)
                    and s2.get("simplicity", 0) >= s1.get("simplicity", 0)
                    and any(
                        [
                            s2.get("speed", 0) > s1.get("speed", 0),
    ...
    def _generate_solution_for_dimensions(self, query: str, dimensions: Dict) -> Dict:
        ...
        # Thorough + permanent = configuration.nix
            ...
            dimensions["speed"] == "thorough"
            and dimensions["persistence"] == "permanent"
        ):
    ...
    def _classify_intervention(self, intervention: str) -> str:
        ...
        if any(
            word in intervention.lower() for word in ["flake", "channel", "overlay"]
        ):
            return "method_change"
        elif any(
            word in intervention.lower() for word in ["version", "update", "rollback"]
        ):
    ...
    def _compare_worlds(self, query: str, counterfactual: Dict) -> Dict:
        ...
        return {
            ...
            "reliability_change": "+15%"
            if "declarative" in str(counterfactual)
            else "0%",
...
def demonstrate_counterfactual():
    ...
    result = hrm.what_if(
        "install tensorflow", "what if I use a flake instead of channels"
    )
    ...
    for outcome in result["likely_outcomes"]:
        print(
            f"  {outcome['scenario']}: {outcome['result']} (p={outcome['probability']})"
        )
    ...
    result = hrm.why_not(
        "install python packages", "error: collision between python3.8 and python3.9"
    )
    ...
    if result["best_alternative"]:
        ...
        print(
            f"  Success probability: {result['best_alternative']['path'].success_probability:.1%}"
        )
