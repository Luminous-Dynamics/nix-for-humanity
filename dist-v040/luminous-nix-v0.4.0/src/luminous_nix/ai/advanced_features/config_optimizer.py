...
class OptimizationPlan:
    ...
    def filter_by_risk(self, max_risk: str) -> List[OptimizationSuggestion]:
        ...
        return [
            s for s in self.suggestions if risk_levels.get(s.rule.risk, 3) <= max_level
        ]
...
class ConfigOptimizer:
    ...
    def __init__(self):
        ...
        self.hrm = (
            HierarchicalReasoningMachine() if HierarchicalReasoningMachine else None
        )
    ...
    def _create_suggestion(
        self, rule: OptimizationRule, match, config: str
        ...
        try:
            ...
            explanation = self._generate_explanation(
                rule, current_value, suggested_value
            )
    ...
    def _generate_explanation(
        self, rule: OptimizationRule, current: str, suggested: str
    ) -> str:
    ...
    def _estimate_impact(self, rule: OptimizationRule) -> Dict[str, Any]:
        ...
        impact = {"performance": 0, "security": 0, "resources": 0, "maintainability": 0}

        # Map rule type to impact
        if rule.type == OptimizationType.PERFORMANCE:
            impact["performance"] = {"low": 5, "medium": 15, "high": 30}.get(
                rule.impact, 0
            )
        elif rule.type == OptimizationType.SECURITY:
            impact["security"] = {"low": 10, "medium": 25, "high": 50}.get(
                rule.impact, 0
            )
        elif rule.type == OptimizationType.RESOURCES:
            impact["resources"] = {"low": 5, "medium": 10, "high": 20}.get(
                rule.impact, 0
            )
        elif rule.type == OptimizationType.BEST_PRACTICES:
            impact["maintainability"] = {"low": 5, "medium": 10, "high": 15}.get(
                rule.impact, 0
            )
    ...
    def _estimate_improvements(
        ...
        if OptimizationType.PERFORMANCE in type_counts:
            improvements["boot_time"] = min(
                type_counts[OptimizationType.PERFORMANCE] * 5, 30
            )

        if OptimizationType.RESOURCES in type_counts:
            improvements["memory_usage"] = min(
                type_counts[OptimizationType.RESOURCES] * 3, 20
            )

        if OptimizationType.SECURITY in type_counts:
            improvements["security_score"] = min(
                type_counts[OptimizationType.SECURITY] * 10, 50
            )

        return improvements

    def _get_ai_analysis(
        self, config: str, suggestions: List[OptimizationSuggestion]
    ) -> str:
        """Get AI analysis of the configuration"""

        # Use POML template for analysis
        template_path = (
            Path(__file__).parent.parent.parent / "poml/templates/config_optimizer.poml"
        )
            ...
            try:
                ...
                variables = {
                    ...
                    "optimization_types": list(
                        set(s.rule.type.value for s in suggestions)
                    ),
                }

                prompt = self.poml_processor.process_template(
                    str(template_path), variables
                )
    ...
    def apply_optimizations(
        ...
        if selections:
            suggestions_to_apply = [
                plan.suggestions[i] for i in selections if i < len(plan.suggestions)
            ]
        else:
    ...
    def benchmark_configuration(self, config_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Package count
            result = subprocess.run(
                ["nix-env", "-q", "--installed"], capture_output=True, text=True
            )
            if result.returncode == 0:
    ...
    def generate_report(self, plan: OptimizationPlan) -> Panel:
        ...
        for metric, value in plan.estimated_improvement.items():
            improvement_text.append(
                f"  • {metric.replace('_', ' ').title()}: {value:.0f}% improvement"
            )
        ...
        report_content = [
            ...
            if improvement_text
            else "  No significant improvements estimated",
        ]
