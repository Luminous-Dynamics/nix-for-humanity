...
class SimpleIntentRecognizer:
    ...
    def __init__(self):
        ...
        self.patterns = {
            IntentType.INSTALL: ["install", "add", "get", "download", "setup"],
            IntentType.UNINSTALL: ["remove", "uninstall", "delete", "purge"],
            IntentType.SEARCH: ["search", "find", "look for", "query", "what is"],
            IntentType.LIST: ["list", "show", "display", "what's installed"],
            IntentType.UPDATE: ["update", "upgrade", "rebuild", "switch"],
            IntentType.CLEANUP: [
                "clean",
                "garbage",
                "collect",
            ...
            IntentType.CONFIGURE: ["configure", "config", "set", "enable", "disable"],
            IntentType.HELP: ["help", "how to", "what does", "explain"],
            IntentType.FLAKE: [
                "flake",
                "flake.nix",
                "create flake",
    ...
    def recognize(self, query: str) -> Intent:
        ...
        # No pattern matched
        return Intent(
            type=IntentType.UNKNOWN, confidence=0.0, entities={}, raw_query=query
        )
