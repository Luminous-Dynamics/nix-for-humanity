...
class SystemHealthMonitor:
    ...
    def check_disk_space(self) -> HealthCheck:
        ...
        try:
            ...
            if status != HealthStatus.HEALTHY:
                ...
                try:
                    nix_store_size = sum(
                        f.stat().st_size for f in Path('/nix/store').rglob('*') if f.is_file()
                    ) / (1024**3)  # GB
                    
                    if nix_store_size > 50:
                        recommendations.insert(0, f"Nix store is {nix_store_size:.1f}GB - consider optimization")
                except:
                    pass
    ...
    def check_memory(self) -> HealthCheck:
        ...
        try:
            ...
            if status != HealthStatus.HEALTHY:
                ...
                if processes:
                    recommendations.append(f"Top memory user: {processes[0][0]} ({processes[0][1]:.1f}%)")
    ...
    def check_cpu(self) -> HealthCheck:
        ...
        try:
            ...
            if status != HealthStatus.HEALTHY:
                ...
                if processes:
                    recommendations.append(f"Top CPU user: {processes[0][0]} ({processes[0][1]:.1f}%)")
        ...
        try:
            ...
            try:
                ...
                if result.returncode != 0:
                    ...
                    recommendations.append("Run: nix-store --verify --check-contents --repair")
            except:
                pass
            ...
            if ssh_config.exists():
                try:
                    ...
                    if 'PermitRootLogin yes' in content:
                        ...
                        recommendations.append("Set PermitRootLogin to 'no' in sshd_config")
                    if 'PasswordAuthentication yes' in content:
                        issues.append("Password authentication enabled for SSH")
                        recommendations.append("Consider using key-based authentication only")
                except:
                    pass
            
            # Check for security updates
            recommendations.append("Check for security updates: nixos-rebuild dry-build")
    ...
    def check_performance(self) -> HealthCheck:
        ...
        try:
            ...
            if issues:
                status = HealthStatus.WARNING
                message = f"Performance issues detected"
            else:
                status = HealthStatus.HEALTHY
        ...
        if format == 'json':
            report = {
                ...
                'metrics': {
                    ...
                    'memory_percent': self.metrics.memory_percent if self.metrics else None,
