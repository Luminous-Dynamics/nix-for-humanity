...
@click.option(
    "--personality",
    type=click.Choice(["gentle", "energetic", "professional"]),
    default="gentle",
    help="Voice personality",
)
@click.option(
    "--verbosity",
...
@click.option(
    "--focus-protection/--no-focus-protection",
    default=True,
...
@click.option(
    "--personality", type=click.Choice(["gentle", "energetic", "professional"])
)
...
def status():
    ...
    click.echo(
        f"Sacred Pauses: {config.pause_before_speech}s before, {config.pause_after_listening}s after"
    )
...
def start_voice_mode(
    voice_only=False, personality="gentle", verbosity="normal", focus_protection=True
):
    """
    Start interactive voice mode
    """
