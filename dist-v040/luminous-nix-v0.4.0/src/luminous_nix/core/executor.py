...
class SafeExecutor:
    ...
    def execute(
        self, command: str, args: list = None, dry_run: bool = None
    ) -> dict[str, Any]:
        """
        ...
        is_query = any(
            word in command for word in ["search", "list", "show", "info", "status"]
        )
    ...
    def _run_command(
        self, command: str, args: list, needs_confirm: bool = False
    ) -> dict[str, Any]:
        """Run with platform trust and JSON optimization."""
        try:
    ...
    def _extract_teaching(self, error: str) -> str:
        ...
        if "no space" in error.lower():
            return (
                "Disk space is precious. Try: ask-nix 'garbage collect' to free space"
            )
        ...
        for pattern in dangerous:
            if pattern in command_str:
                return ValidationResult(
                    False, "This could be dangerous", ["Consider a safer approach"]
                )
