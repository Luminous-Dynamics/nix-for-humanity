...
class NativeFastAPI:
    ...
    def search_packages_fast(self, query: str) -> Tuple[List[Dict], float]:
        ...
        if self._package_cache and (time.time() - self._last_cache_update < self._cache_ttl):
            ...
            if result.returncode == 0 and result.stdout:
                ...
                for name, info in packages.items():
                    results.append({
                        ...
                        "description": info.get("description", "")[:100]  # Truncate for speed
                    })
                    ...
                    if isinstance(profiles, dict):
                        for key, item in profiles.items():
                            if isinstance(item, dict):
                                packages.append({
                                    "name": item.get("originalUrl", key).split("#")[-1],
                                    "version": item.get("version", ""),
                                    "store_path": item.get("storePaths", [""])[0] if isinstance(item.get("storePaths"), list) else ""
                                })
                    elif isinstance(profiles, list):
                        for item in profiles:
                            if isinstance(item, dict):
                                packages.append({
                                    "name": item.get("originalUrl", "").split("#")[-1],
                                    "version": item.get("version", ""),
                                    "store_path": item.get("storePaths", [""])[0] if isinstance(item.get("storePaths"), list) else ""
                                })
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "package_cache_size": len(self._package_cache) if self._package_cache else 0,
            "cache_age_seconds": time.time() - self._last_cache_update if self._last_cache_update else None,
            "lru_cache_info": {
