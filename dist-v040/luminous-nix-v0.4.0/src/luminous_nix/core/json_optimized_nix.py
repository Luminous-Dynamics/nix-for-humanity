...
class JSONOptimizedNix:
    ...
    def search_packages(
        self, query: str, channel: str = "nixpkgs"
    ...
    def get_package_info(self, package: str) -> Optional[Dict]:
        ...
        try:
            ...
            cmd = [
                ...
                f"nixpkgs#{package}",
                "--apply",
                'p: { name = p.name or "unknown"; version = p.version or "unknown"; meta = p.meta or {}; }',
            ]
    ...
    def get_system_info(self) -> Dict:
        ...
        try:
            result = subprocess.run(
                ["nixos-version", "--json"], capture_output=True, text=True, timeout=5
            )
            if result.returncode == 0:
    ...
    def build_with_log(self, attribute: str) -> Tuple[bool, Dict, float]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                build_info = {"success": False, "error": result.stderr}
                elapsed_ms = (time.time() - start_time) * 1000
    ...
    def _list_installed_legacy(
        self, profile: Optional[str], start_time: float
    ) -> Tuple[List[Dict], float]:
