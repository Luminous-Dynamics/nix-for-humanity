...
class NixOSDoctor:
    ...
    def _collect_system_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                ["nixos-rebuild", "list-generations"], capture_output=True, text=True
            )
            ...
            for i, line in enumerate(lines, 1):
                ...
                if stripped and not stripped.startswith("#"):
                    ...
                    if any(
                        ...
                        for k in ["services.", "programs.", "environment.", "users."]
                    ):
    ...
    def _check_services(self):
        ...
        critical_services = ["NetworkManager", "systemd-resolved", "nix-daemon"]
    ...
    def _check_channels(self):
        ...
        try:
            result = subprocess.run(
                ["nix-channel", "--list"], capture_output=True, text=True
            )
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        if not self.issues_found:
            ...
            # Count by severity
            critical = sum(
                1 for i in self.issues_found if i.severity == Severity.CRITICAL
            )
            ...
            if auto_fixable:
                recommendations.append(
                    f"🔧 {len(auto_fixable)} issues can be fixed automatically"
                )

            # Specific recommendations
            if any(i.type == IssueType.DISK_SPACE for i in self.issues_found):
                recommendations.append(
                    "💾 Free disk space with 'nix-collect-garbage -d'"
                )
