...
class RateLimiter:
    ...
    def check_rate(self, user_id: str | None = None) -> tuple[bool, str | None]:
...
class SecureIntentRecognizer:
    ...
    def recognize(self, text: str, user_id: str | None = None) -> dict[str, any]:
        ...
        if not allowed:
            return {"error": "RATE_LIMITED", "reason": reason, "intent": None}
        ...
        try:
            ...
            return {
                ...
                "llm_assessment": llm_assessment
                if llm_assessment.get("available")
                else None,
            }

        except Exception as e:
            logger.error(f"Intent recognition failed: {e}")
            return {"error": "RECOGNITION_ERROR", "reason": str(e), "intent": None}

    def _log_security_event(
        self, text: str, assessment: SecurityAssessment, user_id: str | None
    ):
