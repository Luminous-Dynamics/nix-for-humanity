...
class IntentRecognitionPipeline:
    ...
    def _load_entity_patterns(self) -> dict[str, list[tuple[str, str]]]:
        ...
        return {
            "package": [
                (
                    r"(?:install|remove|update|search for|search|find|get|add|delete) (\S+)",
                    "direct",
                ),
                ...
                (
                    r"(?:i need|i want|looking for|give me) (?:a |an |some )?(\w+[\w-]*)",
                    "natural",
                ),
                (
                    r"(?:a |an |some )?(\w+) (?:editor|browser|player|manager|tool|program|app|server)",
                    "category",
                ),
        ...
        for intent, patterns in self.intent_patterns.items():
            ...
            for pattern, base_score in patterns:
                if re.search(pattern, normalized, re.IGNORECASE):
                    ...
                    position_factor = (
                        1.0 - (match.start() / max(len(normalized), 1)) * 0.1
                    )
    ...
    def _extract_entities(self, original: str, normalized: str) -> list[Entity]:
        ...
        for compound, package in compound_mappings.items():
            if compound in original_lower:
                ...
                original = original.replace(compound, f"COMPOUND_{package}")
        ...
        for entity_type, patterns in self.entity_patterns.items():
            for pattern, pattern_type in patterns:
                ...
                for match in matches:
                    ...
                    confidence = (
                        1.0 if pattern_type in ["direct", "explicit", "path"] else 0.8
                    )
    ...
    def _enhance_with_context(
        self, scores: dict[Intent, float], context: dict[str, Any]
    ) -> dict[Intent, float]:
            ...
            for turn in context.get("recent_history", [])[-3:]:
                if "intent" in turn:
                    ...
                    if recent_intent == Intent.SEARCH and Intent.INSTALL in enhanced:
                        ...
                        recent_intent == Intent.INSTALL and Intent.CONFIGURE in enhanced
                    ):
    ...
    def _generate_suggestions(
        self, intent: Intent, entities: list[Entity], confidence: float
    ) -> list[str]:
        ...
        if confidence < 0.6:
            suggestions.append(
                "I'm not entirely sure what you want. Could you be more specific?"
            )

            # Suggest possible interpretations
            if intent == Intent.UNKNOWN:
                suggestions.append(
                    "Did you mean to: install, search, update, or get help?"
                )
            else:
        ...
        if intent == Intent.SEARCH and entities:
            suggestions.append(
                f"After searching, you can install with: install {entities[0].value}"
            )
        elif intent == Intent.INSTALL and entities:
            suggestions.append(
                f"After installing, you might want to: configure {entities[0].value}"
            )

        return suggestions

    def learn_from_feedback(
        self, query: str, correct_intent: Intent, was_successful: bool
    ):
    ...
    def get_recognition_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "success_rate": len(self.successful_matches)
            / max(len(self.successful_matches) + len(self.failed_matches), 1),
            "total_patterns": sum(
