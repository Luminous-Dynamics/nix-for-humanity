...
Home Manager Integration for Luminous Nix
...
class HomeManager:
    ...
    def _add_theme_to_config(
        self, config: HomeConfig, theme_name: str, applications: list[str]
    ):
        ...
        if theme_name in self.themes:
            ...
            for app in applications:
                theme = ThemeConfig(
                    name=f"{theme_name}_{app}", type=app, settings=theme_data
                )
...
"""
    ...
    def apply_config(self, config: HomeConfig, preview: bool = True) -> dict[str, Any]:
        ...
        if preview:
            ...
            result["commands"] = ["home-manager init", "home-manager switch"]
        else:
    ...
    def list_managed_configs(self) -> dict[str, list[str]]:
        ...
        managed = {"dotfiles": [], "themes": [], "packages": [], "backups": []}
