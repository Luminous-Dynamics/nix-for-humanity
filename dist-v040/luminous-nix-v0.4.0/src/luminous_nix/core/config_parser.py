...
class ConfigurationParser:
    ...
    def _extract_imports(self, lines: list[str]) -> list[NixImport]:
        ...
        import_pattern = re.compile(
            r"^\s*imports\s*=\s*\[(.*?)\];", re.MULTILINE | re.DOTALL
        )
    ...
    def _extract_complex_services(self, content: str) -> dict[str, Any]:
        ...
        ssh_block = re.search(
            r"services\.openssh\s*=\s*\{(.*?)\};", content, re.MULTILINE | re.DOTALL
        )
        if ssh_block:
            ssh_config = {}
            ...
            if "permitRootLogin" in block_content:
                root_login = re.search(
                    r'permitRootLogin\s*=\s*"?(\w+)"?', block_content
                )
                if root_login:
                    ssh_config["permitRootLogin"] = root_login.group(1)
            if "passwordAuthentication" in block_content:
                pwd_auth = re.search(
                    r"passwordAuthentication\s*=\s*(\w+)", block_content
                )
                if pwd_auth:
    ...
    def _extract_users(self, content: str) -> dict[str, Any]:
        ...
        user_pattern = re.compile(
            r"users\.users\.(\w+)\s*=\s*\{(.*?)\};", re.MULTILINE | re.DOTALL
        )
        ...
        for match in user_pattern.finditer(content):
            ...
            if groups_match:
                ...
                groups = [
                    g.strip().strip('"') for g in groups_text.split() if g.strip()
                ]
    ...
    def _extract_boot_config(self, content: str) -> dict[str, Any]:
        ...
        if "boot.loader.systemd-boot.enable = true" in content:
            ...
            device_match = re.search(
                r'boot\.loader\.grub\.device\s*=\s*"([^"]+)"', content
            )
            if device_match:
        ...
        modules_match = re.search(
            r"boot\.initrd\.kernelModules\s*=\s*\[(.*?)\]", content
        )
        if modules_match:
    ...
    def _extract_networking(self, content: str) -> dict[str, Any]:
        ...
        # Allowed ports
        tcp_ports = re.search(
            r"networking\.firewall\.allowedTCPPorts\s*=\s*\[(.*?)\]", content
        )
        if tcp_ports:
    ...
    def _extract_modules(self, lines: list[str]) -> list[NixModule]:
        ...
        for i, line in enumerate(lines):
            ...
            if option_match:
                ...
                module = NixModule(
                    name=option_path, path=path_parts, value=value, line_number=i + 1
                )
    ...
    def _generate_suggestions(self, config: ParsedConfiguration) -> list[str]:
        ...
        for username, user_config in config.users.items():
            if user_config.get("has_plaintext_password"):
                suggestions.append(
                    f"🔒 User '{username}' has plaintext password - use hashedPassword instead"
                )

        if config.networking_config.get("firewall_disabled"):
            suggestions.append(
                "🔥 Firewall is disabled - consider enabling for security"
            )

        # Service suggestions
        if (
            "openssh" in config.services
            and config.services["openssh"].get("permitRootLogin") == "yes"
        ):
        ...
        if len(config.packages) > 50:
            suggestions.append(
                "📦 Many packages installed - consider using Home Manager for user packages"
            )
    ...
    def suggest_improvements(self, config: ParsedConfiguration) -> list[str]:
        ...
        if "docker" not in config.services and any(
            "container" in pkg for pkg in config.packages
        ):
            improvements.append(
