...
class ConfigResult:
    ...
    message: str
    error: str | None = None
    config: ParsedConfiguration | None = None
...
class ConfigExecutor:
    ...
    def execute(
        self, intent_type: str, query: str, entities: dict[str, Any]
    ) -> ConfigResult:
        """
    ...
    def _calculate_statistics(self, config: ParsedConfiguration) -> dict[str, Any]:
        ...
        stats = {
            ...
            "enabled_services": sum(
                1 for s in config.services.values() if s.get("enabled", False)
            ),
        ...
        dev_packages = [
            ...
            if any(
                kw in p.lower()
                for kw in ["gcc", "python", "node", "rust", "git", "vim", "emacs"]
            )
        ]
        system_packages = [
            p
            for p in config.packages
            if any(kw in p.lower() for kw in ["htop", "tree", "wget", "curl", "tmux"])
        ]
    ...
    def _build_result_message(
        self, config: ParsedConfiguration, is_valid: bool, statistics: dict[str, Any]
    ) -> str:
        """
        ...
        lines.append(
            f"🔧 Services: {statistics['enabled_services']}/{statistics['total_services']} enabled"
        )
    ...
    def generate_fix(self, issue: str) -> str:
        ...
        fixes = {
            "firewall": self.parser.generate_snippet(
                "networking.firewall.enable", True
            ),
            "hostname": self.parser.generate_snippet(
    ...
    def learn_from_config(self, config: ParsedConfiguration) -> dict[str, Any]:
        ...
        learning["system_characteristics"] = {
            ...
            "service_count": len(
                [s for s in config.services.values() if s.get("enabled")]
            ),
            "user_count": len(config.users),
            "uses_flakes": any("flake" in imp.path.lower() for imp in config.imports),
            "uses_home_manager": any(
                "home-manager" in imp.path.lower() for imp in config.imports
            ),
        }
