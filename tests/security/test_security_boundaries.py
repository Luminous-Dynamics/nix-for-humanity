...
class TestSecurityBoundaries(unittest.TestCase):
    ...
    def test_command_injection_prevention(self):
        ...
        for malicious_input in malicious_inputs:
            with self.subTest(input=malicious_input):
                ...
                self.assertTrue(
                    result["blocked"], f"Malicious input not blocked: {malicious_input}"
                )
                self.assertIn(
                    "security",
                    result["reason"].lower(),
    ...
    def test_path_traversal_prevention(self):
        ...
        for path_input in path_traversal_inputs:
            with self.subTest(path=path_input):
                ...
                self.assertTrue(
                    result["blocked"], f"Path traversal not blocked: {path_input}"
                )
    ...
    def test_input_length_limits(self):
        ...
        test_cases = [
            ...
            (
                "long_but_reasonable",
                "install firefox with very long package name",
                True,
            ),
        ...
        for case_name, input_text, should_pass in test_cases:
            with self.subTest(case=case_name):
                ...
                if should_pass:
                    self.assertTrue(
                        result["valid"], f"Valid input rejected: {case_name}"
                    )
                else:
                    self.assertFalse(
                        result["valid"], f"Invalid input accepted: {case_name}"
                    )
    ...
    def test_network_based_attacks_prevention(self):
        ...
        for attack in network_attacks:
            with self.subTest(attack=attack):
                ...
                self.assertTrue(
                    result["blocked"], f"Network attack not blocked: {attack}"
                )
    ...
    def test_privacy_protection(self):
        ...
        for data_type, sensitive_value in sensitive_data.items():
            with self.subTest(data_type=data_type):
                ...
                self.assertTrue(
                    result["protected"], f"Sensitive data not protected: {data_type}"
                )
                self.assertNotIn(
                    sensitive_value,
                    result["logged_output"],
    ...
    def test_safe_command_execution(self):
        ...
        for command, should_be_safe in test_commands:
            with self.subTest(command=command):
                ...
                if should_be_safe:
                    self.assertTrue(result["safe"], f"Safe command blocked: {command}")
                else:
                    self.assertFalse(
                        result["safe"], f"Unsafe command allowed: {command}"
                    )
    ...
    def test_file_system_access_controls(self):
        ...
        for file_path, should_allow in file_access_tests:
            with self.subTest(path=file_path):
                ...
                if should_allow:
                    self.assertTrue(
                        result["allowed"], f"Legitimate file access denied: {file_path}"
                    )
                else:
                    self.assertFalse(
                        result["allowed"],
    ...
    def test_session_security(self):
        ...
        for test_name, session_token, should_be_valid in session_tests:
            with self.subTest(test=test_name):
                ...
                if should_be_valid:
                    self.assertTrue(
                        result["valid"], f"Valid session rejected: {test_name}"
                    )
                else:
                    self.assertFalse(
                        result["valid"], f"Invalid session accepted: {test_name}"
                    )
    ...
    def _validate_script_security(self, script_input):
        ...
        sanitized = re.sub(
            r"<script[^>]*>.*?</script>", "", script_input, flags=re.IGNORECASE
        )
        sanitized = re.sub(r"javascript:", "", sanitized, flags=re.IGNORECASE)
        sanitized = re.sub(r"<[^>]+>", "", sanitized)  # Remove all HTML tags

        return {"sanitized": sanitized != script_input, "output": sanitized}
...
class TestNetworkSecurity(unittest.TestCase):
    ...
    def test_data_exfiltration_prevention(self):
        ...
        for attempt in exfiltration_attempts:
            ...
            self.assertTrue(
                result["blocked"], f"Data exfiltration not blocked: {attempt}"
            )
    ...
    def _detect_exfiltration(self, command):
        ...
        for pattern in exfiltration_patterns:
            if re.search(pattern, command):
                return {
                    "blocked": True,
                    "reason": f"Data exfiltration pattern: {pattern}",
                }
...
class TestCryptographicSecurity(unittest.TestCase):
    ...
    def test_secure_random_generation(self):
        ...
        self.assertEqual(
            len(random_values), len(set(random_values)), "Random values not unique"
        )
