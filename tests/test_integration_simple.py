...
def test_native_api_simple():
    ...
    try:
        ...
        try:
            ...
            if result:
                print(f"  {Colors.GREEN}✅ Listed {len(result)} generations{Colors.RESET}")
            else:
                print(f"  {Colors.YELLOW}⚠️  No generations found (expected on non-NixOS){Colors.RESET}")
        except Exception as e:
            print(f"  {Colors.YELLOW}⚠️  API call failed: {str(e)[:50]}...{Colors.RESET}")
...
def test_json_optimization():
    ...
    try:
        ...
        if info:
            print(f"  {Colors.GREEN}✅ Got system info: {info.get('nixos_version', 'unknown')}{Colors.RESET}")
        else:
...
def test_hrm_import():
    ...
    try:
        ...
        if result and result.get("intent"):
            print(f"  {Colors.GREEN}✅ Intent recognized: {result['intent']}{Colors.RESET}")
        ...
        print(f"  {Colors.YELLOW}⚠️  Import failed (PyTorch not installed): {str(e)[:50]}...{Colors.RESET}")
...
def test_integrated_backend():
    ...
    try:
        ...
        if result:
            print(f"  {Colors.GREEN}✅ Query processed in {elapsed_ms:.1f}ms{Colors.RESET}")
            if elapsed_ms < 100:
...
def test_rust_module():
    ...
    try:
        ...
        print(f"     Run: cd rust && maturin develop")
        return False
    except Exception as e:
        print(f"  {Colors.RED}❌ Error: {e}{Colors.RESET}")
        return False
...
def test_performance_check():
    ...
    # Test 1: Import speed
    start = time.time()
    from luminous_nix.core import executor
...
def main():
    ...
    print(f"{Colors.BOLD}{Colors.BLUE}LUMINOUS NIX v0.4.0 - SIMPLIFIED INTEGRATION TEST{Colors.RESET}")
