...
def test_cli_adapter_basic():
    ...
    try:
        ...
        public_methods = [method for method in dir(adapter) if not method.startswith('_')]
    ...
    if adapter_path.exists():
        ...
        for line in lines:
            ...
            if (stripped and 
                ...
                stripped != '"""' and
                stripped != "'''" and
                not (stripped.startswith('"') and stripped.endswith('"') and len(stripped.split()) < 5)):
                executable_lines += 1
                
        print(f"📄 CLI Adapter: {total_lines} total lines, ~{executable_lines} executable")
