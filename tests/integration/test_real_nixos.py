...
def test_real_backend():
    ...
    if response.success:
        ...
        print(f"   ⚠️  Search: {'timeout' if 'timeout' in response.text.lower() else 'failed'}")
    ...
    assert "DRY RUN" in response.text or "Would" in response.text, "Should indicate dry run"
...
def test_cli_with_real_backend():
    ...
    assert "packages" in result.lower() or "installed" in result.lower(), "Should show packages"
