...
class TestCLIAdapterCore:
    ...
    def cli_adapter(self, mock_core):
        ...
        with patch('nix_for_humanity.adapters.cli_adapter.Query') as mock_query:
            with patch('nix_for_humanity.adapters.cli_adapter.ExecutionMode') as mock_mode:
                with patch('nix_for_humanity.adapters.cli_adapter.PersonalityStyle') as mock_style:
                    with patch('nix_for_humanity.adapters.cli_adapter.uuid') as mock_uuid:
                        # Mock UUID generation
                        mock_uuid.uuid4.return_value = Mock(__str__=Mock(return_value='test-session-id-12345'))
                        
                        # Mock ExecutionMode constants
    ...
    def test_process_query_execution_modes(self, cli_adapter, mock_core):
        ...
        mock_core.reset_mock()
        response = cli_adapter.process_query("install firefox", execute=False, dry_run=False)
        query_arg = mock_core.process.call_args[0][0]
...
class TestCLIPersonalityAdaptation:
    ...
    def test_invalid_personality_handling(self, cli_adapter):
        ...
        cli_adapter.core.personality_system.set_style.side_effect = ValueError("Invalid style")
...
class TestCLIOutputFormatting:
    ...
    def test_rich_fallback_to_simple(self, cli_adapter):
        ...
        cli_adapter.visual_mode = True
        response = Mock(text="Fallback response", suggestions=[], feedback_requested=False)
...
class TestCLIAccessibility:
    ...
    def test_screen_reader_friendly_output(self, cli_adapter):
        ...
        response = Mock(
            ...
            suggestions=["Try: firefox --help for options", "See: about:config for settings"],
            feedback_requested=False
        )
    ...
    def test_terminal_width_independence(self, cli_adapter):
        ...
        mock_print.assert_any_call("   • This is a long suggestion that tests word wrapping behavior")
...
class TestCLIPerformance:
    ...
    def test_query_processing_performance(self, cli_adapter):
        ...
        assert processing_time < 1.0, f"Query processing took {processing_time}s, should be <1s"
...
class TestCLIFeedbackSystem:
    ...
    def test_negative_feedback_with_improvement(self, cli_adapter):
        ...
        response = Mock(text="Could be better", suggestions=[], feedback_requested=False)
...
class TestCLIAdapterCoverage:
    ...
    def test_all_public_methods_covered(self):
        ...
        for method in cli_adapter_methods:
            assert hasattr(CLIAdapter, method), f"Method {method} not found in CLIAdapter"
    ...
    def test_all_private_methods_covered(self):
        ...
        for method in private_methods:
            assert hasattr(CLIAdapter, method), f"Private method {method} not found in CLIAdapter"
