...
def run_coverage(html=False, xml=False, min_coverage=95):
    ...
    if html:
        ...
        print_success(f"HTML report generated at: {TESTS_DIR / 'htmlcov' / 'index.html'}")
...
def run_linters(fix=False):
    ...
    if not available_tools:
        print_info("No linting tools found. Install with: pip install black isort flake8 mypy")
        return 0
...
def main():
    ...
    parser.add_argument(
        '--fix',
        action='store_true',
        help='Fix code style issues automatically'
    )
    parser.add_argument(
        '--benchmark',
        action='store_true',
    ...
    run_all = not any([args.unit, args.integration, args.e2e, args.coverage, args.lint, args.benchmark])
    ...
    if run_all or args.coverage:
        result = run_coverage(html=args.html, xml=args.xml, min_coverage=args.min_coverage)
        exit_code = max(exit_code, result)
