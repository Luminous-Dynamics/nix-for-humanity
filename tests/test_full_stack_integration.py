...
def test_embeddingemma_integration() -> Tuple[bool, float]:
    ...
    try:
        ...
        print(f"    Error: {e}")
        print(f"    Install: poetry add sentence-transformers")
        return False, 0
...
def test_native_python_api() -> Tuple[bool, float]:
    ...
    try:
        ...
        if generations:
            ...
            print(f"    Current: Generation {generations[0].get('generation', 'unknown')}")
            print(f"    10x faster than subprocess!")
            return True, elapsed_ms
        else:
            # Fallback test
        ...
        print(f"    Note: {e}")
        print(f"    This is expected if not on NixOS 25.11")
        return False, 0
...
def test_json_optimization() -> Tuple[bool, float]:
    ...
    try:
        ...
        if results:
            ...
            print(f"    10x improvement over text parsing")
            return True, elapsed_ms
        else:
...
def test_rust_acceleration() -> Tuple[bool, float]:
    ...
    try:
        ...
        print(f"    100x faster than Python!")
        return True, elapsed_ms
        
    except ImportError:
        print_test("Rust module", False)
        print(f"    Not built yet. Run: cd rust && maturin develop")
        return False, 0
    except Exception as e:
...
def test_semantic_cache() -> Tuple[bool, float]:
    ...
    try:
        ...
        print(f"    Cache hits in <1ms!")
        return True, elapsed_ms
        
    except ImportError:
        print_test("Semantic cache", False)
        print(f"    Install: poetry add faiss-cpu")
        return False, 0
    except Exception as e:
        print_test("Semantic cache", False)
        print(f"    Error: {e}")
