...
class TestPersonaJourneys(unittest.TestCase):
    ...
    def test_grandma_rose_journey(self):
        ...
        for command in persona.typical_commands:
            ...
            self.assertTrue(
                any(word in display_text.lower() for word in ['help', 'happy', 'great']),
    ...
    def test_alex_accessibility_journey(self):
        ...
        for command in persona.typical_commands:
            ...
            if 'command' in response:
                self.assertTrue(
                    'Command:' in display_text or
                    'Run:' in display_text or
                    'Execute:' in display_text,
    ...
    def test_dr_sarah_technical_journey(self):
        ...
        for command in persona.typical_commands:
            ...
            if 'latex' in command:
                display_text = response.get('display', '')
                self.assertTrue(
                    'texlive-full' in display_text or
                    'texlive' in display_text,
    ...
    def test_carlos_learning_journey(self):
        ...
        for command in persona.typical_commands:
            ...
            educational_indicators = ['learn', 'understand', 'explain', 'because', 'this helps']
            self.assertTrue(
                any(indicator in display_text.lower() for indicator in educational_indicators),
    ...
    def test_priya_efficiency_journey(self):
        ...
        for command in persona.typical_commands:
            ...
            display_text = response.get('display', '')
            efficiency_indicators = ['quick', 'fast', 'immediately', 'right away', 'now']
            self.assertTrue(
                any(indicator in display_text.lower() for indicator in efficiency_indicators),
    ...
    def test_viktor_esl_journey(self):
        ...
        for command in persona.typical_commands:
            ...
            complex_terms = ['repository', 'binary', 'compilation', 'instantiation', 'derivation']
            for term in complex_terms:
                self.assertNotIn(term.lower(), display_text.lower(),
    ...
    def test_luna_consistency_journey(self):
        ...
        if len(commands) > 1:
            self.assertEqual(commands[0], commands[1],
                "Commands inconsistent for Luna (autism)")
        
        # Check explanation detail (Luna needs detailed explanations)
        for display_text in display_texts:
            if display_text:
                self.assertGreater(len(display_text), 50,
                    "Response too brief for Luna's detail needs")
    ...
    def test_all_personas_core_task_success(self):
        ...
        for persona in PERSONAS:
            ...
            # Check basic success
            self.assertIn('display', response, f"No display response for {persona.name}")
            
            # Check appropriate complexity level
            display_text = response['display']
            if persona.age >= 65:  # Older personas
                self.assertLess(len(display_text), 200,
                    f"Response too long for {persona.name}")
            
            if 'ADHD' in persona.characteristics:
                self.assertLess(len(display_text), 150,
    ...
    def test_complete_user_flow(self, mock_run):
        ...
        self.assertTrue(
            'zoom' in response3['display'].lower() or
            'application' in response3['display'].lower()
        )
    ...
    def test_error_recovery_flow(self):
        ...
        for personality, persona_name in personas_to_test:
            ...
            if personality == 'friendly':
                ...
                self.assertTrue(
                    'package' in response['display'].lower() or
                    'not found' in response['display'].lower()
                )
