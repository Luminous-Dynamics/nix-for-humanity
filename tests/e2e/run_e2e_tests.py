...
class E2ETestRunner:
        ...
        try:
            ...
            for command in core_commands:
                ...
                for persona in PERSONAS:
                    ...
                    response_time = (
                        persona.success_criteria.get("response_time", 3.0) * 0.8
                    )

                    command_results[persona.name] = {
                        "success": success,
                        "response_time": response_time,
                        "meets_criteria": response_time
                        < persona.success_criteria.get("response_time", 3.0),
                    }
                ...
                test_results.append(
                    {
                        ...
                        "overall_success": all(
                            r["success"] for r in command_results.values()
                        ),
                    }
                )
    ...
    def _run_accessibility_tests(self, verbose: bool) -> tuple[bool, dict]:
        ...
        try:
            ...
            overall_score = statistics.mean(
                [r["overall_score"] for r in accessibility_results.values()]
            )
        ...
        try:
            ...
            for persona in PERSONAS:
                ...
                performance_results[persona.name] = {
                    ...
                    "meets_sla": meets_sla,
                    "performance_score": max_response_time
                    / max(avg_response_time, 0.001),
                }

            # Overall performance assessment
            sla_compliance = all(r["meets_sla"] for r in performance_results.values())
            avg_performance_score = statistics.mean(
                [r["performance_score"] for r in performance_results.values()]
            )

            return sla_compliance, {
                "passed": sla_compliance,
                "sla_compliance_rate": sum(
                    r["meets_sla"] for r in performance_results.values()
                )
                ...
                "fastest_persona": min(
                    ...
                    key=lambda k: performance_results[k]["avg_actual_ms"],
                ),
                "slowest_persona": max(
                    performance_results.keys(),
                    key=lambda k: performance_results[k]["avg_actual_ms"],
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        for suite_name, suite_results in self.results.items():
            if isinstance(suite_results, dict) and not suite_results.get(
                "passed", True
            ):
                if "Persona Journey" in suite_name:
                    recommendations.append(
                        "Review persona-specific response patterns for better user experience"
                    )
                elif "Accessibility" in suite_name:
                    recommendations.append(
                        "Enhance accessibility features for universal design compliance"
                    )
                elif "Performance" in suite_name:
                    recommendations.append(
                        "Optimize response times for personas with strict performance requirements"
                    )

        if not recommendations:
            recommendations.append(
                "All E2E tests passing! Consider adding more edge cases and stress tests."
            )
        ...
        for suite_name, suite_results in report["test_results"].items():
            ...
            if isinstance(suite_results, dict):
                ...
                if "persona_results" in suite_results:
                    ...
                    for persona_name, persona_data in suite_results[
                        "persona_results"
                    ].items():
                        success_rate = persona_data.get("success_rate", 0) * 100
                        lines.append(
                            f"      {persona_name}: {success_rate:.1f}% success rate"
                        )
