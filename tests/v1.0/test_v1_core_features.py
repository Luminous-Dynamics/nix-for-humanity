...
class TestV1CoreFeatures:
    ...
    def test_natural_language_understanding(self, backend):
        ...
        test_cases = [
            ...
            (
                "I want to search for python packages",
                IntentType.SEARCH,
                {"query": "python"},
            ),
    ...
    def test_smart_package_discovery(self, backend):
        ...
        for query, expected_packages in test_cases:
            ...
            for expected in expected_packages:
                assert any(
                    expected in name for name in result_names
    ...
    def test_native_nix_api_performance(self, mock_nix_api, backend):
        ...
        for operation, params in operations:
            ...
            assert (
                elapsed < 0.1
            ), f"Operation '{operation}' took {elapsed:.2f}s (> 0.1s limit)"
...
class TestV1Integration:
    ...
    def test_complete_user_journey(self, backend):
        ...
        response = backend.process_request(
            Request(command="I want to install a web browser")
        )
        assert response.success
        assert (
            "firefox" in response.message.lower()
            or "chromium" in response.message.lower()
        )

        # 2. User searches for specific package
        response = backend.process_request(Request(command="search for firefox"))
        assert response.success
        ...
        response = backend.process_request(
            Request(command="show me my current generation")
        )
        assert response.success
        ...
        response = backend.process_request(
            Request(command="update system", dry_run=True)
        )
