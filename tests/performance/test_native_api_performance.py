    ...
    def test_concurrent_operations_performance(self):
        ...
        async def concurrent_performance_test():
            ...
            operations = [
                NixOperation(type=OperationType.LIST_GENERATIONS),
                NixOperation(
                    type=OperationType.INSTALL, packages=["test1"], dry_run=True
                ),
                NixOperation(
                    type=OperationType.INSTALL, packages=["test2"], dry_run=True
                ),
            ...
            successful_count = sum(
                1 for r in results if isinstance(r, NixResult) and hasattr(r, "success")
            )
            self.assertGreater(
                successful_count,
                0,
        ...
        async def memory_test():
            ...
            for i in range(20):
                operation = NixOperation(
                    type=OperationType.INSTALL, packages=[f"package{i}"], dry_run=True
                )
                result = await self.backend.execute(operation)
