...
from core.headless_engine import (
    HeadlessEngine, Intent, Context, Response
)
...
class TestHeadlessEngine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Patch the imports
        self.patches = [
            patch('core.headless_engine.NixOSKnowledgeEngine', return_value=self.knowledge_mock),
            patch('core.headless_engine.FeedbackCollector', return_value=self.feedback_mock),
            patch('core.headless_engine.get_plugin_manager', return_value=self.plugin_manager_mock),
            patch('core.headless_engine.CommandPreferenceManager', return_value=self.learning_mock),
            patch('core.headless_engine.IntelligentPackageCache', return_value=self.cache_mock),
        ]
    ...
    def test_process_basic_query(self):
        ...
        self.knowledge_mock.format_response.return_value = "I'll install Firefox for you."
        
        # Mock _apply_personality and _extract_commands
        with patch.object(engine, '_apply_personality', return_value="I'll install Firefox for you."):
            with patch.object(engine, '_extract_commands', return_value=['nix-env -iA nixpkgs.firefox']):
                # Process query
    ...
    def test_plugin_handling(self):
        ...
        with patch.object(engine, '_apply_personality', return_value='Plugin handled this!'):
            # Process
    ...
    def test_personality_application(self):
        ...
        with patch.object(engine, '_apply_personality', side_effect=mock_apply_personality):
            # Set up basic response
            self.knowledge_mock.extract_intent.return_value = {'action': 'help'}
            self.knowledge_mock.get_solution.return_value = {}
            self.knowledge_mock.format_response.return_value = "Here's some help text with 💡 emoji"
    ...
    def test_visual_generation(self):
        ...
        self.knowledge_mock.format_response.return_value = "Multiple installation methods available."
        
        # Mock _apply_personality, _generate_visual, and _extract_commands
        with patch.object(engine, '_apply_personality', return_value="Multiple installation methods available."):
            with patch.object(engine, '_generate_visual', return_value={
                'type': 'options',
                'title': 'Installation Methods',
                'choices': [
                    {'name': 'User Install', 'description': 'Install to user profile'},
                    {'name': 'System Install', 'description': 'Add to configuration.nix'}
                ]
            }):
                with patch.object(engine, '_extract_commands', return_value=['nix-env -iA nixpkgs.firefox', 'environment.systemPackages = [ pkgs.firefox ];']):
    ...
    def test_tracking_interaction(self):
        ...
        with patch.object(engine, '_apply_personality', return_value="Installing Emacs..."):
    ...
    def test_get_stats(self):
        ...
        self.plugin_manager_mock.get_plugin_info.return_value = {
            'total_plugins': 5
        }
