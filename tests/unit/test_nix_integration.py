...
class TestNixOSIntegration(unittest.TestCase):
    ...
    def test_execute_intent_install_package(self):
        ...
        mock_result.data = {
            "config_snippet": "environment.systemPackages = [ pkgs.firefox ];"
        }
        mock_result.error = None
        ...
        # Verify educational context
        self.assertIn(
            "declarative configuration", result["education"]["why_it_matters"]
        )
    ...
    def test_get_nixos_version(self):
        ...
        with patch(
            "builtins.open",
            unittest.mock.mock_open(read_data='NAME="NixOS"\nVERSION="24.05 (Uakari)"'),
        ):
    ...
    def test_get_nixos_version_malformed(self):
        ...
        with patch("builtins.open", unittest.mock.mock_open(read_data="NAME=NixOS")):
            version = self.integration._get_nixos_version()
...
class TestConvenienceFunctions(unittest.TestCase):
    ...
    def test_full_update_workflow(self):
        ...
        mock_info_result.data = {
            "generations": [{"generation": 42, "date": "2024-01-03", "current": True}]
        }
