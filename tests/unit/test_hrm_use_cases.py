...
def test_dependency_resolution():
    ...
    task = ReasoningTask(
        ...
        goal_state={
            "installed": ["python311", "python312", "tensorflow", "numpy-1.24", "numpy-1.21"],
            "no_conflicts": True
        }
    )
...
def test_configuration_planning():
    ...
    task = ReasoningTask(
        ...
        current_state={},
        goal_state={
            "services": ["nginx", "postgresql", "redis", "docker", "prometheus", "grafana"],
            "security": "hardened",
            "monitoring": "complete"
        }
    )
    ...
    print(f"📊 Hierarchical Planning Results:")
...
def test_realtime_preview():
    ...
    for query in queries:
        ...
        print(f"  📋 Config preview: {result.solution.split('\\n')[0][:60]}...")
