...
module_path = os.path.join(os.path.dirname(__file__), '../../scripts/nix-knowledge-engine.py')
...
class TestNixOSKnowledgeEngine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.engine.init_db = nix_knowledge_engine.NixOSKnowledgeEngine.init_db.__get__(self.engine)
        self.engine._populate_initial_knowledge = nix_knowledge_engine.NixOSKnowledgeEngine._populate_initial_knowledge.__get__(self.engine)
        self.engine.extract_intent = nix_knowledge_engine.NixOSKnowledgeEngine.extract_intent.__get__(self.engine)
        self.engine.get_solution = nix_knowledge_engine.NixOSKnowledgeEngine.get_solution.__get__(self.engine)
        self.engine._get_install_methods = nix_knowledge_engine.NixOSKnowledgeEngine._get_install_methods.__get__(self.engine)
        self.engine.format_response = nix_knowledge_engine.NixOSKnowledgeEngine.format_response.__get__(self.engine)
    ...
    def test_init_db_creates_tables(self):
        ...
        c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='solutions'")
