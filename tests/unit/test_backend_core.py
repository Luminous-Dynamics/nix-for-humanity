...
class TestNixForHumanityBackend(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patches = [
            patch('core.backend.IntentRecognizer', return_value=self.mock_intent_recognizer),
    ...
    def test_find_nixos_rebuild_path_known_paths(self):
        ...
        with patch('pathlib.Path.exists') as mock_exists:
            ...
            expected_path = Path("/nix/store/nmg1ksa23fpsl631x3n8lnp9467vqiqi-nixos-rebuild-ng-0.0.0/lib/python3.13/site-packages")
            self.assertEqual(result, expected_path)
    
    def test_find_nixos_rebuild_path_via_which(self):
        """Test finding nixos-rebuild path via which command."""
        with patch('pathlib.Path.exists', return_value=False):  # Known paths don't exist
            with patch('subprocess.run') as mock_run:
                # Mock successful which command
                mock_run.return_value = Mock(
                    returncode=0,
                    stdout='/nix/store/abc/bin/nixos-rebuild\n'
                )
    ...
    async def test_process_request_success(self, mock_create_task):
        ...
        with patch('backend.security.input_validator.InputValidator.validate_input') as mock_validate:
    ...
    async def test_process_request_with_execution(self):
        ...
        self.mock_executor.execute = AsyncMock(
            return_value=Result(
                success=True,
                output="Package vim installed successfully",
                error=None
            )
        )
        
        with patch('backend.security.input_validator.InputValidator.validate_input') as mock_validate:
            mock_validate.return_value = {'valid': True, 'sanitized_input': 'install vim'}
    ...
    async def test_process_request_exception_handling(self):
        ...
        with patch('backend.security.input_validator.InputValidator.validate_input') as mock_validate:
            mock_validate.return_value = {'valid': True, 'sanitized_input': 'test error'}
    ...
    def test_build_response_text_personalities(self):
        ...
        for personality, check_fn in personalities.items():
            response_text = self.backend._build_response_text(intent, knowledge, personality)
            self.assertTrue(
...
class TestBackendAsyncIntegration(unittest.TestCase):
    ...
    def test_async_to_sync_conversion(self):
        ...
        with patch('core.backend.IntentRecognizer'), \
