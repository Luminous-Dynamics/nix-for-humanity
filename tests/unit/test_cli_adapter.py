...
class TestCLIAdapter(SacredTestBase):
    ...
    async def test_parse_arguments_respects_user_choice(self):
        ...
        with patch('sys.argv', ['ask-nix', 'install', 'firefox']):
            ...
            self.assertEqual(args.personality, 'friendly')  # Default respects most users
    ...
    async def test_format_response_adapts_to_personas(self):
        ...
        for persona in self.test_personas:
            ...
            if persona.name == "maya_adhd":
                ...
                self.assertNotIn("═" * 50, output, "Box characters bad for screen readers")
    ...
    async def test_collect_feedback_enables_learning(self):
        ...
        response = Response(
            text="I installed Firefox for you",
            data={"collect_feedback": True}
        )
    ...
    async def test_collect_feedback_not_helpful(self):
        ...
        with patch('builtins.input', side_effect=['n', 'Too complex, I just wanted VS Code']):
    ...
    async def test_gather_feedback_with_comment(self):
        ...
        with patch('builtins.input', side_effect=['n', 'Too many options, just tell me the best way']):
    ...
    async def test_personality_adaptation_serves_all_personas(self):
        ...
        persona_tests = [
            ...
            ("grandma_rose", "--friendly", "I need Firefox", lambda r: "for you" in r.text),
            ("dr_sarah", "--technical", "install firefox-esr", lambda r: "nixpkgs" in r.text),
            ("alex_blind", "--accessible", "install firefox", lambda r: hasattr(r, 'screen_reader_optimized') and r.screen_reader_optimized),
            ("carlos_learner", "--encouraging", "install firefox", lambda r: "learn" in r.text.lower())
        ]
        ...
        for persona_name, flag, query, validation in persona_tests:
            with patch('sys.argv', ['ask-nix', flag, query]):
                ...
                self.assertTrue(
                    validation(response),
                    f"Response not optimized for {persona_name}"
                )
                
                # Verify sacred behavior for all personas
                self.assert_sacred_behavior(response, self.get_test_persona(persona_name))
    ...
    async def test_error_handling_protects_users(self):
        ...
        for error_type, user_message in error_scenarios:
            ...
            with patch('sys.argv', ['ask-nix', 'install something']):
                ...
                if hasattr(self.adapter, 'current_personality') and self.adapter.current_personality != 'technical':
    ...
    async def test_speak_response_readiness(self):
        ...
        for text, persona_name in test_responses:
            ...
            if persona_name == "maya_adhd":
                ...
                self.assertLess(len(text.split()), 5, "Too wordy for Maya's voice output")
                ...
                self.assertTrue(
                    any(tech in text.lower() for tech in ["version", "nixpkgs", "installed"]),
