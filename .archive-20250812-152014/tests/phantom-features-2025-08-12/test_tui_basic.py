...
def test_tui_app_imports():
    ...
    tui_apps = [
        ...
        ("src/nix_for_humanity/tui/app.py", "luminous_nix.tui.app"),
        ("src/nix_for_humanity/tui/main_app.py", "luminous_nix.tui.main_app"),
        (
            "src/nix_for_humanity/tui/enhanced_app.py",
            "luminous_nix.tui.enhanced_app",
        ),
    ]
    ...
    for app_path, module_name in tui_apps:
        ...
        if full_path.exists():
            ...
            try:
                ...
                print(
                    f"   📄 {total_lines} total lines, ~{executable_lines} executable"
                )
...
def estimate_tui_coverage_potential():
    ...
    test_files = list(project_root.glob("tests/**/test_tui*.py")) + list(
        project_root.glob("tests/**/test_*tui*.py")
    )
    ...
    if test_files:
        ...
        coverage_ratio = (
            total_test_lines / total_executable if total_executable > 0 else 0
        )
