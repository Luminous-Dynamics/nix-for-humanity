...
def test_semantic_understanding():
    ...
    print(f"\n📈 Match Statistics:")
    print(f"   Category matches: {stats['category_matches']} ({stats.get('category_rate', 0):.1f}%)")
    print(f"   Synonym matches: {stats['synonym_matches']} ({stats.get('synonym_rate', 0):.1f}%)")
    print(f"   Learned matches: {stats['learned_matches']} ({stats.get('learned_rate', 0):.1f}%)")
    print(f"   Fuzzy matches: {stats['fuzzy_matches']} ({stats.get('fuzzy_rate', 0):.1f}%)")
...
def test_learning_capability():
    ...
    print(f"   Suggestions: {intent1.suggested_packages[:3] if intent1.suggested_packages else 'None'}")
    ...
    print(f"   Suggestions: {intent2.suggested_packages[:3] if intent2.suggested_packages else 'None'}")
    
    # Check if learning worked
    success = (
        intent2.confidence > intent1.confidence and
        "neovim" in intent2.suggested_packages[:1]
    )
...
def test_performance():
    ...
    queries = [
        ...
        ("complex", "I need a privacy-focused web browser with good extension support that works well on older hardware"),
...
def main():
    ...
    print("  • Natural language understanding ('I need to edit code' → vim, neovim, vscode)")
