...
class CelebrationConfig:
    ...
    sound_effect: str | None = None  # For future audio
...
class CelebrationEffects:
    ...
    @classmethod
    def get_celebration_for_operation(
        cls, operation: str, duration_ms: float
    ) -> CelebrationConfig:
        ...
        if "learn" in operation.lower():
            return cls.CELEBRATIONS[CelebrationStyle.LEARNING]
        if "build" in operation.lower() or "compile" in operation.lower():
            return cls.CELEBRATIONS[CelebrationStyle.BUILDING]
        if "clean" in operation.lower() or "garbage" in operation.lower():
    ...
    @staticmethod
    def create_special_pattern(
        pattern_name: str, center_x: int, center_y: int
    ...
    def record_operation(
        ...
        if success:
            ...
            if duration_ms < 100:
                ...
                if (
                    self.stats["instant_operations"] == 1
                    and "first_instant" not in self.achieved_milestones
                ):
        ...
        if (
            self.stats["total_commands"] == 10
            and "10_commands" not in self.achieved_milestones
        ):
            self.achieved_milestones.add("10_commands")
            return "10_commands"

        if (
            self.stats["total_commands"] == 100
            and "100_operations" not in self.achieved_milestones
        ):
