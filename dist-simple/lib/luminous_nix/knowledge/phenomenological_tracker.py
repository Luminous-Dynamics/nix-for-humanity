...
class PhenomenologicalTracker:
    ...
    def _calculate_flow_probability(self, response_time_ms: int) -> float:
        ...
        if len(self.interaction_buffer) >= 3:
            ...
            time_variance = math.sqrt(
                sum((t - sum(recent_times) / 3) ** 2 for t in recent_times) / 3
            )
        ...
        # Combine factors
        flow_prob = (
            recent_success_rate * 0.4 + speed_score * 0.3 + consistency_score * 0.3
        )
    ...
    def _calculate_confusion_probability(self) -> float:
        ...
        # Combine factors
        confusion_prob = (
            repetition_score * 0.4 + time_trend_score * 0.3 + error_score * 0.3
        )
    ...
    def _calculate_satisfaction_score(self) -> float:
        ...
        reasonable_times = sum(
            1 for t in recent_times if t < self.thresholds["slow_response"]
        )
        time_satisfaction = (
            reasonable_times / len(recent_times) if recent_times else 0.5
        )
    ...
    def _calculate_engagement_level(self, response_time_ms: int) -> float:
        ...
        if response_time_ms < 500:  # Too fast, might be clicking through
            return 0.3
        if response_time_ms < self.thresholds["fast_response"]:
            return 0.9  # Engaged and responding quickly
        if response_time_ms < self.thresholds["slow_response"]:
    ...
    def _calculate_learning_momentum(self) -> float:
        ...
        older = (
            self.interaction_buffer[-10:-5]
            if len(self.interaction_buffer) >= 10
            else []
        )
        older_success = (
            sum(1 for i in older if i["success"]) if older else recent_success
        )
        ...
        complexity_progression = len(set(recent_queries)) / len(
            recent_queries
        )  # Variety indicates progress
    ...
    def _infer_emotional_state(
        ...
        if satisfaction > 0.7 and success:
            return EmotionalState.SATISFIED
        if confusion_prob > 0.4:
            return EmotionalState.ANXIOUS
        if success and satisfaction > 0.5:
            return EmotionalState.CONFIDENT
        if satisfaction > 0.4:
            return EmotionalState.CURIOUS
        return EmotionalState.NEUTRAL

    def _infer_cognitive_load(
        self, response_time_ms: int, confusion_prob: float, error_count: int
    ) -> CognitiveLoad:
        """Infer cognitive load from indicators"""
        if confusion_prob > 0.7 or error_count > 3:
            return CognitiveLoad.OVERWHELMED
        if (
            response_time_ms > self.thresholds["slow_response"] or confusion_prob > 0.5
        ):
            return CognitiveLoad.HIGH
        if (
            response_time_ms < self.thresholds["fast_response"] and confusion_prob < 0.2
        ):
    ...
    def _calculate_stress_indicators(self) -> dict[str, float]:
        ...
        if len(self.interaction_buffer) >= 2:
            ...
            if len(self.error_window) >= 2:
                ...
                indicators["error_clustering"] = max(
                    0, 1 - (avg_gap / 60)
            ...
            for i in range(1, len(self.interaction_buffer)):
                gap = (
                    self.interaction_buffer[i]["timestamp"]
                    - self.interaction_buffer[i - 1]["timestamp"]
                )
                interaction_gaps.append(gap)

            if interaction_gaps:
                avg_gap = sum(interaction_gaps) / len(interaction_gaps)
                indicators["interaction_pressure"] = max(
                    0, 1 - (avg_gap / 10)
    ...
    def get_current_state(self) -> PhenomenologicalSnapshot | None:
    ...
    def get_state_summary(self) -> dict[str, Any]:
        ...
        if not self.state_history:
            return {"current_state": None, "trend": "unknown", "recommendations": []}
        ...
        if len(self.state_history) >= 3:
            recent_satisfaction = [
                s.satisfaction_score for s in self.state_history[-3:]
            ]
            satisfaction_trend = (
                "improving"
            ...
            recent_confusion = [
                s.confusion_probability for s in self.state_history[-3:]
            ]
            confusion_trend = (
                "increasing"
                if recent_confusion[-1] > recent_confusion[0]
                else "decreasing"
            )
        else:
        ...
        if current.emotional_state == EmotionalState.FRUSTRATED:
            recommendations.append(
                "Consider simpler explanations or breaking down the task"
            )
        ...
        if current.cognitive_load == CognitiveLoad.OVERWHELMED:
            recommendations.append(
                "Reduce complexity and provide step-by-step guidance"
            )
        ...
        if current.learning_momentum < -0.3:
            ...
            recommendations.append(
                "User is progressing well - consider advancing topics"
            )
...
# Singleton instance
_tracker_instance: PhenomenologicalTracker | None = None
