...
class ConfigLoader:
        ...
        try:
            ...
            if path_obj.suffix in [".yaml", ".yml"]:
                if HAS_YAML:
                    ...
                    self.logger.warning(
                        f"YAML file {path} found but PyYAML not installed"
                    )
                    return None
                ...
                if HAS_TOML:
                    ...
                    self.logger.warning(
                        f"TOML file {path} found but toml not installed"
                    )
                    return None
    ...
    def _apply_env_overrides(self, config: dict[str, Any]) -> dict[str, Any]:
        ...
        env_mappings = {
            ...
            # Learning settings
            "NIX_HUMANITY_LEARNING_ENABLED": (
                "learning",
                "enabled",
                lambda v: v.lower() == "true",
            ),
            # Voice settings
            "NIX_HUMANITY_VOICE_ENABLED": (
                "voice",
                "enabled",
                lambda v: v.lower() == "true",
            ),
            ...
            "NIX_HUMANITY_SCREEN_READER": (
                "accessibility",
                "screen_reader",
                lambda v: v.lower() == "true",
            ),
            "NIX_HUMANITY_HIGH_CONTRAST": (
                "accessibility",
                "high_contrast",
                lambda v: v.lower() == "true",
            ),
            "NIX_HUMANITY_ACCESSIBLE": (
                "accessibility",
                "screen_reader",
                lambda v: v.lower() == "true",
            ),
