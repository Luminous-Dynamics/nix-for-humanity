...
class PackageDiscoveryCLI:
    ...
    def _generate_suggestions(
        self, intent: SearchIntent, results: list[PackageInfo]
    ) -> list[str]:
        ...
        if intent.category:
            suggestions.append(
                f"See all {intent.category}s with 'list {intent.category}s'"
            )
            suggestions.append(
                f"Compare popular {intent.category}s with 'best {intent.category}s'"
            )
        ...
        if response["results"]:
            ...
            for i, pkg in enumerate(response["results"], 1):
                ...
                if pkg.get("alternatives"):
                    output.append(
                        f"   🔄 Alternatives: {', '.join(pkg['alternatives'])}"
                    )

                if pkg.get("install_with"):
                    output.append(
                        f"   📦 Install with: {', '.join(pkg['install_with'])}"
                    )
...
class PackageDiscoveryCommand:
    ...
    def format_recommendations(self, response: dict[str, Any]) -> str:
        ...
        output = ["\n" + "=" * 60, "🎯 Personalized Recommendations", "=" * 60, ""]
