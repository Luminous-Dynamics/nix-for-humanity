...
class PackageIntelligence:
        ...
        for pattern in category_patterns:
            if match := re.search(pattern, query_lower):
                ...
                if category := self._match_category(potential_category):
                    return SearchIntent(query=query, action="find", category=category)

        # Default to search
        return SearchIntent(query=query, action="search")

    def _match_category(self, term: str) -> str | None:
    ...
    def _find_category(self, package: str) -> str | None:
            ...
            if result.returncode == 0 and result.stdout:
                ...
                for path, info in packages.items():
                    pkg = PackageInfo(
                        ...
                        categories=[
                            self._find_category(info.get("pname", ""))
                            or "uncategorized"
                        ],
                    )
                    results.append(pkg)
