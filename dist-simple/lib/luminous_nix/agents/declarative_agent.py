...
from .poml_bridge_v2 import POMLOrchestrator, POMLProcessor
...
class DeclarativeAgent:
    ...
    def __init__(self):
        ...
        self.poml_orchestrator.register_prompt(
            "transform", Path(__file__).parent / "transform_prompt_v2.poml"
        )
    ...
    def transform(
        self, intention: str, understanding: dict[str, Any]
    ) -> TransformationSet:
        """
        ...
        # Example: "install firefox"
            ...
            if package:
                transformations.extend(
                    self._install_package_transformations(package, understanding)
                )
    ...
    def manifest(
        ...
        if not transformation_set.is_safe():
            return (
                False,
                f"Transformation deemed unsafe (safety: {transformation_set.safety_score:.2f})",
            )

        if transformation_set.confidence < self.min_confidence:
            return (
                False,
                f"Insufficient confidence (confidence: {transformation_set.confidence:.2f})",
            )

        self.logger.info(
            f"✨ Manifesting {len(transformation_set.transformations)} transformations"
        )
        ...
        new_config = self._apply_transformations(
            current_config, transformation_set.transformations
        )
    ...
    def _parse_intention(self, intention: str) -> dict[str, Any]:
        ...
        return {"raw": intention, "action": "unknown", "target": "unknown"}

    def _add_nginx_transformations(
    ...
    def _enable_docker_transformations(
        self, understanding: dict[str, Any]
    ) -> list[Transformation]:
    ...
    def _install_package_transformations(
        self, package: str, understanding: dict[str, Any]
    ) -> list[Transformation]:
    ...
    def _extract_package_name(self, intention: str) -> str | None:
    ...
    def _calculate_confidence(
        self, transformations: list[Transformation], understanding: dict[str, Any]
    ) -> float:
        ...
        operation_factor = sum(
            1.0 if t.operation == "add" else 0.5 for t in transformations
        ) / len(transformations)

        # Package installations are well-understood, higher base confidence
        package_bonus = (
            0.3 if any("systemPackages" in t.path for t in transformations) else 0.0
        )
    ...
    def _calculate_safety(
        ...
        critical_count = sum(
            ...
            if any(critical in t.path for critical in ["boot", "kernel", "systemd"])
        )
    ...
    def _apply_transformations(
        self, config: str, transformations: list[Transformation]
    ) -> str:
    ...
    def _store_transformation_history(
        self, transformation_set: TransformationSet, config_path: str, backup_path: str
    ):
        ...
        history_entry = {
            ...
            "transformations": [
                t.to_dict() for t in transformation_set.transformations
            ],
...
"""
    ...
    success, result = agent.manifest(
        transformation_set, "/tmp/test-config.nix", preview=True
    )
    if success:
        preview_lines = result.splitlines()[-5:]  # Last 5 lines
        print("  ✅ Success! Preview of changes:")
