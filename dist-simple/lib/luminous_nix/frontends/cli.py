...
try:
    from luminous_nix.core.command_executor import CommandExecutor, CommandType, CommandStatus
...
class UnifiedNixAssistant:
    ...
    def __init__(self):
        ...
        try:
            from luminous_nix.core.secure_intent_integration import create_secure_pipeline_for_cli
            ...
            if self.verbose_level > 0:
                ...
                print(f"🔐 Secure intent recognition enabled (security: {security_level})")
        except ImportError:
    ...
    def answer(self, query: str):
        ...
        if self.intent_pipeline:
            ...
            if hasattr(intent_result, 'metadata') and intent_result.metadata.get('security_block'):
                print(f"🚫 {intent_result.suggestions[0] if intent_result.suggestions else 'Input blocked for security reasons'}")
            ...
            if intent_result.confidence > 0.7:
                ...
                if hasattr(self.intent_pipeline, 'learn_correction') and not self.skip_confirmation:
                ...
                if intent_result.suggestions:
                    ...
                    if not self.skip_confirmation:
                        ...
                        if response.lower() == 'y':
                            ...
                            self._handle_incorrect_intent(query, intent_result.primary_intent)
                            return
        ...
        if "list" in query_lower and ("installed" in query_lower or "packages" in query_lower):
            ...
            if self.command_executor and self.command_executor.get_history():
                ...
                self._handle_rollback()
        elif "create" in query_lower and ("shell" in query_lower or "environment" in query_lower or "dev" in query_lower):
            intent = 'create_shell'
            # Extract language from query
            languages = ['python', 'javascript', 'typescript', 'rust', 'go', 'java', 'c', 'cpp', 'c++', 'haskell', 'ruby']
            found_language = None
            ...
            if found_language:
                ...
                print("Supported: python, javascript, typescript, rust, go, java, c, cpp, haskell, ruby")
        elif ("garbage" in query_lower and "collect" in query_lower) or ("clean" in query_lower and ("up" in query_lower or "disk" in query_lower or "space" in query_lower)):
            intent = 'garbage_collect'
            self._handle_garbage_collect()
        elif ("diagnose" in query_lower or "troubleshoot" in query_lower or "fix" in query_lower or 
              ("something" in query_lower and "wrong" in query_lower)):
            intent = 'diagnose'
    ...
    def _install_package(self, package: str):
        ...
        try:
            ...
            if profile_check.returncode != 0 and "incompatible with 'nix-env'" in profile_check.stderr:
                ...
                if not success:
                    ...
                    print("Please resolve the profile issue manually before installing packages")
                    return
    ...
    def _remove_package(self, package: str):
        ...
        try:
            ...
            result = subprocess.run(["nix", "profile", "list"], 
