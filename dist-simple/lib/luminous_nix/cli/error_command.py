...
from luminous_nix.core.error_translator import ErrorTranslator, TranslatedError
...
@click.option('--persona', '-p', type=click.Choice(['minimal', 'friendly', 'encouraging', 'technical']), 
              default='friendly', help='Response style')
@click.option('--verbose', '-v', is_flag=True, help='Show detailed explanation')
def explain(error_text: Optional[str], file: Optional[str], persona: str, verbose: bool):
    ...
    """
    # Try to use AST-enhanced error analysis if available
    orchestrator = SystemOrchestrator()
    use_ast = orchestrator.has_capability('ast_error_intelligence')
    ...
    # Translate the error using AST analysis if available
    if use_ast:
        # Use AST-enhanced error analysis
        analysis = orchestrator.analyze_error_ast(error_text, file)
        # Convert to TranslatedError format for consistency
        translator = ErrorTranslator()
        translated = translator.translate_error(error_text)
        # Enhance with AST insights
        if 'location' in analysis:
            translated.detailed_explanation = f"Error at {analysis['location']}\n" + translated.detailed_explanation
    else:
        # Use standard error translator
        translator = ErrorTranslator()
        translated = translator.translate_error(error_text)
...
def common(error_type: Optional[str]):
    ...
    common_errors = {
        "missing": {
            ...
            "examples": [
                "attribute 'nodejs_18' missing",
                "Package 'vscode' not found"
            ],
            "solutions": [
...
def analyze(log_file: str, all: bool):
    ...
    click.echo(click.style(f"🔍 Found {len(errors_found)} error(s) in {log_file}", fg='cyan', bold=True))
    click.echo()
