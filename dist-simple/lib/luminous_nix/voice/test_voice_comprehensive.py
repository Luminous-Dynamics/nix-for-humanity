...
from sacred_voice_interface import (
    SacredVoiceInterface, VoiceConfig, FlowState, 
    InterruptionLevel, create_voice_interface
)
from voice_nlp_bridge import (
    VoiceNLPBridge, VoiceContext, create_voice_bridge
)
...
class TestSacredVoiceInterface(unittest.TestCase):
    ...
    def test_speak_with_interruption_control(self):
        ...
        mock_speaker.say.assert_called_with("Hello world...")  # Gentle personality adds ...
...
class TestVoiceNLPBridge(unittest.TestCase):
    ...
    def test_voice_preprocessing(self):
        ...
        self.assertEqual(
            self.bridge._preprocess_voice_input("fire fox"),
            "firefox"
        )
        self.assertEqual(
            self.bridge._preprocess_voice_input("nix o s rebuild"),
            "nixos rebuild"
        )
    ...
    def test_query_extraction(self):
        ...
        query = self.bridge._extract_query(
            "please install firefox browser",
            IntentType.INSTALL
        )
        ...
        query = self.bridge._extract_query(
            "can you please install the firefox",
            IntentType.INSTALL
        )
...
def run_all_tests():
    ...
    print(f"\n{'='*60}")
    print(f"Voice Interface Test Coverage Report")
    print(f"{'='*60}")
