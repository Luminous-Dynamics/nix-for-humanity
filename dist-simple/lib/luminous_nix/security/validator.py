...
class InputValidator:
    ...
    @classmethod
    def validate_input(cls, user_input: str, input_type: str = 'general') -> Dict[str, Any]:
        """
        ...
        if len(user_input) > cls.MAX_INPUT_LENGTH:
            return {
                ...
                'suggestions': ['Please shorten your request', 'Break it into smaller parts']
            }
    ...
    def _validate_nlp_input(cls, user_input: str) -> Dict[str, Any]:
        ...
        sanitized = ''.join(char for char in sanitized if not ord(char) < 32 or char == '\n')
    ...
    def _validate_package_name(cls, package_name: str) -> Dict[str, Any]:
        ...
        if any(sus in package_name.lower() for sus in suspicious_packages):
            return {
                ...
                'suggestions': ['Please verify the package name', 'Check nixpkgs for valid packages']
            }
            
        return {
            'valid': True,
            'sanitized_input': package_name
        }
    ...
    @classmethod
    def validate_command(cls, command: List[str]) -> Tuple[bool, Optional[str]]:
        """
        ...
        allowed_commands = [
            ...
            'nixos-rebuild', 'nix-shell', 'nix-instantiate', 'nix-collect-garbage',
            'systemctl', 'journalctl', 'ip', 'ping', 'df', 'free', 'ps',
            'which', 'whereis', 'ls', 'cat', 'echo', 'true', 'false'
        ]
    ...
    @classmethod
    def validate_nix_expression(cls, expr: str) -> Tuple[bool, Optional[str]]:
        """
...
class SecurityContext:
    ...
    def __init__(self, operation_type: str, user: Optional[str] = None):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.start_time:
            ...
            logger.info(f"Security context ended: {self.operation_type} took {elapsed:.2f}s")
