...
try:
    from .plugin_loader import DiscoveredPlugin, PluginLoader
    from .sandbox import ConsentRequired, PluginSandbox
except ImportError:
...
class RouteMatch:
    ...
    handler_id: str | None  # plugin ID if plugin
    intent_pattern: str
    plugin_info: Any | None = None  # Plugin info if plugin match
...
class ConsciousnessRouter:
    ...
    def _match_core_pattern(self, query: str) -> RouteMatch | None:
        """
    ...
    def _match_plugin_pattern(self, query: str) -> RouteMatch | None:
        """
    ...
    async def execute(
        self, user_query: str, context: dict | None = None
    ) -> dict[str, Any]:
        """
    ...
    async def _execute_core(
        self, query: str, route: RouteMatch, context: dict | None
    ) -> dict[str, Any]:
        """
    ...
    async def _execute_plugin(
        ...
        try:
            ...
            result = await sandbox.execute(
                route.intent_pattern, {"query": query, "context": context}
            )
    ...
    async def _create_sandbox(self, plugin_id: str) -> PluginSandbox | None:
        """
