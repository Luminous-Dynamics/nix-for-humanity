...
class ManifestValidator:
    ...
    def validate_manifest(self, manifest_path: Path) -> ValidationResult:
        ...
        return ValidationResult(
            valid=True, errors=errors, warnings=warnings, manifest_data=manifest_data
        )
    ...
    def _semantic_validation(self, manifest: dict) -> list[str]:
        ...
        required_perms = (
            manifest.get("capabilities", {}).get("permissions", {}).get("required", [])
        )
        if (
            "filesystem.write" in required_perms
            and "filesystem.read" not in required_perms
        ):
            warnings.append(
        ...
        data_sharing = (
            manifest.get("boundaries", {}).get("data_policy", {}).get("sharing")
        )
    ...
    def print_validation_report(
        self, result: ValidationResult, plugin_name: str = "Plugin"
    ):
        """
        ...
        if result.valid and result.manifest_data:
            principle = result.manifest_data.get("consciousness", {}).get(
                "governing_principle"
            )
            promise = result.manifest_data.get("consciousness", {}).get(
                "sacred_promise"
            )
