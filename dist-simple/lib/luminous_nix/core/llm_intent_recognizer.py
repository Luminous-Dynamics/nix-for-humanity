...
class LLMIntentRecognizer:
    ...
    def _init_llm(self):
        ...
        try:
            ...
            if self.llm_client.is_available():
                logger.info("LLM available for intent recognition")
            else:
                self.llm_client = None
        except ImportError:
    ...
    def recognize(self, text: str, context: Optional[Dict] = None) -> Intent:
        ...
        if self.llm_client and (pattern_intent.confidence < 0.7 or 
                                pattern_intent.type == IntentType.UNKNOWN):
            try:
    ...
    def _llm_classify(self, text: str, context: Optional[Dict] = None) -> Optional[Intent]:
        """Use LLM to classify intent."""
        
        prompt = self._build_classification_prompt(text)
        
        try:
            response = self.llm_client.query(prompt, temperature=0.1)  # Low temp for consistency
            
            # Parse LLM response
            intent_data = self._parse_llm_response(response, text)
...
JSON response:"""
    ...
    def _parse_llm_response(self, response: str, original_text: str) -> Optional[Dict]:
    ...
    def _log_intent_learning(self, text: str, pattern_intent: Intent, llm_intent: Intent):
...
class SmartIntentRouter:
    ...
    def recognize(self, text: str, **kwargs) -> Intent:
        ...
        if self.prefer_llm and self.recognizer.llm_client:
            ...
            self.recognizer.pattern_threshold = 0.95  # Only use patterns if very confident
