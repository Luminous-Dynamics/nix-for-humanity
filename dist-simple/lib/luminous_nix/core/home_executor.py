...
from .home_manager import HomeConfig, HomeManager
...
class HomeExecutor:
    ...
    def execute(
        ...
        try:
            ...
            if "add" in query_lower or "manage" in query_lower:
    ...
    def _add_dotfile(self, query: str, entities: dict[str, Any]) -> HomeResult:
        ...
        return HomeResult(
            ...
            command="home-manager switch",
            explanation="Run 'home-manager switch' to apply the new configuration",
        )
    ...
    def check_home_manager_installed(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["home-manager", "--version"], capture_output=True, text=True, timeout=5
            )
