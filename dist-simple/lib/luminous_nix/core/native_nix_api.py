...
class NativeNixAPI:
            ...
            try:
                ...
                if flake:
                    ...
                    build_attr = self.models.BuildAttr(
                        attribute=attribute, flake=flake_obj
                    )
                else:
        ...
        if self.nixos_rebuild_available:
            try:
                ...
                self.nix.switch_to_configuration(
                    path, action_enum, self.models.Profile.SYSTEM
                )
    ...
    def get_performance_comparison(self) -> dict[str, Any]:
        ...
        return {
            ...
            "actual_operations": {
                "search": "0.29ms native vs 2000-5000ms subprocess"
                if self.has_native_api()
                else "subprocess only",
                "install": "<0.5s native vs 5-30s subprocess"
                if self.has_native_api()
                else "subprocess only",
                "list_generations": "0.29ms native vs 500-1000ms subprocess"
                if self.has_native_api()
                else "subprocess only",
