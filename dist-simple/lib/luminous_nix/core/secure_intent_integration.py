...
from .intent_secure_wrapper import ProductionIntentRecognizer, create_production_recognizer
from .intents import IntentType, Intent
from .intent_pipeline import IntentRecognitionResult, Entity
...
class SecureIntentPipeline:
    ...
    def __init__(
        ...
        logger.info(f"Secure intent pipeline initialized with {security_level} security")
    
    def recognize(self, query: str, context: Optional[Dict] = None) -> IntentRecognitionResult:
        ...
        if not result.get('success'):
            ...
            if error_type in ['MALICIOUS_INPUT', 'ADVERSARIAL_INPUT']:
                logger.warning(f"Security threat blocked: {error_type} from user {user_id}")
            ...
            return IntentRecognitionResult(
                ...
                suggestions=[result.get('message', 'Input blocked for security reasons')],
                metadata={
                    'security_block': True,
        ...
        return IntentRecognitionResult(
            ...
            entities=entities,
            original_query=query,  # Add required field
            normalized_query=security_info.get('sanitized_query', query),  # Add required field
    ...
    def learn_correction(
        ...
        if success:
            ...
            logger.info(f"Learned correction: '{original_query}' -> {correct_intent.value}")
...
def create_secure_pipeline_for_cli(
    ...
    return SecureIntentPipeline(
        ...
        enable_learning=not os.environ.get('LUMINOUS_NO_LEARNING', '').lower() == 'true'
    )
