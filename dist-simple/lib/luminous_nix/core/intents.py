    ...
    def _match_patterns(self, text: str) -> Optional[Intent]:
        ...
        for pattern in self.discover_package_patterns:
            if match := re.search(pattern, text, re.IGNORECASE):
                ...
                search_term = None
                for group in match.groups():
                    if group and group not in ['discover', 'find', 'suggest', 'recommend', 'what', 'should', 'use', 'for', 'me', 'a', 'package', 'tool', 'app', 'application']:
        ...
        for pattern in self.find_by_command_patterns:
            if match := re.search(pattern, text, re.IGNORECASE):
                ...
                command = None
                for group in match.groups():
                    if group and group not in ['what', 'which', 'package', 'has', 'contains', 'provides', 'that', 'find', 'with', 'is', 'in', 'command']:
                        command = group.strip()
                        break
        ...
        for pattern in self.change_password_patterns:
            if match := re.search(pattern, text, re.IGNORECASE):
                ...
                username = None
                for group in match.groups():
                    if group and group not in ['change', 'reset', 'set', 'password', 'for', '\'s', 'new', 'my']:
                        username = group.strip()
                        break
        ...
        for pattern in self.install_patterns:
            if match := re.search(pattern, text, re.IGNORECASE):
                ...
                for i, group in enumerate(groups):
                    if group and group not in ['install', 'add', 'get', 'need', 'want', 'set up', 
                                              'can you', 'please', 'could you', 'I', 'would like']:
                        package = group
                        break
