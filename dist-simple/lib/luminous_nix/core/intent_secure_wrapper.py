...
from .intent_security import SecureIntentRecognizer, ThreatLevel
...
class ProductionIntentRecognizer:
            ...
            if self.security_enabled:
                ...
                if result.get('error'):
                    ...
                    if threat in ['MALICIOUS_INPUT', 'ADVERSARIAL_INPUT']:
                        logger.warning(f"High-severity threat detected: {threat} from user {user_id}")
    ...
    def _format_error_response(self, error_result: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            'details': error_result.get('reason') if self.security_level != "high" else None
        }
