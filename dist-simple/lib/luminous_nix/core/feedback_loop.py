...
class FeedbackLoop:
    ...
    async def process_feedback(self, signal: FeedbackSignal):
        ...
        if signal.type in self.processors:
            ...
            logger.debug(
                f"🔄 Processed {signal.type.value} feedback from {signal.source}"
            )
    ...
    async def _process_success(self, signal: FeedbackSignal) -> dict[str, Any]:
        ...
        if self.shared_state:
            ...
            self.shared_state.modify(
                "user_expertise", 0.02, max_val=1.0, source="feedback_loop"
            )
        ...
        if self.learning_system:
            ...
            await self.learning_system.store_pattern(
                pattern_type="success", data=signal.data
            )
    ...
    async def _process_failure(self, signal: FeedbackSignal) -> dict[str, Any]:
        ...
        if self.shared_state:
            ...
            self.shared_state.modify(
                "frustration_level", 0.1, max_val=1.0, source="feedback_loop"
            )

        # Learn failure patterns to avoid
        if self.learning_system:
            await self.learning_system.store_pattern(
                pattern_type="failure", data=signal.data
            )
    ...
    async def _adapt_explanation_depth(self, delta: float):
        ...
        self.parameters["verbosity"] = max(
            0.0, min(1.0, self.parameters["verbosity"] + delta)
        )
    ...
    async def _adapt_assistance_level(self, delta: float):
        ...
        self.parameters["proactivity"] = max(
            0.0, min(1.0, self.parameters["proactivity"] + delta)
        )
        logger.info(
            f"🤝 Assistance level adjusted: {self.parameters['proactivity']:.2f}"
        )
    ...
    def get_feedback_summary(self) -> dict[str, Any]:
        ...
        successes = sum(
            1 for s in self.feedback_history if s.type == FeedbackType.SUCCESS
        )
        failures = sum(
            1 for s in self.feedback_history if s.type == FeedbackType.FAILURE
        )

        success_rate = (
            successes / (successes + failures) if (successes + failures) > 0 else 0.5
        )
    ...
    async def create_feedback_from_event(
        ...
        signal = FeedbackSignal(type=feedback_type, source=event_type, data=event_data)
