...
def create_intent_recognizer(config: Optional[IntentRecognitionConfig] = None):
    """
    ...
    if config.enable_llm:
        try:
            ...
            if insights['capabilities']['ai_available']:
                logger.info(f"🤖 Enhanced intent recognition enabled (mode: {config.mode})")
            else:
    ...
    def recognize(self, text: str, **kwargs):
        ...
        if self._is_enhanced:
            ...
            return self._recognizer.recognize(
                text, 
                explain=self.config.explain_recognition
            )
        else:
            # Basic recognizer
