...
class FlakeExecutor:
    ...
    def execute(
        ...
        try:
            ...
            if intent_type == "develop" or "dev environment" in query.lower():
    ...
    def _create_flake(self, query: str, entities: dict[str, Any]) -> FlakeResult:
        ...
        return FlakeResult(success=False, message=message, error=message)

    def _create_dev_environment(
    ...
    def check_flake_support(self) -> bool:
        ...
        try:
            ...
            config_result = subprocess.run(
                ["nix", "show-config"], capture_output=True, text=True, timeout=5
            )

            return (
                "experimental-features" in config_result.stdout
                and "flakes" in config_result.stdout
            )
