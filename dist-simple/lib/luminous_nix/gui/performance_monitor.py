...
class PerformanceMonitor:
    ...
    def __init__(self, db_path: str = None):
        if db_path is None:
            db_path = Path.home() / '.local' / 'share' / 'luminous-nix' / 'performance.db'
    ...
    def get_metrics(self, 
                    ...
                    persona: Optional[str] = None) -> List[PerformanceMetric]:
    ...
    def calculate_summary(self, date: Optional[datetime] = None) -> Dict:
        ...
        summary = {
            ...
            "pattern_reuse_rate": sum(1 for m in metrics if m.pattern_reuse) / len(metrics)
        }
        ...
        for metric in recent:
            ...
            if metric.component_count > 5:
                bottlenecks["complex_requests"].append({
                    "request": metric.request,
                    "components": metric.component_count
                })
    ...
    def generate_report(self) -> str:
        ...
        if today_summary:
            ...
            report.append(f"Avg Generation Time: {today_summary['avg_generation_time']:.2f}ms")
        ...
        if trends["dates"]:
            ...
            if len(trends["generation_times"]) >= 2:
                time_trend = trends["generation_times"][-1] - trends["generation_times"][0]
                trend_symbol = "📈" if time_trend > 0 else "📉"
                report.append(f"Generation Time: {trend_symbol} {abs(time_trend):.2f}ms")
    ...
    def visualize_dashboard(self):
        ...
        if recent:
            ...
            print(f"Avg Time: {sum(m.generation_time for m in recent)/len(recent):.2f}ms")
            print(f"Success Rate: {sum(1 for m in recent if m.success)/len(recent):.0%}")
        ...
        if trends["generation_times"]:
            ...
            for i, (date, value) in enumerate(zip(trends["dates"], trends["generation_times"])):
...
def run_performance_test():
    ...
    for request, persona in test_cases:
        context = UserContext(
            user_id=f"{persona}_user",
            expertise_level=persona
        )
