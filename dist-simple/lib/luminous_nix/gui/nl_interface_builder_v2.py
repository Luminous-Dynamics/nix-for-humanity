...
from hybrid_nlp_llm import HybridNLPLLM, EnhancedIntent
...
try:
    from .nl_interface_builder import UserContext, InterfaceSpecification
except ImportError:
    from nl_interface_builder import UserContext, InterfaceSpecification
...
class NLInterfaceBuilderV2:
    ...
    def _prepare_parser_context(self, 
                                request: str, 
                                user_context: Optional[UserContext]) -> Dict:
    ...
    def _handle_ambiguities(self, 
                           intent: EnhancedIntent, 
                           request: str,
                           context: Optional[UserContext]) -> EnhancedIntent:
        ...
        if "Vague terminology" in intent.ambiguities:
            ...
            if context and context.expertise_level == "beginner":
                ...
                intent.components_needed = [
                    {"type": "display", "properties": {"simple": True}, "priority": "high"}
                ]
                ...
                intent.components_needed.append(
                    {"type": "advanced_controls", "properties": {}, "priority": "medium"}
                )
    ...
    def _generate_components_from_intent(self, 
                                        intent: EnhancedIntent,
                                        context: Optional[UserContext]) -> List[SynthesizedComponent]:
    ...
    def _determine_visual_style(self, 
                               intent: EnhancedIntent,
                               context: Optional[UserContext]) -> str:
        """Determine visual style from intent and context"""
        
        # Check explicit style preferences
        if 'minimal' in intent.style_preferences:
            return 'minimal'
        elif 'playful' in intent.style_preferences:
            return 'playful'
        elif 'professional' in intent.style_preferences:
            return 'serious'
    ...
    def _determine_color_scheme(self, 
                               intent: EnhancedIntent,
                               context: Optional[UserContext]) -> str:
        ...
        if 'dark' in intent.style_preferences:
            return 'dark'
        elif 'light' in intent.style_preferences:
            return 'light'
    ...
    def _compose_interface(self, 
                          ...
                          intent: EnhancedIntent,
                          context: Optional[UserContext]) -> InterfaceSpecification:
        ...
        theme = {
            ...
            'accessibility': context.preferences.get('accessibility', 'standard') if context else 'standard'
        }
        ...
        data_bindings = {
            ...
            'update_strategy': 'realtime' if 'realtime' in intent.style_preferences else 'on_demand',
            'cache': True
        }
    ...
    def _determine_gestures(self, context: Optional[UserContext]) -> List[str]:
    ...
    def refine_interface(self, 
        ...
        if refinement_intent.action == 'modify':
            ...
            # Add new components
            new_components = self._generate_components_from_intent(refinement_intent, context)
            ...
            if refinement_intent.target:
                interface.components = [
                    c for c in interface.components 
...
if __name__ == "__main__":
    ...
    test_cases = [
        ...
        # Complex request
        ("Build a real-time dashboard showing server metrics with CPU, memory, and disk usage charts, dark theme, and automatic refresh every 5 seconds", "expert"),
        
        # Ambiguous request
        ("Make it better", "intermediate"),
        
        # Conversational
        ("Actually, can you make the charts bigger and add some animations?", "intermediate")
    ]
