...
class UIGeneratorCLI:
    ...
    def _get_user_context(self, user_id: Optional[str] = None) -> UserContext:
    ...
    def list_interfaces(self) -> List[Dict]:
        ...
        interfaces_dir = Path.home() / '.local' / 'share' / 'luminous-nix' / 'interfaces'
        ...
        for spec_file in sorted(interfaces_dir.glob("*.json"), reverse=True)[:20]:
            with open(spec_file, 'r') as f:
                ...
                interfaces.append({
                    "id": spec_file.stem,
                    "created": spec.get("metadata", {}).get("generated_at", "unknown"),
    ...
    def export_interface(self,
        ...
        if output_path is None:
            output_path = Path.cwd() / f"interface_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{format}"
        else:
    ...
    def _load_last_interface(self):
        ...
        last_file = Path.home() / '.local' / 'share' / 'luminous-nix' / 'interfaces' / 'last.json'
    ...
    def _load_interface(self, interface_id: str):
        ...
        spec_file = Path.home() / '.local' / 'share' / 'luminous-nix' / 'interfaces' / f"{interface_id}.json"
        if not spec_file.exists():
            # Try as full filename
            spec_file = Path.home() / '.local' / 'share' / 'luminous-nix' / 'interfaces' / f"interface_{interface_id}.json"
    ...
    def _load_interface_from_file(self, spec_file: Path):
        ...
        from component_synthesis_engine import SynthesizedComponent, ComponentDNA
...
"""
        ...
        for comp in interface.components:
            html += f'    <div class="component">{comp.name}: {comp.dna.purpose}</div>\n'
...
def integrate_with_ask_nix():
    ...
    def add_ui_commands(subparsers):
        ...
        create_parser = ui_subparsers.add_parser('create', help='Create interface from natural language')
        ...
        show_parser.add_argument('--last', action='store_true', help='Show last interface')
        ...
        export_parser.add_argument('--format', choices=['json', 'html', 'python'], default='json')
        export_parser.add_argument('--output', help='Output path')
        export_parser.add_argument('--id', help='Interface ID to export')
        
        # Statistics command
        stats_parser = ui_subparsers.add_parser('stats', help='Show UI generation statistics')
    ...
    def handle_ui_command(args):
        ...
        if args.ui_command == 'create':
            ...
            if result['success']:
                ...
                print(f"❌ Failed to create interface")
            ...
            if result['success']:
                print(f"✅ Interface refined successfully!")
                print(f"   Components: {result['components']}")
            else:
...
Examples:
  ...
  %(prog)s stats
    ...
    if args.command == 'create':
        ...
        interfaces = cli.list_interfaces()
        for intf in interfaces:
            print(f"{intf['id']}: {intf['request'][:40]}... ({intf['components']} components)")
