...
class IntentParser(ABC):
    ...
    @abstractmethod
    def parse(self, request: str, context: Optional[Dict] = None) -> EnhancedIntent:
        pass
...
class RuleBasedNLP(IntentParser):
    ...
    def __init__(self):
        ...
        self.patterns = {
            'actions': {
                'create': re.compile(r'\b(create|make|build|generate|design|construct)\b', re.I),
                'show': re.compile(r'\b(show|display|present|view|see|list)\b', re.I),
                'modify': re.compile(r'\b(change|modify|update|edit|adjust|alter)\b', re.I),
                'analyze': re.compile(r'\b(analyze|inspect|examine|review|check)\b', re.I),
            },
            'interfaces': {
                'dashboard': re.compile(r'\b(dashboard|overview|summary|panel|monitor)\b', re.I),
                'form': re.compile(r'\b(form|input|entry|submission|questionnaire)\b', re.I),
                ...
                'chart': re.compile(r'\b(chart|graph|plot|visualization|diagram)\b', re.I),
            },
            'styles': {
                'dark': re.compile(r'\b(dark|night|black)\s*(theme|mode)?\b', re.I),
                'minimal': re.compile(r'\b(minimal|simple|clean|basic|zen)\b', re.I),
                'playful': re.compile(r'\b(fun|playful|colorful|animated|vibrant)\b', re.I),
            },
            'components': {
    ...
    def _calculate_confidence(self, action: str, interface_type: str, components: List) -> IntentConfidence:
        ...
        components_conf = min(0.9, 0.3 * len(components))  # More components = more confident
...
Be specific and actionable. If the request is vague, make reasonable assumptions and note them in ambiguities."""
    ...
    def _prepare_prompt(self, request: str, context: Optional[Dict]) -> str:
    ...
    def _mock_llm_response(self, prompt: str) -> str:
        ...
        response = {
            "action": "create",
            "interface_type": "dashboard" if "dashboard" in prompt.lower() else "general",
...
class HybridNLPLLM:
    ...
    def _merge_intents(self, 
                       nlp_intent: EnhancedIntent, 
                       llm_intent: EnhancedIntent) -> EnhancedIntent:
        ...
        merged.confidence = IntentConfidence(
            overall=(nlp_intent.confidence.overall + llm_intent.confidence.overall) / 2 + 0.1,  # Bonus for hybrid
            action=max(nlp_intent.confidence.action, llm_intent.confidence.action),
            interface_type=max(nlp_intent.confidence.interface_type, llm_intent.confidence.interface_type),
            components=max(nlp_intent.confidence.components, llm_intent.confidence.components),
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            **self.stats,
            ...
            'cache_hit_rate': (self.stats['cache_hits'] / (total + self.stats['cache_hits'])) * 100
        }
