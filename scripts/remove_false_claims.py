...
def fix_file(filepath: Path) -> bool:
    ...
    replacements = [
        ...
        (r'native Python-Nix API', 'subprocess-based operations'),
        (r'Native nixos-rebuild-ng API loaded', 'Using subprocess (nixos-rebuild-ng not available)'),
    ...
    content = re.sub(
        r'"performance_boost":\s*"[^"]*"',
        '"performance_boost": "1x"',
        content
    )
...
def main():
    ...
    files_to_check.extend((project_root / "docs").glob("**/*.md") if (project_root / "docs").exists() else [])
