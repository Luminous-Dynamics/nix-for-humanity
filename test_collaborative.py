...
def test_basic_network():
    ...
    print(f"\n📊 Network Statistics:")
    print(f"   Node 1 - Peers: {stats1['peer_count']}, Entries: {stats1['shared_entries']}")
    print(f"   Node 2 - Peers: {stats2['peer_count']}, Entries: {stats2['shared_entries']}")
...
def test_multi_node_collaboration():
    ...
    unique_entries = len(set(
        entry.query
        for node in nodes
        for entry in node.shared_cache.values()
    ))
...
def test_resilience():
    ...
    for i, node in enumerate(nodes):
        print(f"   Node {remaining_nodes[i]}: {len(node.shared_cache)} entries, {len(node.peers)} peers")
